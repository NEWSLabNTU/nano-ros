...
set -euo pipefail
cd "$(dirname "$0")/.."
# No `grep -q` here any more, so `scripts/lib/grep-q.sh` is no longer sourced:
# its two call sites were the baseline lookup and the stale-entry sweep, and
# phase-456 W6 removed both with the ratchet. A sourced helper nobody calls
# reads as a claim about this file that is not true.
...
'
  ...
  # 10. phase-456 W6 — THE MUTATION, on a REAL header. The cases above drive the
