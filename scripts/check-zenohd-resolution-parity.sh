...
# Resolved ABSOLUTELY, because the per-case `PATH=...` prefix assignment applies
# to the command lookup as well as to the command: with PATH pointing at the
# synthetic bin dir, `PATH=... bash -c` cannot find `bash`. Every case then
# "resolved nothing", which the table happened to accept for one row and reject
# for eight — an informative failure only because the parse guard above had
# already stopped the silent version of it.
...
while IFS=$'\x1f' read -r name explicit path ament distro optros expected; do
    ...
    ran=$((ran + 1))
    explicit="$(expand "$explicit")"; path="$(expand "$path")"
    ament="$(expand "$ament")";       expected="$(expand "$expected")"
    optros="$(expand "$optros")"

    got="$(
        NROS_RMW_ZENOHD="$explicit" PATH="$path" AMENT_PREFIX_PATH="$ament" \
