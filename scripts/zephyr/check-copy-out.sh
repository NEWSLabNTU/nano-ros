...
# `nros` CLI (Phase 218: resolved from $NROS_CLI / PATH /
# packages/cli/target/release/nros / ~/.nros/bin via
# scripts/build/cargo.sh::nros_cli_bin).
nros_cargo_ensure_codegen_c
codegen_tool="$(nros_cargo_codegen_c_bin)"
[ -x "$codegen_tool" ] || { echo "FAIL: nros CLI not found ($codegen_tool); run just setup-cli"; exit 1; }
