...
def declared_capability_patterns(path=None):
    """The reasons a lane may legitimately skip on, as compiled patterns.

    A MISSING file means nothing is declared, which makes every capability skip
    a failure. That is the safe direction: the alternative — treating an absent
    file as "allow everything" — is how a ratchet silently stops ratcheting.
    """
    path = path or CAPABILITY_BASELINE
    out = []
    try:
...
def self_test() -> int:
    ...
    # reason is accepted, that an undeclared one is NOT, and that a MISSING
    # baseline denies everything rather than allowing everything.
    import tempfile

    cap_failures = 0

    def cap_case(label, patterns, reason, want_allowed):
        nonlocal_failures = 0
        with tempfile.NamedTemporaryFile("w", suffix=".txt", delete=False) as fh:
            fh.write("# generated by --self-test\n" + "\n".join(patterns) + "\n")
            path = fh.name
        try:
            pats = declared_capability_patterns(path)
            allowed = any(p.search(reason) for p in pats)
        finally:
            os.unlink(path)
    ...
    cap_failures += cap_case(
        "a declared reason is allowed",
        [r"no `rmw_zenoh_cpp/rmw_zenohd` found"],
        "no `rmw_zenoh_cpp/rmw_zenohd` found. Source your ROS setup",
        True,
    )
    cap_failures += cap_case(
        "an UNdeclared reason is refused",
        [r"no `rmw_zenoh_cpp/rmw_zenohd` found"],
        "idlc not found on PATH",
        False,
    )
    cap_failures += cap_case(
        "an empty declaration refuses everything",
        [],
        "no `rmw_zenoh_cpp/rmw_zenohd` found",
        False,
    )
    # A missing FILE must deny, not allow — the direction that keeps a ratchet
    # ratcheting when someone deletes it.
    missing = declared_capability_patterns("/nonexistent/capability-baseline.txt")
    ok = missing == []
    print("  %-52s %s" % ("a missing baseline file declares nothing", "ok" if ok else "FAILED"))
...
def main(argv: list[str]) -> int:
    ...
    surprises: list[str] = []
    laundered: list[str] = []
    # issue 1161 — capability skips this lane never declared it may take.
    undeclared_caps: list[tuple[str, str]] = []
    declared_caps = declared_capability_patterns()
    ...
    for case, cls, text in skips(root):
        ...
        # issue 1161 — a capability skip is a provisioning gap, and an
        # UNDECLARED one means the lane quietly did less than it claimed. The
        # same rule 0584 applied to fixtures, one class over.
        if cls == "capability":
            reason = reason_of(text)
            if not any(p.search(reason) for p in declared_caps):
                undeclared_caps.append((ident, reason))
    ...
    if undeclared_caps:
        print("", file=sys.stderr)
        print(
            f"ERROR: {len(undeclared_caps)} test(s) skipped for a capability this lane "
            "never declared it may lack:", file=sys.stderr,
        )
        seen = set()
        for ident, reason in undeclared_caps:
            if reason in seen:
                continue
            seen.add(reason)
            print(f"  {reason}", file=sys.stderr)
            print(f"      first seen: {ident}", file=sys.stderr)
        print(
            "  Issue 0584 made an absent in-lane FIXTURE a hard failure. This is the\n"
            "  same sentence one class over: the thing the test needs is not here, the\n"
            "  run did less than it claimed, and it reported pass (issue 1161).\n"
            "  Provision it, narrow the lane so it does not claim these tests, or --\n"
            "  if some host may legitimately lack it -- declare it in\n"
            "  .config/capability-skip-baseline.txt with what would retire the line.",
            file=sys.stderr,
        )
        rc = 1
    if laundered:
