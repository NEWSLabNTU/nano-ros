...
PLATFORM_DIR="$REPO_ROOT/packages/zpico/zpico-sys/c/platform"
...
# Generate zenoh_generic_config.h (required when -DZENOH_GENERIC is set).
# Matches the config header generated by zpico-sys/build.rs for bare-metal
# single-threaded TCP. This standalone build is a build-readiness check;
# the real library is produced by zpico-sys/build.rs during `cargo build`.
mkdir -p "$BUILD_DIR/zenoh-config"
cat > "$BUILD_DIR/zenoh-config/zenoh_generic_config.h" << 'EOF'
/**
 * zenoh_generic_config.h - Generated by build-zenoh-pico.sh (ESP32-C3)
 *
 * Matches the config header generated by zpico-sys/build.rs for bare-metal.
...
# Note: No system.c/network.c — bare-metal platform symbols (clock, memory,
# network, etc.) are provided by Rust FFI through the zpico-platform-* crates.
# This standalone build is a compile-only readiness check for zenoh-pico core
# + zpico shim. The actual library comes from zpico-sys/build.rs.
