...
- **C++ API**: `nros-cpp` freestanding C++14 over typed extern "C" FFI to `nros-node`. Mirrors rclcpp. Error `nros::Result` + `NROS_TRY`. Codegen `cargo nano-ros generate-cpp` or CMake `nano_ros_generate_interfaces(... LANGUAGE CPP)`. Std opt-in via `NROS_CPP_STD`. Zephyr `CONFIG_NROS_CPP_API=y`. Action/service C/C++ clients are executor-spin-driven (no blocking `zpico_get`) since Phase 77 (archived); the old FreeRTOS-QEMU hang is fixed (`LWIP_NETCONN_SEM_PER_THREAD=1`). Native **cyclonedds** C++ action `get_result`/feedback receive is still open (Phase 171.0.b) — C+C and rust+rust cyclonedds actions work e2e; **cpp+cpp** times out (`-2`) on `get_result`. Diagnosed 2026-05-21 (do NOT re-derive): the goal-service reply works (uses blocking `call_raw`), but the **async service path** (`send_request_raw` + arena-polled `try_recv_get_result_reply`) fails *only* for the cpp combo. Traced through the Cyclone backend: reply writer↔reader **match fine** (`proxy_reader_add_connection`), `send_reply` returns ok=true, and `service_try_recv_reply_raw` **receives + correlates the reply correctly** (seq+guid match). The break is *above* the backend: a get_result request+reply round-trips during the cpp client's feedback-poll phase (before `nros_cpp_action_client_get_result` is even called), the result trampoline never fires, and the seq counter shows two `seq=0` sends (`next_seq` not advancing as expected) — i.e. a duplicate/early get_result send whose reply is consumed without propagating to `BLOCKING_RESULT_LEN`. Next step: find the unexpected early `send_get_result_request` caller (not the example, not the header `get_result`) and why the arena dispatch consumes its matched reply without firing `cpp_result_trampoline`. Cross-impl pairings (rust↔C↔cpp) also diverge on feedback/result CDR framing — only same-language pairs are supported. Feedback loss in the stock cpp example is a *separate* secondary issue (server computes instantly inside the goal callback → volatile feedback dropped before the reader matches; C/rust pace with delays + execute after accept).
