...
- **Messages**: `nros generate-rust` from `package.xml`. **Never hand-write.** Example `generated/` gitignored. Only `packages/interfaces/rcl-interfaces/generated/<edition>/` in git (`nros-` prefix, e.g. `humble/nros-builtin-interfaces`). The bundled base interfaces (`std_msgs`, `builtin_interfaces`, …) now ship inside the external `nros` CLI, not an in-tree `packages/codegen/interfaces/` (retired with the codegen submodule). `nros-core` re-exports `heapless`.
