...
| What an image DECLARES, for whoever sizes a buffer from it | `<build>/nros/sizing/<entry>.toml`, written by `nros sync` (RFC-0100 D4, phase-454 W4). Read it through `nros-sizing-descriptor` — never hand-parse it, and never re-carry a fact through an env knob (issues 0460/0491). Per-FIELD status: a value or a `[<section>.refused]` reason, never a silent default. `just` has no verb; inspect with `nros ws sizing-descriptor --descriptor <path>`. **Written on ONE road — a single-package cargo leaf** (`system.toml` beside a `[package]` manifest), which names it to cargo as a `relative = true` `[env]` row in `build/<image>/nros-cargo.toml`; a workspace image and every cmake/west road write none, so every D5 derivation is INERT there and the blocker is the producer, not the variable (phase-454 W11, measured) |
