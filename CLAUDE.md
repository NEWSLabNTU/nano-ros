...
| 100 | AGX Orin SPE infrastructure (Cortex-R5F + IVC) — Cortex-R critical-section feature on `nros-platform-freertos`, `armv7r-none-eabihf` toolchain wiring, `Z_FEATURE_LINK_IVC` in vendored zenoh-pico (POSIX-mock + real `tegra_ivc_channel_*`), `nros-board-orin-spe` against NVIDIA FSP (workspace-excluded; user-supplied SDK), POSIX mock end-to-end smoke test. Application-side port lives in `autoware_sentinel` Phase 11. | Not Started |
