...
| ROS 2 on a host with no apt ROS (Arch, Fedora, NixOS) | [docs/development/ros2-on-non-ubuntu.md](docs/development/ros2-on-non-ubuntu.md) — a setup script for an Ubuntu distrobox, and nothing else. **The build system asks ONE question about an environment: are ROS 2 ament packages discoverable, so msg packages can be found? Whether that Linux is bare metal, a container or a VM is the operator's business and must not be encoded** (issue 1248). **BUILD WHERE YOU RUN** — clone nano-ros INSIDE the box and work there; never build on the host and run in the box, and never mirror one tree into the other. `ros2-box-sync.sh` did the latter and is RETIRED: every bug it had was a bug in the copy (#758, #1229, #0925, #1243), and its rules could not be right by construction because provisioned source and build output share directories. `$HOME` is SHARED with a distrobox, so `ros2-box-env.sh` gives the inner toolchain its own store — one knob, not a box concept |
...
| What an image DECLARES, for whoever sizes a buffer from it | `<build>/nros/sizing/<entry>.toml`, written by `nros sync` (RFC-0100 D4, phase-454 W4). Read it through `nros-sizing-descriptor` — never hand-parse it, and never re-carry a fact through an env knob (issues 0460/0491). Per-FIELD status: a value or a `[<section>.refused]` reason, never a silent default. `just` has no verb; inspect with `nros ws sizing-descriptor --descriptor <path>`. **TWO producers, one composer** (phase-454 W12/W14). A single-package cargo LEAF (`system.toml` beside a `[package]` manifest) has all three inventories and states everything; a workspace cargo image and every cmake/west/NuttX entry have only the resolved SystemModel, so `write_for_model` states the counts and all four QoS policies and **REFUSES** `wire_bound_bytes`, `storage_bytes`, `[types]`'s three maxima and `registration_path`, every reason naming **issue 1393**. Refusal is what makes a partial descriptor safe: `Fact::stated()` is the only accessor that yields a value, and across all 7 consumers (each built twice, knobs diffed) **no output difference is caused by a refusal** — every one is a fact the contract STATED. **No contract ⇒ no file**, because an all-refused descriptor moves the `[meta] basis` every consumer guards on to say nothing. Named to cargo as a `relative = true` `[env]` row (leaf + workspace) or on the emitted Corrosion command (cmake — issue 0460, never `set(ENV{})`) |
| Where a leaf DECLARES its QoS, and how it reaches that descriptor | **`<leaf>/system.contract.yaml`**, beside `system.toml` (phase-454 W12) — a leaf's launch file is SYNTHESISED into a generated dir, so the provider-sidecar channel the resolver reads has nowhere a user could author; `nros sync` carries the file there. It is joined onto the probe's rows by `nros_cli_core::contract_join` on the ONE key both inventories can state: **the endpoint's NAME**. The probe keys rows by `id`, which for a subscription is the CALLBACK name, so joining on that publishes a depth against the wrong endpoint — an UNDER-size. A row is attributed only when the node declares NO remaps, the written name is ABSOLUTE, and `(kind, type, name)` picks out one row **on each side**; anything else REFUSES per row, keeps `undeclared_endpoints` counting it, and every consumer keeps its worst case. No contract ⇒ the rows are untouched and the image is byte-identical. Measured: `examples/native/rust/listener` 273,802 → 172,298 B (−37 %) from one `KEEP_LAST(1)` row |
| Verify the book's setup flow on a pristine host | `just probe bootstrap` — runs the `probe=NN`-tagged book blocks in a clean container (`scripts/probe/`, issue 0204), down BOTH front doors: the checkout track, and `just probe installed` (phase-447 A3) — builds the asset `release-nros.yml` would publish, then `first-project.md` on a host with NO checkout |
...
- **Launch toolchain (RFC-0060, amended to TWO repositories — phase-332 W1/W2 landed):** nano-ros
  ...
  msgs) are never built by nano-ros. `ros-launch-manifest` (spec) is a git-TAG cargo dep — ONE tag
  ...
  in `play_launch/tests/`, which is its pin, not ours. **This line no longer states the version,
  because stating it here has drifted twice** — it read `v0.1.0` until 2026-08-18 and `v0.1.8`
  until 2026-09-22, when the manifests already said `v0.1.37`. Read it where it is true:
  `git grep 'ros-launch-manifest.*tag' -- '*/Cargo.toml'`.
...
- **An inherited absolute path outranks the checkout you are building, and a
  ...
  **The `just` side was a LEXICAL PREFIX REWRITE, which is not the rule**
  ...
  become the same lexical test, and the rewrite fired on the DEFAULTS — 9 of 21
  exports came out doubled and surfaced as `fatal error: nros/platform.h: No
  such file or directory`, i.e. as a missing source file rather than a broken
  environment. Only the DEEPEST owning checkout separates the two rows, which is
  per-VALUE and no prefix can answer it. Gate: `check-inherited-checkout-paths`,
  whose probes built synthetic checkouts SIDE BY SIDE only (the 0196 reach gap)
  and now run in both shapes.
...
- **0196 runs the OTHER way too: a reach WIDER than the rule is a false report,
  ...
  the old measurement.
- **rustdoc has TWO scopes, and `RUSTDOCFLAGS=-D warnings` is what makes the wider
  one answer at all** — `just check rustdoc-links` (the six DEPLOYED crates) +
  `just check rustdoc-workspace` (every other member, DERIVED). `cargo doc` exits
  0 over a warning, and workspace-level deny reaches only crates that opted in.
  → issue 1116 (archived) / phase-452 W4.
...
- **A CONFIGURE-time emitter has no `DEPENDS` to carry its tool — put the tool in
  ...
  **A TOOL edge is not enough: it names the binary that BUILD DIR cached, and a
  persistent store workspace outlives the checkout that configured it — three
  nights of tier 2 reached no cell while every mtime edge read current (issue
  1360). So the EMITTED VERSION is a freshness input too, read from
  `codegen_version.rs` and never from a stale `nros --codegen-version`; both
  rules are `check-codegen-tool-reconfigure`.
