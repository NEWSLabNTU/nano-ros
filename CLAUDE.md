...
| 95 | Example coverage parity — close the `(platform × lang × backend × use-case)` matrix on already-supported backends. Landed (19 crates): A Zephyr xrce-rust svc/action, B Zephyr dds-rust svc/action/async, C Zephyr cpp-xrce, F native dds-rust svc/action. Deferred (33 crates) behind named prerequisites: D Zephyr cpp-dds + E Zephyr c-dds — Phase 71.6 (cortex_a9 `#[global_allocator]` for nros-c/nros-cpp staticlibs) + nros-rmw-dds dual-feature struct refactor (`std`+`nostd-runtime` simultaneous activation); G native c-dds + H native cpp-dds — Phase 78 (per-RMW install prefix or component layout, since `find_package(NanoRos)` ships one RMW per install dir) | Partial — A/B/C/F complete; D/E/G/H deferred behind named prerequisites |
