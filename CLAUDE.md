...
| 95 | Example coverage parity — closed the `(platform × lang × backend × use-case)` matrix on already-supported backends with 51 new example crates: A Zephyr xrce-rust svc/action (4), B Zephyr dds-rust svc/action/async (5), C Zephyr cpp-xrce (6), D Zephyr cpp-dds (6), E Zephyr c-dds (6), F native dds-rust svc/action (4), G native c-dds (6), H native cpp-dds (6). In-phase prerequisites landed: Phase 71.6 (Zephyr `#[global_allocator]` + critical-section impl + cortex_a9 Rust target wiring for nros-c/nros-cpp staticlibs), `nros-rmw-dds` dual-feature struct refactor (`std`+`nostd-runtime` simultaneous activation), `dds` added to `install-local-posix`'s RMW loop (per-RMW lib namespacing already worked). Cross-instance/process E2E for B-cortex_a9, C-cpp/xrce, F-native-svc/action remain `#[ignore]`d behind unrelated dust-dds SEDP / xrce-cpp-API session-demux bugs — example crates themselves all build and reach readiness | Complete |
| 96 | Phase 95 cross-process E2E follow-ups — three independent fixes that each turn one or more `#[ignore]`d tests back on: 96.1 nros-cpp xrce `Subscription::try_recv()` demux (cpp shape doesn't receive on a shared XRCE Agent — rust + c shapes work fine), 96.2 `test_talker_param_declaration` flake under `just test-all` load (passes solo), 96.3 cross-link to Phase 71.28 / 71.29 (dust-dds service SEDP discovery + Cortex-A9 GEM RX queue tuning) | Not Started |
| 97 | Zero-copy raw pub/sub API — `SlotLending` + `SlotBorrowing` traits in `nros-rmw` (GAT-based), `PublishLoan` + `RecvView` in `nros-node` w/ per-publisher `TxArena` (97.A–97.E arena path; 97.F zenoh-pico via `z_bytes_from_static_buf`; 97.G XRCE-DDS via `uxr_prepare_output_stream`); compile-time gated behind `lending` / `rmw-lending` features | In Progress (traits + arena + Zenoh + XRCE lending impls landed; uORB arena path landed) |
