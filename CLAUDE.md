...
**`nros setup` is the canonical provisioner** (Phase 187 index → 195 standalone CLI → 197 `just`→`nros` migration → 218 monorepo merge). All SDK / toolchain / transport-source fetching is done by the `nros` CLI, **index-driven from `nros-sdk-index.toml`** — the SSoT for every pin (`dest`/`ref`/`submodule` come from data, never a hardcoded path). The CLI **ships from the in-tree sub-workspace at `packages/cli/`** (Phase 218); built via `just setup-cli` which runs `cargo build --release --manifest-path packages/cli/Cargo.toml --bin nros`, producing `packages/cli/target/release/nros`. One checkout = one CLI version = one runtime ABI — no `NROS_VERSION` pin to police. `scripts/install-nros-prebuilt.sh` is the no-Rust path: fetches the tagged-release `nros-<triple>.tar.gz` from this repo's GitHub release (Phase 218.G), sha256-verifies, drops the binary at `packages/cli/target/release/nros`. `just setup base` / `scripts/bootstrap.sh` build the in-tree CLI from source; `scripts/build/cargo.sh::nros_cli_bin` resolves it via **`$NROS_CLI` → PATH → `packages/cli/target/release/nros` → `~/.nros/bin` (transitional) → error**. The pre-218 `scripts/install-nros.sh` curl shim is deleted. Surfaces: `nros setup <board>` (a board's toolchain + sources), `--tool <t>` (one tool, e.g. `zenohd`/`qemu`; `--prefix` places it where the harness already looks, e.g. `build/zenohd`), `--source <s>` (one `[source.*]`, e.g. `zenoh-pico`/`cyclonedds-src`/`px4-rs` — replaces a hand `git submodule update <path>`), `--rmw <zenoh|xrce|cyclonedds>`, `--list` / `--licenses` / `--dry-run`. **The `just <module> setup` recipes are thin callers of these** — never inline a `git submodule update <path>` or a hardcoded SDK pin in a recipe; add the pin to `nros-sdk-index.toml` and call `nros setup`. `just setup [tier]` only orchestrates the module recipes (tiers below); `just doctor` is read-only readiness.
...
**Zephyr version support** (`docs/development/zephyr-version-support.md`): a supported config is a `(zephyr × zephyr-lang-rust)` pair — the **floor is 3.7** (the official in-tree Rust module didn't exist below it, so nothing below can link the nros Rust staticlib). Window = current LTS (default, `NROS_ZEPHYR_VERSION=3.7`/`west.yml`) + ≤1 rolling (`4.4`/`west-4.4.yml`). Per-line patches are dispatched from `scripts/zephyr/patches/<line>.sh` (no inline version branches in `just/zephyr.just`); adding a line = drop a patch set + pin the pair (checklist in the doc).
...
## Environment
`.env` (gitignored). **Run ONE of `direnv allow`, `source ./activate.sh` (bash/zsh), or `source ./activate.fish` (fish) once after clone** else `zpico-sys/build.rs` panics `"FREERTOS_PORT not set"`. The activate files are the Phase 218.C SSoT for env exports + PATH wiring (puts `packages/cli/target/release/` on PATH so the in-tree `nros` resolves).
...
- **Sweep contract for agents**: every `just <plat>` invocation needs `source ./activate.sh` first (PATH wires `nros`, `play_launch_parser`, `zenohd`, etc.). Without it, codegen-system / launch / RMW selection hit "binary not found" early — `nros plan` shells out to `play_launch_parser` via PATH lookup; the install-stamp alone doesn't help. Driver-loop scripts MUST source it before fixture builds; the pre-218 pattern `export PATH="$HOME/.nros/bin:$PATH"` is insufficient (misses `play_launch_parser` under `~/.nros/sdk/play_launch_parser/bin/`). `just doctor` enforces this — `[PATH]` state (installed but not on PATH) exits 1 with the `source ./activate.sh` hint. Cross-ref Phase 220.A.3 / 220.J.
...
- **Messages**: `nros generate-rust` from `package.xml`. **Never hand-write.** Example `generated/` gitignored. Only `packages/interfaces/rcl-interfaces/generated/<edition>/` in git (`nros-` prefix, e.g. `humble/nros-builtin-interfaces`). The bundled base interfaces (`std_msgs`, `builtin_interfaces`, …) ship inside the in-tree `nros` CLI under `packages/cli/` (Phase 218; the historical `packages/codegen/interfaces/` retired with the codegen submodule in Phase 195.D). `nros-core` re-exports `heapless`.
