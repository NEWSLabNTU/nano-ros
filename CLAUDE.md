...
| 102 | RMW API alignment — named `nros_rmw_ret_t` constants (`NROS_RMW_RET_OK / TIMEOUT / UNSUPPORTED / INCOMPATIBLE_QOS / …`) in place of bare `int32_t`, plus visible entity structs (`nros_rmw_publisher_t` / `_subscriber_t` / `_service_*_t` / `_session_t`) carrying `topic_name`, `qos`, `loan_caps` + an opaque `void *backend_data` slot. Drops the `rmw_set_error_string` thread-local. Bumps the C vtable major version — no backward-compat shim. | Not Started |
| 103 | RMW typed-loan path — adds `loan_publish_typed` / `commit_publish_typed` / `loan_recv_typed` / `release_recv_typed` vtable entries alongside the existing CDR-byte loan (Phase 99). Backends with intra-process / shared-memory transport (uORB first; future iceoryx, zenoh-pico SHM link) hand the publisher a typed-buffer slot — eliminating the CDR encode step for true zero-copy. Codegen emits `<MsgType>_IS_LOANABLE` (true for fixed-shape POD); runtime gates path on it. Buffers stay publisher-embedded — no backend allocator dependency. | Not Started |
