...
title: "Readiness greps use string literals, so a wrong marker burns the whole
  timeout in silence — 12 call sites to audit"
status: open
type: tech-debt
area: testing
...
1. Audit the 12, measuring each rather than pattern-matching.
2. Replace every literal with the `output::*` constant for the binary being
   waited on — per LANGUAGE where the demos differ, as
   `native_example_pubsub_e2e.rs::listener_ready_marker` now does.
3. Consider a gate: `wait_for_output_pattern` callers in `tests/` should take a
   constant, not a literal. Mechanically checkable, and it is the only thing that
   stops this returning — the same argument as `check-string-conventions`.
4. Issue 0471 is the deeper half. While a timeout can return `Ok`, no amount of
   marker correctness makes these waits self-reporting.
