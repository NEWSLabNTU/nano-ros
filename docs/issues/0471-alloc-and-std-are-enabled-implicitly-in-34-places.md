...
type: bug
area: build
related: [issue-0467, issue-0468, issue-0464, rfc-0033, rfc-0034, phase-341]
---
...
`nros-bridge` all had `std = ["alloc", …]`. Removed in phase-341 W2 (revised).
...
Issue 0467 is fixed on the SOURCE side instead: `nros-core`'s `heap::{Vec,
...
phase-341 W3's `default = []`, a plain host build of `nros-c` has neither `std`
...
phase-341 W4's `check-feature-contract` gains: **no feature body may list
