...
## Folded in: the VALUE claim (was issue 1400, 2026-09-21)

Issue 1400 was filed separately and folded here the same day, because it is about
this surface and the maintainer's test for folding is the surface. It is not a
duplicate: everything above asks whether `lending` is EXERCISED, and 1400 asks
whether, where it IS exercised, the thing it does is worth doing. Recommendation
(C) fixed the honesty defects one at a time — the `Ok(None)` transient,
`can_loan_messages` derived from the vtable, the header's availability note, the
no-std compile ratchet — and none of them touched the value claim. Its content is
carried here rather than summarised; `archived/1400-*.md` is the stub.
...
### One backend of four fills the vtable slot

`borrow_loaned_message` is declared once
(`packages/core/nros-rmw-abi/include/nros/rmw_vtable.h:564`) and filled once:
...
NULLs the loan trio and fills `publish_streamed` instead, for the reason §2
measured (`uxr_prepare_output_stream` has no cancel and is not contiguous past
one history block).
...
that buffer exactly as it would have encoded into its own, and `commit_slot`
(`packages/rmw/cffi/src/lib.rs:3602-3620`) ends at
...
**uORB is the fourth backend, and it is the evidence the study above did not
weigh.** It existed when the study was written (the backend moved to
`packages/rmw/uorb` on 2026-07-31) and §5 names it once, in passing, as one of
the three backends the fallback path serves; the per-backend study read
zenoh-pico, Cyclone and XRCE and not uORB. Yet uORB is the one whose NATIVE shape
most looks like it should be the loan's best case, and it is the shape UPSTREAM's
typed loan has: `publisher_publish_raw`
...
(`packages/rmw/uorb/nros-rmw-uorb/src/uorb_abi.hpp:42`). There is no CDR stage
there at all — the bytes ARE the struct, which is exactly the "give me a `T`"
that §1 records as upstream's loan and §4 records as Cyclone's `fixed_size` gate.
The backend that could serve a typed loan natively is the one that leaves the
slot NULL, and takes a `Box`ed staging buffer instead.

### It is `alloc`-only, so it refuses permanently on the targets it was written for

The `#[cfg(not(feature = "alloc"))]` arm of the same function
(`packages/rmw/cffi/src/lib.rs:3522-3547`) returns `Err(TransportError::Unsupported)`,
surfacing as `NROS_RET_NOT_ALLOWED` — correctly PERMANENT since this issue's fix,
and correctly documented as such in the shipped header. But "correct refusal" is
the whole behaviour on a freestanding image with Cyclone, XRCE or uORB, which is
the RAM-tight, copy-count-sensitive deployment the feature's stated benefit is
about.

### Zero callers outside its own tests

A tree-wide grep for `nros_publisher_loan`, `nros_subscription_borrow`,
`try_loan`, `try_lend_slot`, `loan_with_timeout` and `.loan(` finds call sites
only in `packages/testing/nros-tests/tests/loan_e2e.rs`,
`packages/rmw/cffi/tests/{loan_native,loan_fallback,loan_no_alloc}.rs` and
`packages/api/nros-cpp/tests/compile/publisher_publish_guards_initialized.cpp:139`.
**No example, no template, no book chapter, no in-tree application calls it** —
the same measurement recorded on 2026-09-03, still true 18 days on. (`examples/`
and `book/` return zero hits for any of those spellings.)

### The fourth backend does not rescue the claim either

`zenoh_pub_loan` (`packages/rmw/zenoh/nros-rmw-zenoh/src/lib.rs:348-378`) lends
from our own static `LendArena` and aliases it into the put; §2 above measured
...
1. **Fill the slots.** Cyclone 0.10.5's `dds_loan_sample` is typed and needs
   `DDS_HAS_SHM` + a live iceoryx endpoint (§2); XRCE's
...
2. **Refuse instead of staging.** Make `try_lend_slot` answer
   ...
   API stops claiming something it does not do, and `can_loan_messages` (already
   derived from the slot, `packages/rmw/cffi/src/lib.rs:2554`) becomes the
   complete answer.
3. **Withdraw the surface.** Retire the publish half of `lending` and name
   ...
   §"On the proposed test lane" already suggests as the follow-up if a copy-count
   measurement shows no difference. Cost: an ABI surface goes, and issue 0781's
   reason for keeping `take_loaned_message` (the only shape that can hand a
   caller a view outliving the call) applies to the RECEIVE half and would have
   to be preserved separately.

A decision between them needs the copy-count measurement §"On the proposed test
lane" asks for — memcpy'd bytes on the publish path, `publish_streamed` versus
`publish_raw` versus `try_loan`, on the cells that already exist — not a lane
that proves delivery. Note the relation to the residue judgement above: that
judgement rules the `ArenaStaging` allocation not worth closing IN PLACE, which
is a decision about the ALLOCATION. It does not decide whether a path that only
adds cost should keep advertising itself as the zero-copy one, and options 2 and
3 are both ways of answering that without paying for a static arena one layer
down.

### Acceptance for this half

Whichever option is taken, the same two things must hold afterwards: no shipped
doc comment or header claims a saving a measurement does not show, and the answer
a caller gets on a NULL-slot backend is the same in an `alloc` build as in a
heap-free one, because the underlying fact (this vtable has no loan slot) is the
same in both.

### Related decision

RFC-0089's loan-family section records the classification this measurement
produced: upstream's typed loan is `absent`, our byte loan is an `extension`, and
the two cannot be unified because they are zero-copy of different things. That
section rules on the NAMES; this half is about whether the thing behind our name
earns its documentation.

### What was re-measured at the fold (2026-09-21, `origin/main` at 3da6315e2)

Every citation above was re-read before folding; all of them still resolve at the
lines given, including the four vtable rows, the two `try_lend_slot` arms, the
`publish_raw` tail of `commit_slot`, uORB's `o_size` check and the `can_loan_messages`
derivation. One thing had MOVED, and it is recorded rather than smoothed over:
the caller set is larger than issue 1400 listed. Besides `loan_e2e.rs`,
`loan_native.rs` and `loan_no_alloc.rs` it now also contains
`packages/rmw/cffi/tests/loan_fallback.rs` (which is the fallback path's own
test, and so is exactly the path this section is about) and one C++ compile-guard
test, `publisher_publish_guards_initialized.cpp:139`, asserting that
`pub.loan(8)` fails on an uninitialized publisher. Both are tests, so the claim
that no example, template, book chapter or application calls the surface is
unchanged — but "only two test files" was already not the number, and a reader
sweeping for callers should grep `try_lend_slot` and `.loan(` as well as
`try_loan`, which is how the two extra files surface.
