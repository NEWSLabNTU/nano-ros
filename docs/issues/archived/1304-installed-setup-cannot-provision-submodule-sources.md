...
-> `nros new my_robot --workspace` -> `nros sync` -> `nros build` -> the entry
prints `Published:` and `Received:`, with no checkout on the host. That probe is
the gate.

The flow it asserts is first-project.md's own, and that page was rewritten to
`nros sync` + `nros build` by phase-445 W5 — which also dropped the `probe=`
