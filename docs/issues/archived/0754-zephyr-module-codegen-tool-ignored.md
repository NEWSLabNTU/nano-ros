...
Implemented as the class, at all three second-discovery sites:

* `zephyr/cmake/nros_rmw_cyclonedds.cmake` — the idlc store rung now
  consults `_NROS_ZEPHYR_CODEGEN_TOOL` → `_NANO_ROS_CODEGEN_TOOL` →
  `CONFIG_NROS_CODEGEN_TOOL` before any `find_program` (rungs inlined
  because this runs before the interface generator is included and the
  shared resolver FATALs where this rung must degrade).
* `cmake/NanoRosCorrosion.cmake` (`_nros_corrosion_store_dir`) and
  `cmake/toolchain/riscv64-threadx.cmake` — same handed-in-CLI-first
  rule; both had the identical independent `find_program(nros)`.

Exercised by a full `build-test-fixtures lane=tier2` (every zephyr +
riscv64-threadx configure ran through the edited paths). The ASI
consumer's `export PATH` workaround is now unnecessary — its removal
on their side is the field confirmation.
