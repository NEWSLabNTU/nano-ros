...
**#356** — Grandfathered committed SystemModels. Surfaced as c/mixed embedded entries codegen-failing
*"places no nodes on board `mps2-an385-freertos`"* (breaking `just {freertos,nuttx,threadx} build-fixtures`),
but the real cause is broader: the CURRENT `nros-launch-resolve` requires explicit `nodes=[..]` on every
deploy block once there is more than one, and NO committed embedded workspace declares them. The portable
committed models (#320) are grandfathered — generated by an older resolver — so ANY re-resolve fails
"node not placed" (proven: forcing the unmodified `examples/workspaces/rust` to re-resolve hits the same
error). Adding the missing c/mixed embedded deploy targets just forces the first such re-resolve. Fix =
resolver change to place `native`(default)+`robot*`(multihost)+`embedded` without explicit `nodes=` (unfreezes
regeneration everywhere), OR author explicit `nodes=` across all embedded workspaces. Vendored-fork +
embedded-build verification → larger than a config tweak. See `0356-*`. (2026-07-31)
See `0354-*`. (2026-07-31)
