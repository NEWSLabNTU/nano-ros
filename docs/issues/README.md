...
**#0792** (api/lifecycle, open 2026-08-25) — twelve C lifecycle entry points cannot be called from C: cbindgen emits `Option_LifecycleCallbackFnCtx` as an OPAQUE forward declaration and passes it by value (`error: variable has incomplete type`), while our own C++ shim spells the same thing correctly as a plain fn-ptr. Plus `nros_lifecycle_init` NULL-checks the node it is given and never stores it, so `nros_make_node_a_lifecycle_node` does not. See `0792-*`.
...
**#0788** (api, open 2026-08-25) — the same API verb is spelled differently in our C, C++ and Rust, and twice ONE language ships both: `nros_service_send_response` sits beside `nros_service_send_reply_raw`, and `ClientTrait` declares `is_server_ready` and `server_available` in the same trait. Six verbs across four phase-379 stages. Scopes W5's rename sweep. See `0788-*`.

**#0789** (api, open 2026-08-25) — `nros-cpp` ships no clock, time or duration surface at all (`timer.hpp` and no sibling) while C has 27 `nros_clock_*`/`nros_time_*` symbols and Rust has the full set, so a ported rclcpp node cannot stamp a message header; and ROS time runs the other way — Rust can be driven by a simulator's `/clock`, C cannot. Found by phase-379's `timer` stage. See `0789-*`.

**#0790** (api/core, open 2026-08-25) — no shutdown hook in any language: no `on_shutdown`, no `add_pre_shutdown_callback`, no phase that runs while entities still work. A node that must park an actuator or release a bus on the way down has nowhere to do it, and on a device there is no OS to reclaim what it holds. Found by phase-379's `init` stage. See `0790-*`.
