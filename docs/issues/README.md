...
**#0789** (api, open 2026-08-25) — nros-cpp had no clock/time/duration surface; the C++ half is FIXED 2026-08-25 (`duration.hpp`/`time.hpp`/`clock.hpp`, `Node::now()`, 19 ledger gaps closed). The other half is open: ROS time exists in Rust and not in C, so a C image cannot be driven by a simulator's `/clock`. See `0789-*`.
...
**#0799** (api/core, open 2026-08-25) — `nros_time_from_nanoseconds` truncates toward zero AND takes `unsigned_abs` of the remainder, so −0.5 s encodes as `{sec:0, nanosec:5e8}` and round-trips as **+0.5 s**; ROS 2's own `Time.msg` specifies floor division with a non-negative remainder. `nros_time_add`/`_sub` route through it, so any arithmetic crossing zero is silently wrong, and a `nros_time_t` in a header is a `builtin_interfaces/Time` on the wire. Found while building the C++ clock surface on top of it. See `0799-*`.
