...
Recently resolved (2026-09-04): **#1025** (build, testing) — no ESP32 flash image could be packed at all, for four days, with every gate green. `nros_fixture_row_artifact_dir` made the group FORMULA single in phase-340 item 7 and left its INPUTS derived twice: the packer spelled `… qemu-esp32-baremetal "" ""` while the producer passed the ROW's cargo args and env, and the key is a function of all three. The two agreed until `41a7d8de7` added `env = { ZPICO_MAX_QUERYABLES = "2" }` to the three esp32 rows, after which the build wrote `cargo-fixtures/qemu-esp32-baremetal-4118800323/` and the packer kept asking for `cargo-fixtures/qemu-esp32-baremetal/`. Fixed by `nros_fixture_row_artifact_dir_by_id`, which reads all three fields from the manifest row the build was handed. Acceptance is a BUILD, as the issue demanded: rc=1 with the ELF one directory over, then rc=0 with three 4 MiB `.bin`s, on the same tree. `check-fixture-artifact-dir-keys` is additional, mutation-checked against the real defect and against a bogus literal. The test-side locators turned out to need NO change and that was measured, not read — `require_prebuilt_binary` redirects through `groups::resolved`, and all three authored leaf paths resolve onto the group dir. Bearing on #0968 is NOT established: its five esp32 tests skip on a host without the Espressif QEMU fork. See `archived/1025-*`.
