...
the merge queue; `open.md` can carry `merge=union` precisely because it is
generated end to end, and this file cannot. The copy that used to sit here was
...
Recently resolved (2026-09-02): **#0982** (codegen, ci) — two `rosidl-codegen` tests were RED ON MAIN and invisible: `BoundState::classify` gained a transport-framing allowance on the RX bound (`next_multiple_of(4)`, so a 133-byte type states 136), the C emitter took it, and the C++ GOLDEN plus `message_size_bound_parity`'s hand-recomputed `max(x1,x2)` did not — issue 0896's own one-rule-two-copies defect, once along the LANGUAGE axis and once along the TEST axis. They hid because `cli-tests` is registered in the `build` list and `just check` runs `check::fast` first, stopping at the first failure: any one red fast gate silently withdraws every lane behind it. That is 0952's mechanism, recorded there for backend BUILDS and here for test LANES. Golden regenerated (RX only, 3 files, each value now equal to the C header's — the evidence the generator was right); the parity test now calls `bounds::transport_framed` rather than keeping a third copy of the rule. The ORDERING is deliberately not fixed — a withdrawn lane should report SKIPPED rather than nothing, same principle as 0975. See `archived/0982-*`.
