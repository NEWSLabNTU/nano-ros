...
**#0844** (testing, open 2026-08-27) — `check-no-tracked-file-find` detects `find` and Python `os.walk` but not `grep -r`, so `core_only_predicate.sh` walked 9.2 GB of gitignored Zephyr SDK under `scripts/` and stalled the fixture build for 37 minutes; `git grep` answers the same question in 0.33 s. See `0844-*`.
