...
- **#1000** ([rmw, third-party]) — Vendored Cyclone: a fired event is off the heap until its handler re-arms it, and `handle_xevk_spdp` has return paths that do neither — a latent leak, NOT the cause of #0997 See `1000-*`.
