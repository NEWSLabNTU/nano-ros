...
Root cause was NOT the platform layer: nros-rmw-cffi's
`static_subscriber_storage` (no-std subscription-handle pool behind the
vtable) was hardcoded to 4 slots; the 5th `create_subscription` returned
BAD_ALLOC and the executor's `map_err(|_| SubscriberCreationFailed)`
masked it. Fixed in the commit below this doc: pool sized via
`NROS_RMW_SUBSCRIBER_SLOTS` (default 8), executor preserves transport
errors, zenoh-pico `_z_send_tcp` gains a write_all drain loop, board
TCP_SND_BUF 4→8 MSS. comp-all verified boot+spin on MPS2 QEMU.

## Ask (addressed by the error-preservation change)
