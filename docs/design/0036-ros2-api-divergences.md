...
| rclcpp returns `T` **and** throws — e.g. `ClientBase::service_is_ready() -> bool`, `Subscription::take() -> bool` | C++ returns **`nros::Expected<T>`**; Rust returns `Result<T, E>`; C keeps rcl's literal shape, `nros_ret_t f(args…, T *out)` | The row above covers the `void`-returning case. Where rclcpp returns a VALUE and throws, collapsing to the bare value loses the error channel entirely — we have no exceptions to move it to. **Upstream `rcl` already separates the two** and says so: `rcl_service_server_is_available` returns `RCL_RET_OK` "if the check was made successfully (**regardless of the service readiness**)" (`rcl/graph.h:829`). So this is not a divergence from rcl — it is a divergence from rclcpp's *exception-dependent* collapse, and it restores rcl's shape. `Expected<T>` (`result.hpp:198`) is ungated, stores inline and allocates nothing. Phase-379 W6 | RFC-0018, phase-379 W6 |
