...
| 0047 | [unified-sched-context-binding](0047-unified-sched-context-binding.md) | Draft | one config-seeded `node_name → sched_context` table looked up at the single `Executor::node_builder(name)` site all languages funnel through (RFC-0046), replacing the four per-shape tier-binding paths (Rust `run_tiers`/codegen bind, C/C++ `NodeBuilder::sched`, rclcpp = none). Config-driven (`[tiers]`+`callback_groups`), same seed-by-name pattern as params/identity; dissolves #121 (rclcpp not sched-bound) with no `NodeHandle` change. Binding only — `run_tiers`/execution (RFC-0016) untouched. Tracked by phase-272 |
