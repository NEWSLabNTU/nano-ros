...
| 0062 | [unified-dependency-ssot](0062-unified-dependency-ssot.md) | Draft | `nros-sdk-index.toml` becomes the SSoT for **every** dependency class — existing `[tool.*]`/`[source.*]`/`[gated.*]` plus new `[system.*]` (OS packages by ABSTRACT key, mapped per package manager apt/dnf/pacman/brew, `check` probe per entry), `[rust.*]` (toolchains/targets/cargo tools) and `[python.*]`; a `[tool.*]` may declare `system = [..]` runtime deps of its dist (the qemu/libslirp hole). Setup AND doctor both derive from the index, so doctor remedies are computed, never hand-written (deletes the remedy-drift class: three remedies pointed at apt where an index prebuilt existed). Sudo boundary: unprivileged classes install first; the `[system.*]` closure is composed into ONE native command per manager and PRINTED (opt-in `--sudo` to run) — a missing OS package degrades, never aborts the sudo-less remainder (0368-F1). rosdep is an OPTIONAL resolver backend for unmapped managers (measured: 12/24 public-db coverage, sudo-less via `ROSDEP_SOURCE_PATH`, in-repo overlay covers the rest), not the SSoT. Motivated by issue 0368's clean-host walk |
