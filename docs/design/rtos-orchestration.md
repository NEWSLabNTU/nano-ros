...
| Board crate `run(config, closure)` pattern (`nros-mps2-an385-freertos`, etc.) | User passes closure; board does HW init + scheduler start.                                                                                           | **Reuse** — codegen-emitted `main` calls board's `run()` w/ generated closure (which spawns tier tasks).                                                                                 |
