...
implements-tracked-by: [phase-327, phase-398, phase-404, phase-447]
supersedes: []
---

# RFC-0062 — One dependency SSoT, system-aware

**Status:** Stable (2026-08-30; Draft 2026-08-01, amended 2026-08-29, 2026-09-11)
...
(QUALIFIED by amendment 5 below: the rosdep TOOL is still never consulted, and
this section's three objections all stand — but a PINNED SNAPSHOT of rosdep's
database, vendored as index data and marked unprobed, is a fallback rung. Read
amendment 5 before acting on the paragraph that follows.)
...
## Amendment 5 (2026-09-11) — a PINNED rosdep snapshot, vendored as index data

Amendment 1 above says "rosdep is NOT consulted", and that sentence stands for
the rosdep TOOL. This amendment qualifies it, because the amendment's own three
objections are all objections to a **host-installed runtime resolver**, and one
of the three is about the DATABASE rather than the resolver — a gap the
rejection left open and did not name.

Landed by phase-447 D3; the decision is RFC-0099 D8's.

### The gap the rejection left

A user writing `<depend>libopencv-dev</depend>` in their own package hits the
ladder's last rung — UNKNOWN, an error — where a ROS user gets it resolved from
rosdep's community keys. That is a **data** gap, not a format one, and nothing
in amendment 1 addresses it: `[prereq.*]` is the right shape and simply does not
know the name.

Measured before deciding, and the numbers are the argument for a fallback rather
than an adoption:

* of the 46 `[prereq.*]` keys, only **4** are the per-package kind rosdep is
  designed for (infra 19, workspace 11, buildtool 7, vendor 5, package 4);
* the upstream database is **2,420 keys**, not the "~10k" this tree has said
  twice — `base.yaml` 1300, `python.yaml` 1092, `ruby.yaml` 17,
  `osx-homebrew.yaml` 211, merged.

So rosdep answers a question this index mostly does not ask, and asks one it
mostly cannot answer. A fallback rung is the only shape both facts allow.

### DECIDED — vendor a projection, not the tool and not the YAML

`nros-rosdep-snapshot.toml` at the repo root, beside `nros-sdk-index.toml`:
generated by `scripts/gen-rosdep-snapshot.py` from a pinned ros/rosdistro
commit, committed, and read by the same TOML reader as the index.

Every objection amendment 1 raised, answered or kept:

| objection (amendment 1) | what a pinned snapshot does |
| --- | --- |
| answers for one provider of four | kept, and structural — the entry type has only manager lists, so it is a FALLBACK rung below `[prereq.*]` and can never claim an SDK dist, a submodule or a source build |
| cannot carry a `check` | **kept, and made visible** — see below |
| present on one host, absent on another | cannot happen: it is a tracked file, and nothing in the reader touches the network, PATH, or anything outside the checkout |

**SCOPE — a projection onto four managers, not the upstream file.** rosdep's OS
vocabulary is ~16 distributions; this tree installs through `apt`, `dnf`,
`pacman`, `brew`. `apt <- ubuntu else debian`, `dnf <- fedora else rhel`,
`pacman <- arch`, `brew <- osx`'s homebrew installer. 1,788 of the 2,420 keys
project onto at least one; the rest map only to distributions we could not act
on. Vendoring those columns would be vendoring data no code here can read, and
it would put a YAML parser in a tool that reads manifest/index files only
(RFC-0099 D10). Measured: **150 KB projected against 505 KB of upstream YAML**.

A key projecting onto NO manager is **dropped, not emitted empty** — the same
rule `[system.*]` already carries ("maps to no package manager at all" is a hard
error), because a row that resolves a `<depend>` and then installs nothing is a
silent success.

An OS whose value is version-keyed (`ubuntu: {jammy: [..], noble: [..]}`)
contributes its `'*'` entry and is otherwise dropped. The OS-VERSION dimension
is RFC-0099 D9 and is not built here; picking one version's answer for all
versions would put a claim in the file that the schema cannot qualify.

**PINNING — the file states its own provenance.** `[snapshot]` carries
`upstream`, a 40-hex `ref`, the generator's path, and every input file with its
sha256. `RosdepSnapshot::validate` refuses an incomplete pin at LOAD, so a blob
with no provenance cannot be read at all rather than being noticed at review
time.

**REVIEW — the format IS the mechanism.** The output is sorted by key, one table
per key, at most four lines each, managers in fixed order. A bump's diff is
therefore one line per changed package list plus the provenance lines, so "what
did upstream change?" is answerable by reading the patch. `--verify` re-derives
at the pinned ref and requires byte equality; it needs the network, so it is a
maintainer action rather than a gate. The offline gate
(`check-rosdep-snapshot`) asks the rest: pin complete, keys sorted and unique,
no entry empty, no entry carrying a field the projection does not produce.

### DECIDED — unprobed, and SAID so at every consumer

This is the objection that survives intact, and it is the one that needs
enforcement rather than a paragraph. Amendment 1: *"the probe is what turns
'missing package' into a named remedy … a resolver that supplies packages but
not probes leaves the diagnosable part to us regardless."*

* `RosdepEntry` has four fields and no fifth. `deny_unknown_fields` REFUSES a
  `check` or a `role`, so a hand-written probe cannot be added to a generated
  file and then silently deleted by the next bump.
* `Resolution::RosdepSnapshot` is a distinct rung, and
  `Resolution::is_unprobed()` is the one predicate for "presence was never
  tested".
* `RosdepSnapshot::PROVENANCE_NOTE` is the one spelling of the sentence, and
  `check-rosdep-snapshot` requires every file that reaches the rung to name that
  constant. A fallback that resolves a name without saying it is unprobed would
  re-create the rejected behaviour quietly, and that is exactly the half a
  reader cannot check by eye.

`nros setup --workspace` prints these under their OWN heading rather than in
the `[prereq.*]` table: those rows print `present`/`MISSING` from a probe, these
can print only what a manager would install, and one table would make "unprobed"
read as "the probe returned unknown".

### DECIDED — last rung, and consulted lazily

Order: workspace package > generated message > `[prereq.*]` > ambient ROS >
self-buildtool > **snapshot** > UNKNOWN. Below `SelfBuildtool` as well as below
`[prereq.*]`: the snapshot has an apt name for `cmake` and for `ament_cmake`,
and answering with it would replace a truthful "the builder supplies this" with
an unprobed guess.

That ordering is load-bearing rather than tidy. **15 of the 46 `[prereq.*]` keys
are also snapshot keys** (`cmake`, `curl`, `clang`, `openocd`, `doxygen`,
`socat`, `zstd`, `unzip`, `graphviz`, `cargo`, `libclang-dev`,
`gcc-arm-none-eabi`, `python3-{dev,pip,venv}`). Those rows carry a `check`; the
snapshot's do not. Had the snapshot won, adding a fallback database would have
silently removed the probes from a third of the authored index.

`nros build` loads the file only when some name is otherwise UNKNOWN — which is
the structural statement of what a fallback is, since a rung that is never asked
cannot change an answer a higher rung gave. `nros setup --workspace` loads it
unconditionally, because reporting what a workspace needs is that verb's job.

### What this amendment does NOT decide

* **rosdep the tool, and rosdep's YAML syntax** — still rejected, unchanged.
  Nothing shells out to `rosdep`, and no `[prereq.*]` is written in its format.
* **Installing from the snapshot.** It resolves names and supplies per-manager
  package lists for reporting. Whether `nros setup` may install from an unprobed
  row — and therefore be unable to tell the user afterwards whether it worked —
  is a separate decision that wants the probe question answered first.
* **The bump cadence.** The pin moves when someone moves it, with a diff. There
  is no schedule and no auto-bump; an unattended bump of a supply-chain input is
  the thing the pin exists to prevent.
* **OS-version granularity** — RFC-0099 D9. Until it exists, a version-keyed
  upstream entry with no `'*'` contributes nothing for that manager.

*Acceptance:* `<depend>libopencv-dev</depend>` in a user's own package resolves;
two hosts with different distributions and no rosdep installed resolve the same
tree the same way; a key from the snapshot is reported under its own heading and
marked unprobed, never mixed with a probed `[prereq.*]` row.
