...
`GenericTimer`, and no hierarchy. (This paragraph also said "no `TimerBase`" and
required a ported `rclcpp::TimerBase::SharedPtr timer_;` to be RENAMED to
`rclcpp::Timer timer_;`. **That half is overturned 2026-09-09** — the rename is
mechanical here and impossible upstream, so it is exactly the edit that empties
the two-library intersection. `TimerBase` returns as a hosted alias onto the flat
`Timer`; see the amendment below.)
