...
| **UX-2** | **Fix self-contained example rule.** Today `examples/mps2-an385-freertos/c/zenoh/talker/CMakeLists.txt:5` does `include(.../../../cmake/freertos-support.cmake)` — hard escape. Move shared cmake into `find_package(NanoRosFreeRTOS)` (already partially there in `cmake/`). Examples become true copy-out templates. | S–M | Low | A.P3 |
