...
- [ ] W2.2 `check-fixtures-stale.sh`: `set -euo pipefail`; probe stderr
...
- [ ] W3.1 `scripts/build/prune-dirs.sh`: ONE canonical definition
...
- [ ] W3.2 Convert `justfile:2163` clean-bindings + `check-example-matrix.sh`
...
- [ ] W4.1 `scripts/test/toolchain-gate.sh` sourced by both
