...
unsupported until **PR #557** (`ci/land-node-std-gate`) merged — that PR
is this finding's other half, and it landed the step whose absence is the
subject here. Cited by PR NUMBER and not by commit: the short hash this
line first carried stopped naming anything the moment that branch was
rebased, which is the same "believed because it was AUTHORED rather than
OBSERVED" shape the paragraph above is about.
