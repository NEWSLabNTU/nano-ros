...
- [x] **111.A.1** — Create `nros-cli-core/` library crate with subcommand traits + dispatch.
- [x] **111.A.2** — Create `nros-cli/` binary crate with `[[bin]] name = "nros"`. clap-based parser.
- [~] **111.A.3** — Reshape `cargo-nano-ros/` to be a thin shim calling into `nros-cli-core`.
      Partial: scaffolder lifted to `cargo_nano_ros::scaffold` lib module so
      both front-ends share it. Full bin-side flattening blocked until
      `cargo-nano-ros` splits into separate lib + bin crates (current
      single-crate layout would create a `cargo-nano-ros → nros-cli-core →
      cargo-nano-ros` Cargo cycle). Tracked as a follow-up.
- [x] **111.A.4** — Implement `nros new` scaffolder. v1 reuses the lifted lib
      scaffolder; per-use-case template trees deferred to the Phase 112
      example sweep.
- [x] **111.A.5** — Implement `nros generate <lang>`. Wraps existing
      `cargo_nano_ros::GenerateConfig` for parity. C++ standalone path errors
      with a pointer to the CMake `nano_ros_generate_interfaces(... LANGUAGE
      CPP)` integration; standalone C++ codegen requires a missing
      `generate_cpp_from_package_xml` lib entry point.
- [x] **111.A.6** — Implement `nros config show` / `nros config check`.
      v1 parses `config.toml`; Kconfig (Zephyr) integration + the
      auto-generated `nros_app_config.h` struct land with Phase 112.D.
- [x] **111.A.7** — Implement `nros doctor`. v1 shells out to `just doctor`
      (or `just <platform> doctor` with `--platform`) from the auto-detected
      workspace root; the justfile already orchestrates per-module doctor
      recipes and is the source of truth.
- [x] **111.A.8** — Implement `nros board list`. Reads `packages/boards/<crate>/Cargo.toml`
      and emits `name | description`. Surfaces 9 board crates today;
      structured chip/flash/ram fields require UX-42 (out of scope per phase
      doc).
- [x] **111.A.9** — Implement `nros build`. Detects project flavor (cargo /
      cmake / west) from file presence and delegates verbatim.
- [x] **111.A.10** — Implement `nros run`. v1 supports POSIX (`cargo run`)
      and ESP32 (`espflash flash --monitor`). QEMU multi-target +
      OpenOCD ARM boards land in v2.
- [x] **111.A.11** — Implement `nros monitor`. v1 surfaces `espflash monitor`;
      `defmt-print` ARM RTT decoder + `addr2line` semihosting decoder
      land alongside Phase 88 (`nros-log`).
- [x] **111.A.12** — Top-level `nros --help` curated via clap-derived
      `Cmd` enum; release subcommands hidden unless built with
      `--features release`.
- [x] **111.A.13** — Shell completions (`nros completions bash|zsh|fish|powershell`)
...
A — `nros` CLI:
- [~] `nros new --platform freertos --rmw zenoh --lang c talker` produces a
      directory that builds with `nros build` and runs with `nros run` on a
      fresh clone, on both Linux and macOS hosts.
      *Status:* scaffold emits a colcon-compatible package tree; `nros build`
      delegation works. End-to-end build of the freertos / c / zenoh tuple
      validated only as a v1 — full per-use-case template (talker /
      listener / service / action) lands with the Phase 112 example sweep.
- [x] `nros doctor` exits 0 on a healthy `just setup`'d workspace; exits
      non-zero with a fixit hint when a SDK path is missing.
      *Status:* delegates to `just doctor`, which already orchestrates every
      per-module doctor and surfaces fixit hints inline.
- [x] `nros generate c` regenerates the same C bindings as `cargo nano-ros
      generate-c` byte-for-byte.
      *Status:* both front-ends route through
      `cargo_nano_ros::generate_c_from_package_xml`.
- [~] `nros board list` enumerates ≥ 9 boards with chip/flash/ram populated.
      *Status:* lists 9 boards today (`name | description`). Structured
      chip / flash / ram / supported_rmw columns blocked on UX-42 (board
      descriptor TOML) — out of phase 111 scope per phase doc.
- [x] The `cargo nano-ros` legacy entry point still works and goes through the
      same code paths.
