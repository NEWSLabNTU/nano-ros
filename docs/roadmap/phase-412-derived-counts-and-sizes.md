...
| `NROS_ZEPHYR_HEAP_SIZE` | 94208 | runtime allocation. No static model, and the honest first step is a high-water reporter, not a derivation. **Note the "does not fault or log — it stops" line in its Kconfig help is now STALE**: `nros_platform_alloc` prints `nros: HEAP EXHAUSTED: request N, arena M, caller %p` since issue 0968. The return is still NULL and the caller still does whatever it does, so the process does not fault — but it is no longer mute, and issue 1010 is that message in the field |
| `NROS_GRAPH_CACHE_SIZE` | 4096 | sized by the PEER graph. Not a property of this image and probably never derivable from it — it holds a liveliness keyexpr for every entity of every node on the domain, remote included. **What WAS wrong is delivery, not derivation**: it had no cargo-lane producer at all, so 65,536 bytes of `.bss` per session was unstatable on every lane but Zephyr-cmake. Fixed 2026-09-08; the knob remains operator-stated |
| `NROS_MAX_LIVELINESS` | 32 | **CORRECTED 2026-09-08: not remote peers, and it IS derivable.** Every token in the array is declared by THIS session — one per node (`ensure_node_liveliness`) plus one per publisher, subscriber, service server and client (`declare_entity_liveliness`). The graph cache's own subscriber takes no slot. So the demand is `local nodes + local entities`, which is the inventory that already derives `MAX_PUBLISHERS` / `MAX_SUBSCRIBERS` / `MAX_QUERYABLES`. `config-knob-census` classified it `derived` all along; this table and that census disagreed. What blocked it was the SECOND rule this phase set, not the first: a derived count is safe only where exhaustion NAMES the knob, and this one was silent from the C arm up. That is fixed, so the derivation is now ordinary work |
...
| [#1130](../issues/archived/1130-derive-cell-entity-bounds-from-the-declaration.md) | **RESOLVED 2026-09-11.** a component's cell registries are sized by a const the author must write. The inventory already knows the figure: `max_cell_entities` goes in `DerivedEntityKnobs`, composed across entries by MAX, abstaining when any entry refuses. An explicit `ENTITY_BOUNDS` keeps winning, being per-class |
