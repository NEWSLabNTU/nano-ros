...
- [x] **112.D.2** Zephyr Kconfig variant — `<nros/app_config.h>` ships at `<prefix>/include/nros/app_config.h`. Under `__ZEPHYR__`, it synthesizes `NROS_APP_CONFIG` from `CONFIG_NROS_*` macros (zenoh locator + domain id; network/scheduling sourced from DTS/Kconfig elsewhere). Non-Zephyr fallback `#error`s if no codegen-generated copy shadows it via the build dir include path.
- [x] **112.D.3** Drop `target_compile_definitions(... APP_*)` blocks. FreeRTOS / ThreadX-Linux / ThreadX-RISCV64 `startup.c` rewritten to read `NROS_APP_CONFIG.network.{ip,mac,gateway,netmask}` and `NROS_APP_CONFIG.scheduling.{app_priority,app_stack_bytes,zenoh_*,poll_*}`. Example sources sweep `APP_ZENOH_LOCATOR` → `NROS_APP_CONFIG.zenoh.locator` and `APP_DOMAIN_ID` → `.zenoh.domain_id`. CMakeLists drop the `target_compile_definitions(... APP_*)` blocks (and NuttX `COMPILE_DEFS APP_*` keyword args). Migration scripts: `tmp/drop-app-defs.py`, `tmp/drop-nuttx-defs.py`, `tmp/migrate-d3.py`, `tmp/add-build-include.py`.
- [x] **112.E.1** Layer-3 `<plat>-support.cmake` shipped via `find_package(NanoRos)` install layout at `<prefix>/lib/cmake/NanoRos/`. Platform assets installed under `<prefix>/share/nano_ros/`: `boards/mps2-an385-freertos/config/`, `drivers/lan9118-lwip/`, `platform/{freertos,threadx-linux,threadx-riscv64}/startup.c`, `platform/threadx-riscv64/cxx-compat/`, `platform/nuttx/nros-nuttx-ffi/`, `toolchains/{arm-freertos-armcm3,armv7a-nuttx-eabi,riscv64-threadx}.cmake` + `riscv64-lld-wrapper.sh`. Fixed pre-existing `nano_ros_generate_config_header` template-search path bug (`../../../share/nano_ros/templates/` from `lib/cmake/NanoRos/`). `nros_threadx_setup_picolibc` now takes optional `CXX_COMPAT_DIR` so layer-3 can point at the installed cxx-compat dir.
- [x] **112.E.2** Migrated 48 example CMakeLists.txt files (FreeRTOS, NuttX, ThreadX-Linux, ThreadX-RISCV64 × C/C++ × {talker, listener, service-{server,client}, action-{server,client}}) from `include("${CMAKE_CURRENT_SOURCE_DIR}/../../../cmake/<plat>-support.cmake")` to `include(<plat>-support)` after `find_package(NanoRos CONFIG REQUIRED)`. Migration: `tmp/migrate-include.sh`. Deleted in-tree `examples/<plat>/cmake/<plat>-support.cmake` and the deprecated `freertos-platform.cmake` shim.
- [x] **112.E.3** Acceptance test: `tests/copy-out-example-test.sh` copies an example to `/tmp`, asserts no `../../../` escapes, configures with `-DCMAKE_PREFIX_PATH=<install>` + `-DCMAKE_TOOLCHAIN_FILE=<install>/share/nano_ros/toolchains/...`, builds. Default fixture: `examples/qemu-arm-freertos/c/zenoh/talker`.
