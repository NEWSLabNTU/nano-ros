...
2. **Move FFI to `zenoh-pico-shim`** - Centralize all `extern "C"` declarations
...
│   └── README.md
│
├── zenoh-pico-shim/                 # EXISTING: Add FFI declarations
│   └── src/
│       ├── lib.rs
│       └── ffi.rs                   # NEW: All extern "C" declarations
│
├── zenoh-pico-shim-sys/             # EXISTING: C code
│   └── c/
│       └── shim/
...
| `zenoh-pico-shim` | FFI declarations, unsafe blocks, error code translation |
| `zenoh-pico-shim-sys` | C compilation, zenoh-pico internals |

## Phases

### Phase 13.1: Move FFI to zenoh-pico-shim

**Status**: Not Started

Move all `extern "C"` declarations from examples to `zenoh-pico-shim/src/ffi.rs`.

**Work Items**:

- [ ] **13.1.1** Create `zenoh-pico-shim/src/ffi.rs` module
...
- [ ] **13.1.3** Export from `zenoh-pico-shim/src/lib.rs`
  ```rust
  pub mod ffi;
...
- [ ] **13.1.4** Update `qemu-rs-common` to use new FFI module
  ...
  - Import from `zenoh-pico-shim::ffi`

**Acceptance Criteria**:
- No `extern "C"` blocks in example code
- All FFI in `zenoh-pico-shim/src/ffi.rs`
...
- [ ] **13.2.7** Add `Cargo.toml` dependencies
  ...
  zenoh-pico-shim = { path = "../zenoh-pico-shim", features = ["smoltcp"] }
...
|-------|-------------|--------|----------|
| 13.1 | Move FFI to zenoh-pico-shim | 1 day | P0 |
