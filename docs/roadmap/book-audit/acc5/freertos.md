BLOCKERS
1. `just freertos zenohd` (doc Run step 1) hardcodes `build/zenohd/zenohd`, which `nros setup qemu-arm-freertos --rmw zenoh` does not populate. Recipe exits 127 on a fresh post-`nros setup` machine. Doc presents this as the primary "Step 1" of Run and only offers the working inline `ZENOH_CONFIG_OVERRIDE='listen/endpoints=["tcp/...`"];scouting/multicast/enabled=false' ros2 run rmw_zenoh_cpp rmw_zenohd form as a commented-out alternative.
...
MISSING STEPS
- No fallback hint when `just freertos zenohd` fails (the inline `ZENOH_CONFIG_OVERRIDE='listen/endpoints=["…`"];scouting/multicast/enabled=false' ros2 run rmw_zenoh_cpp rmw_zenohd form is buried as a commented-out line).
...
Acceptance bar (0 BLOCKERS): **NOT MET** — `just freertos zenohd` recipe still needs to be fixed (or the doc needs to lead with the inline `ZENOH_CONFIG_OVERRIDE='listen/endpoints=["...`"];scouting/multicast/enabled=false' ros2 run rmw_zenoh_cpp rmw_zenohd form).
