...
**Status:** Every named work item in the doc has now landed at least at the "authored against documented kernel API" level. POSIX + NuttX C ports are runtime-tested on host (14 tests through CffiPlatform / direct extern). FreeRTOS / ThreadX timer.c are syntax-verified against in-tree kernel headers; their net.c + Zephyr / ESP-IDF C files compile only inside their respective kernel build contexts (lwIP / NetX Duo / Zephyr workspace / ESP-IDF component tree). 121.6.timer-macro (Rust adapter for the opaque TimerHandle) + per-kernel multicast (`mcast_*`) remain stubbed and open. 121.3.deprecate-rust per kernel stays gated on runtime parity tests landing on its target board.
