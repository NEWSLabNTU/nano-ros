...
**Status:** 121.1 (header + Rust mirror), 121.2.a (macros), 121.2.{posix,freertos,nuttx,threadx,zephyr} (RTOS crate wiring), 121.2.embedded (five bare-metal / FSP crates), 121.2.wire-feature (nros-platform fan-out covering all ten crates), 121.3.{posix,nuttx,freertos,threadx,zephyr,esp-idf} (six native C ports for the 39-core surface authored — POSIX/NuttX host-built + runtime-tested, FreeRTOS/ThreadX syntax-verified against in-tree kernel headers, Zephyr/ESP-IDF authored), 121.4 (drift gate + C-stub harness + POSIX parity test), and 121.6.headers (extended-surface canonical headers — `platform_timer.h` + `platform_net.h`) all landed.
...
- [ ] **121.6.rust-mirror** — Extend `nros-platform-cffi/src/lib.rs` with `unsafe extern "C" { … }` blocks declaring every new symbol, gated behind opt-in features (`extended-net-abi`, `extended-timer-abi`) so legacy consumers building only the 39-core surface aren't forced to supply the new symbols. Update the drift-gate script to check both new headers when the gate features are active.
- [ ] **121.6.macros** — Extend the `nros_platform_export_*!` macro family with `export_timer!` and `export_net!`. `export_timer!` needs a small adapter because Rust's `PlatformTimer::create_periodic` returns `Result<TimerHandle, TimerError>` while the C ABI returns `void *`; the macro hides this with a per-call `Result::ok().map(...).unwrap_or(null_mut())` translation. `export_net!` is straightforward; signatures map 1:1 once the `usize::MAX` ↔ `(size_t) -1` equivalence is documented.
- [ ] **121.6.posix-c** — Author POSIX implementations: `packages/core/nros-platform-posix-c/src/net.c` (BSD sockets — `socket`, `connect`, `bind`, `accept`, `listen`, `setsockopt(SO_RCVTIMEO)`, `recv`, `send`, `setsockopt(IP_ADD_MEMBERSHIP)` for multicast) + `src/timer.c` (`timer_create` + `SIGEV_THREAD` callback dispatch, or `timerfd_*` on Linux specifically). Wire both into `posix-c-port` feature so the Cargo integration test exercises them.
- [ ] **121.6.freertos-c** — `packages/core/nros-platform-freertos-c/src/net.c` (lwIP BSD socket API: `lwip_socket`, `lwip_recv`, etc.) + `src/timer.c` (`xTimerCreate` / `xTimerStart` / `xTimerDelete` with `xTimerPendFunctionCall` for callbacks deferred to the timer task). CMakeLists.txt grows a dependency on the parent build's lwIP target.
- [ ] **121.6.threadx-c** — `src/net.c` against NetX Duo BSD socket layer (`nx_bsd_*`) + `src/timer.c` against `tx_timer_create` / `tx_timer_change` / `tx_timer_delete`. CMakeLists.txt grows a NetX Duo target dep.
- [ ] **121.6.zephyr-c** — `src/net.c` against Zephyr's NSOS BSD socket layer + `src/timer.c` against `k_timer_init` / `k_timer_start` / `k_timer_stop`. As Zephyr-module integration.
- [ ] **121.6.nuttx-c** — `src/net.c` against NuttX's POSIX socket layer (largely a copy of the POSIX impl with NuttX-specific `recvtimeo` semantics handled). Timer reuses POSIX `timer_*` since NuttX has POSIX timers.
- [ ] **121.6.esp-idf-c** — `src/net.c` against ESP-IDF's BSD socket layer (lwIP underneath) + `src/timer.c` against `esp_timer_create` / `esp_timer_start_periodic` / `esp_timer_start_once`. Lands as a separate IDF component file.
