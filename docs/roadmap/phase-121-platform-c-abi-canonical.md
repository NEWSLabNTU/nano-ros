...
**Status:** Every named work item in the doc has now landed at least at the "authored against documented kernel API" level. **Runtime-verified on host** through CffiPlatform / direct extern: POSIX (14 cargo tests), NuttX (CMake build via POSIX-c reuse, 71 T symbols), FreeRTOS-Posix (`tests/freertos-c-smoke/`, `just freertos test-c-port`), ThreadX-linux (`tests/threadx-c-smoke/`, `just threadx_linux test-c-port`). **Build-harness ready, runtime deferred** to actual hardware/sim: Zephyr (module path wired, smoke app deferred — needs west-driven sample app at `tests/zephyr-c-smoke/`), ESP-IDF (`just esp_idf setup/build-c-port/test-c-port`, smoke app at `tests/esp-idf-c-smoke/`, gated on opt-in `just esp_idf setup` which clones ~5 GB IDF). 121.6.timer-macro (Rust adapter for the opaque TimerHandle) + per-kernel multicast (`mcast_*`) remain stubbed and open. 121.3.deprecate-rust per kernel stays gated on full runtime parity tests landing on its target board.
