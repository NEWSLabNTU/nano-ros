...
**Status:** 121.1 (header + Rust mirror), 121.2.a (macros), 121.2.{posix,freertos,nuttx,threadx,zephyr} (RTOS crate wiring), 121.2.embedded (five bare-metal / FSP crates), 121.2.wire-feature (nros-platform fan-out covering all ten crates), 121.3.{posix,nuttx,freertos,threadx,zephyr,esp-idf} (six native C ports authored, POSIX/NuttX host-built + tested, the other four scaffolded against their kernel SDKs), and 121.4 (drift gate + C-stub harness + POSIX parity test) all landed. Remaining work: 121.2.rtic (future, on-demand); 121.3.build-verification (build smoke tests for freertos / threadx / zephyr / esp-idf as their SDKs land in the workspace); 121.3.deprecate-rust (when Rust↔C-port parity is proven per platform); 121.5 (porter / internals docs); per-RTOS parity tests for freertos/nuttx/threadx/zephyr/embedded (require cross toolchains, deferred).
...
- [ ] **121.3.build-verification** — None of the four cross-target C ports above have been built against a real SDK in this branch; each ships authored against the kernel's documented API + observed behaviour of the matching Rust crate. Replace this checkbox with per-port build smoke tests as the SDKs land in the workspace.
