...
| `phase-459-W2` | `phase-459-W0` | `resolve_plan_sched` in `packages/cli/nros-cli-core/src/codegen/entry/mod.rs` (NOT `plan_from_model`, which cannot see cmake callback groups); `packages/cli/nros-cli-core/tests/derived_tiers_entry.rs` (new). `emit_cpp.rs` needed no change | `cargo test -p nros-cli-core --test derived_tiers_entry` (`codegen entry --lang cpp --board zephyr` on the W0 fixture ends in `run_tiers(..., 4u)`) | no |
...
the W0 fixture ends in `ZephyrBoard::run_tiers(..., __nros_tiers, 4u)`, one
tier per NODE (W1's correction), with the two 10 Hz nodes in the FIRST two
group tables and the two 30 Hz nodes after them.

That order reads backwards and is correct. `resolve_tiers` sorts descending
by RAW priority and deliberately does not invert (`lib.rs`: "the system owner
authors numbers correct for the target RTOS's direction; v1 does not
invert"), while Zephyr counts a LOWER number as more urgent. The island's
30 Hz nodes land on 5 and its 10 Hz nodes on 6, so descending by raw value
puts the 10 Hz pair first. Tier 0 is therefore the LEAST urgent tier on
Zephyr, not the most. An earlier draft of this gate asserted the opposite
and would have passed only against an emitter that was wrong. `derive.rs` already refuses a `sched_class` the target
cannot honour; this wave adds no dimension.

Claim: phase-459-W2. Depends on: phase-459-W0. Owns: resolve_plan_sched in packages/cli/nros-cli-core/src/codegen/entry/mod.rs; packages/cli/nros-cli-core/tests/derived_tiers_entry.rs (new). Gate: cargo test -p nros-cli-core --test derived_tiers_entry. Status: PR #1231, in the queue; the generated entry for the W0 fixture ends in ZephyrBoard::run_tiers(..., __nros_tiers, 4u) over four derived tiers, the two 30 Hz nodes at priority 5 and the two 10 Hz at 6, both numbers being pool addresses from the board plan and not rank positions. Gates: 3 tests in derived_tiers_entry, 3 in derived_tiers_bake, nros-cli-core 1371 lib tests green. Two doc corrections, both carried above: this wave CANNOT live in plan_from_model, because cmake callback groups reach the plan only through enrich_plan and the rlm v0.1.37 SystemModel has no callback-group field, so it lives in resolve_plan_sched in the same file; and emit_cpp.rs needed no change at all.
...
Claim: phase-459-W4. Depends on: phase-459-W0 (fixture gate only). Owns: packages/core/nros-orchestration-ir/src/rtos_realizer.rs; packages/core/nros-orchestration-ir/src/priority_plan.rs (new) and its mod line in lib.rs; the realize_rtos call in packages/core/nros-orchestration-ir/src/derive.rs; scripts/lib/priority_plan.py; scripts/check-tier-priority-plan-image.py; docs/design/0079-priority-is-allocated-not-authored.md; packages/platform/nros-platform/src/board/tier.rs. Gate: cargo test -p nros-orchestration-ir priority_plan; python3 scripts/check-tier-priority-plan-image.py on the fixture .config. Status: PR #1231, in the queue; `realize_rtos` allocates from `PriorityPlan::pool.app` rather than from a hardcoded band, and the island projection holds: 30 Hz at priority 5, 10 Hz at 6, with the transport band [0, 4] and the application pool [5, 14] read from the board's own `.config`. Gates: `priority_plan` 9 passed (163 crate-wide), `check-tier-priority-plan-image.py` OK on the island `.config` with 8 pins, and a READ=16 negative control reports the stale band [0, 14] and fails those pins; selftest OK over 14 cases.
...
| the entry runs what was derived | W2 fixture test on `codegen entry --lang cpp --board zephyr` | output ends in `run_tiers(..., 4u)`, one tier per node; group tables match the plan, least urgent first |
