...
- [x] **108.C.xrce.3 — Full DDS QoS via FastDDS XML profiles.** When `qos_needs_xml(&qos)` returns true (any of: `deadline_ms`, `lifespan_ms`, `liveliness_lease_ms`, or `liveliness_kind ∈ {ManualByTopic, ManualByNode}`), `create_publisher` / `create_subscriber` build a FastDDS XML profile string (`build_topic_xml`, `build_datawriter_xml`, `build_datareader_xml`) into a 1024-byte heapless string and route through `uxr_buffer_create_topic_xml` / `_publisher_xml` / `_subscriber_xml` / `_datawriter_xml` / `_datareader_xml`. The XML emits `<durability>` / `<reliability>` / `<history>` (always) plus `<deadline>`, `<lifespan>`, `<liveliness>` (only when non-zero / non-default). The default Reliable/Volatile/KeepLast(10) profile keeps the smaller binary `_bin` path. `supported_qos_policies()` now advertises `LIFESPAN | LIVELINESS_AUTOMATIC | LIVELINESS_MANUAL_BY_TOPIC | LIVELINESS_MANUAL_BY_NODE | LIVELINESS_LEASE` in addition to the prior `CORE | DURABILITY_TL | AVOID_ROS_NAMESPACE_CONVENTIONS | DEADLINE`. Agent-side enforcement: lifespan drops expired samples on the wire; liveliness configures the DDS DataReader. Status-event delivery from agent to client is NOT implemented — XRCE protocol carries no session→client liveliness-changed callback — so `Subscriber::supports_event(LivelinessChanged)` stays `false`, and `assert_liveliness()` is a no-op (no XRCE submessage). 9 unit tests cover XML well-formedness + zero-omission + ms-to-(sec,nanosec) splitting. Adds 5 extern fns to `xrce-sys`. (`<this commit>`)
...
- [x] **108.C.zenoh.4-followup — Per-publisher liveliness count + pub-side keepalive timer.** Two pieces:
  1. **Per-publisher count.** Added `zpico_liveliness_get_count(handle)` to the C shim — exposes the running reply-count from the pending-get slot's reply handler (which now increments `reply_count` on every reply, not just the first). Rust wrapper `Context::liveliness_get_count` plus `ZenohSubscriber::handle_count_transition` use the count to compute a real `alive_count_change` on each query completion. `LivelinessChanged` now reports the actual delta between the previous and new alive count, not the prior `{0, 1}` approximation.
  2. **Pub-side `LivelinessLost`.** `ZenohPublisher` captures `liveliness_kind` + `liveliness_lease_ms` at create. `assert_liveliness()` refreshes `last_assert_at_ms` for `MANUAL_BY_TOPIC` / `MANUAL_BY_NODE`. `publish_raw` calls `check_liveliness_lost` before each wire write — fires `LivelinessLost` rate-limited to ≤ 1 per lease window when the gap exceeds `liveliness_lease_ms`. AUTOMATIC + NONE kinds never fire (zenoh session keepalive covers AUTOMATIC; NONE means opt-out). Known limitation: if the app stops publishing entirely, no event fires (publisher path has no spin tick).
  
  `supported_qos_policies()` now also advertises `LIVELINESS_MANUAL_BY_TOPIC` + `LIVELINESS_MANUAL_BY_NODE`. Cross-backend matrix test extended with a `LivelinessLost` scenario (lease=100ms, sleep > lease, re-publish → 1 fire; assert_liveliness resets the lease, next publish does not fire). (`<this commit>`)
...
- [x] **108.C.x.1 — Cross-backend status-event matrix.** Per-backend smoke test verifying `Subscriber::supports_event` / `Publisher::supports_event` mask + `register_event_callback` ok/err for supported/unsupported kinds.
  - **dust-DDS** — `nros-rmw-dds/tests/status_events_matrix.rs`. 3 tests passing (sub-side mask: full Tier-1; pub-side mask: full Tier-1; supported_qos_policies: full DDS surface).
  - **XRCE-DDS** — inline at `nros-rmw-xrce/src/lib.rs::xml_tests`. 2 tests passing (sub: only `RequestedDeadlineMissed`; pub: only `OfferedDeadlineMissed`). No agent / session needed — exercises the trait method via stub-constructed entities.
  - **uORB** — `nros-rmw-uorb/tests/typeless_api.rs`. 2 tests passing (sub: only `MessageLost`; pub: nothing) plus pre-existing `message_lost_event_fires_on_dropped_messages` behavior test.
  - **zenoh-pico** — `nros-rmw-zenoh/tests/status_events_matrix.rs`. 1 combined `zenoh_event_matrix` test passing (sub-side full Tier-1 mask, pub-side OfferedDeadlineMissed + LivelinessLost slot, supported_qos_policies includes CORE+DEADLINE+LIFESPAN+LIVELINESS_AUTOMATIC). Run via `cargo test --features "platform-posix link-tcp"`. Uses an in-process zenohd fixture (project's `build/zenohd/zenohd`) on a free TCP port; client mode against it. All assertions packed into a single `#[test]` because zpico-sys's C shim keeps entity slots in `static` arrays that segfault under multiple-session bring-up. Unblocked the **alignment bug** root cause: `nros-platform-posix::net::Socket` had a `_tls_sock: *mut c_void` field that doesn't exist in zenoh-pico's TLS-disabled `_z_sys_net_socket_t` (default build), so the Rust struct mismatched the C ABI by 12 bytes / different alignment, and `&*(ptr as *const Socket)` panicked on `udp_send`. Restored the TLS-off layout (`{ int _fd; }`); a future TLS opt-in can re-introduce `_tls_sock` behind a `link-tls` cargo feature.
  Side-fixes landed alongside: zenoh shim had a duplicate `has_data` definition (E0201 under `platform-posix`-only test build) — kept the deadline+liveliness-checking version (Phase 108.C.zenoh.4); `RMW_ATTACHMENT_SIZE` import was `safety-e2e`-gated but the new MessageLost path (108.C.zenoh.5) needed it unconditionally; unused `PlatformClock as _` imports removed from `now_ms` helpers; `TIMESTAMP_INCREMENT_NS` const marked `#[allow(dead_code)]` (legacy pre-Phase-108 timestamping landing pad). `EmbeddedRawPublisher::supports_event` added (already existed on `RawSubscription` and the typed `Publisher<M>` wrapper). (`<this commit>`)
...
- [x] **108.B.7 — C / C++ user-facing wrappers.** C-side `nros_qos_t` already carried the full DDS-shaped surface from earlier Phase 109 prep (deadline_ms, lifespan_ms, liveliness_kind, liveliness_lease_ms, avoid_ros_namespace_conventions + DEFAULT/SENSOR_DATA/SERVICES profile constants). Added: `nros_publisher_assert_liveliness(publisher) -> nros_ret_t` C entry routing to `RmwPublisher::assert_liveliness()`. C++ side: `nros::QoS` extended with `Liveliness` enum + chainable setters (`deadline_ms`, `lifespan_ms`, `liveliness`, `liveliness_lease_ms`, `avoid_ros_namespace_conventions`) + matching const accessors; FFI `nros_cpp_qos_t` grew matching fields; all 6 `Node::create_*` call sites (publisher / subscription / service / client / action_server / action_client) populate the new fields; `Publisher<M>::assert_liveliness()` mirrors the C entry via `nros_cpp_publisher_assert_liveliness`. cbindgen regenerated `nros_generated.h` + `nros_cpp_ffi.h`. (commit `0fea442e`)
- [x] **108.B.8 — Book + Doxygen updates.** `book/src/design/rmw-vs-upstream.md` § 7 grew a per-backend QoS coverage matrix (dust-DDS native, XRCE-DDS via FastDDS XML profile when extended, zenoh-pico shim-emulated, uORB CORE only) plus cross-link to status-events; § 8 backend-coverage matrix updated to reflect 108.C reality (XRCE no native events except clock-emulated deadline; zenoh alive_count ∈ {0,1}; uORB MessageLost via publish-counter). `book/src/concepts/ros2-comparison.md` § 7 grew a manual-liveliness paragraph + cross-link. `mdbook build` clean. Doxygen rendering picks up the C/C++ surface additions automatically (cbindgen-generated headers carry the new doc-comments). (commit `<this commit>`)
...
| XRCE-DDS | ✅ Full DDS QoS via FastDDS XML profiles when extended (DEADLINE / LIFESPAN / LIVELINESS); binary fast path for default (commits `95df4d39`, `d049adb7`, `<this commit>`) | n/a | 🟡 DEADLINE (sub+pub) shim-emulated; LIVELINESS configured via XML but event delivery not feasible at this layer | ✅ honoured |
