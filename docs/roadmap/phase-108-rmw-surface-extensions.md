...
- [x] **108.C.x.1 — Cross-backend status-event matrix.** Per-backend smoke test verifying `Subscriber::supports_event` / `Publisher::supports_event` mask + `register_event_callback` ok/err for supported/unsupported kinds.
  ...
  - **zenoh-pico** — `nros-rmw-zenoh/tests/status_events_matrix.rs`. 3 tests landed but `#[ignore]`'d, blocked on a pre-existing `nros-platform-posix::net::udp_send` alignment bug (line 455) that crashes any zenoh-pico peer-mode session-open. Same panic as `tests/zenoh_integration.rs::test_session_open_close_peer`. Tests unblock on platform-posix fix; until then zenoh's `supports_event` matrix is documented in source + covered by the pre-existing per-policy behavior tests in `nros-rmw-zenoh/src/shim/{publisher,subscriber}.rs`.
