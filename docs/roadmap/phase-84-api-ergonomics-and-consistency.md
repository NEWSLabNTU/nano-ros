...
- [x] 84.D7 — **Partial**. What landed: (a) closed the `spin_once(-1)` timer-freeze footgun by clamping `timeout_ms` to `[0, i32::MAX]` at function entry in `Executor::spin_once` — the previous code only clamped the timer delta, so `spin_once(-1)` silently stopped advancing timers; now both I/O wait and timer delta see the same non-negative value. (b) Added `Duration`-taking parallel APIs as thin wrappers: `Executor::spin_once_for` / `spin_with_period`, `Promise::wait_for`, `Subscription::wait_next_for`. Users who want `Duration` can switch one call site at a time; existing `i32` / `u64` signatures stay. **Deferred**: the full "measure timer delta from a wall clock instead of reusing the poll timeout" refactor — that needs nros-node to depend on `nros-platform::clock_ms()`, which is a real cross-crate dependency decision worth its own PR. Until then, `spin_once(timeout_ms)` that actually blocks longer than `timeout_ms` inside `drive_io` still undercounts timer delta (same as today, just no worse).
