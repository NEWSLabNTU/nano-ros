...
- [ ] 84.D1 — Unify error types: the `nros` umbrella crate exposes **one** public error type (the unified `NanoRosError`-shaped type, with topic/service-name context). Per-crate errors (`nros-core::SerError`, `nros-node::NodeError`, `nros-rmw::TransportError`, etc.) become `pub(crate)` or at minimum stop being re-exported from `nros`. Implement `From<*>` for each sub-error so `?` bubbles cleanly. Users of the umbrella crate see one error surface; contributors working inside a sub-crate can still use the narrower type.
- [ ] 84.D2 — Fix the silent-hang footgun in `Node::create_action_server`: either have the server auto-drain `get_result` queries during `spin_once`, or add a fused `poll()` method that calls `try_accept_goal` + `try_handle_cancel` + `try_handle_get_result`. Update `examples/native/rust/zenoh/action-server/` accordingly.
- [ ] 84.D3 — `Client::send_request` / `ActionClient::send_goal_future` / `get_result_future` are single-slot — a second concurrent call silently overwrites. Either return `Err(TransportError::Full)` on re-entry, or allocate real slots.
- [ ] 84.D4 — `QosReliabilityPolicy::default()` returns `BestEffort` while `QosSettings::default()` is Reliable. Make the enum default match ROS 2 (`Reliable`).
- [ ] 84.D5 — Expand `prelude` with `Promise`, `TransportError`, manual-poll `Subscription`, `Publisher`, `HandleId`, `SpinOnceResult`
- [ ] 84.D6 — Replace `ExecutorConfig::from_env()`'s `Box::leak` with owned strings (gate behind `std`)
- [ ] 84.D7 — Take `core::time::Duration` on every wait API (`spin_once`, `spin`, `SpinOptions.timeout`, `Promise::wait`, `FeedbackStream::wait_next`, `spin_one_period`). Drop the negative-means-indefinite sentinel — **no in-repo example uses it**, and `spin_once(-1)` has a latent bug at `spin.rs:1391` where `timeout_ms.max(0) = 0` silently freezes timers. Users who want "forever" write `loop { exec.spin_once(Duration::from_millis(10)); }` and get timer delivery for free. Have the executor measure timer delta via `Clock` internally instead of reusing the poll timeout — removes the `spin_once` / timer-delta coupling entirely.
- [ ] 84.D8 — `executor.parameter::<T>("name")` should return `Result<ParameterBuilder, NodeError>` instead of `expect`ing
- [ ] 84.D9 — Add `Subscription::recv().await` / `wait_next()` / `futures_core::Stream` impls (currently only `FeedbackStream` has them)
- [ ] 84.D10 — Always export `PublisherOptions` / `SubscriberOptions` unconditionally. An RMW feature is always selected (backend is mandatory at compile time), so the `cfg(not(feature = "rmw-zenoh"))` gate is strictly wrong for XRCE/DDS users today. Remove all three cfg gates.

### Group E — RMW: backend asymmetry, static state, open signature

- [ ] 84.E1 — XRCE backend: implement `try_recv_raw_with_info` (fill `MessageInfo` from `SampleIdentity` / XRCE timestamps); add `safety-e2e` feature wiring. Silent divergence today.
...
- [ ] 84.F1 — Extract `zpico-smoltcp-platform` crate containing `net.rs` (~502 lines/platform, byte-identical across 4 bare-metal platforms)
- [ ] 84.F2 — Extract `nros-baremetal-common` crate for the shared `xorshift32` RNG (`random.rs`, 70 lines × 4), `sleep.rs` (44 lines × 4), and libc stubs (`libc_stubs.rs`, 247 lines × 2)
- [ ] 84.F3 — Split timing API into two traits: **`MonotonicClock::now() -> core::time::Duration`** (portable, every platform implements) and optional **`CycleCounter::{enable, read, cycles_per_us, measure}`** (raw u32 cycles, only on platforms with real hardware cycle counters — MPS2/STM32F4 via DWT). Drop ESP32's fake `CycleCounter` (it was returning µs cast as "cycles"); ESP32 keeps only `MonotonicClock`. Migrate every example that calls `CycleCounter::read()` / `measure()` to `MonotonicClock` unless it specifically needs cycle-exact WCET — audit each call site. Document the split in `book/src/reference/platform-api.md` and `book/src/porting/custom-platform.md`.
...
- [ ] 84.F5 — Move `set_network_state` globals (4 copies × ~45 lines of `static mut` + `set/clear` + `smoltcp_network_poll`) into a shared crate, with a `critical_section::Mutex<RefCell<_>>` or at minimum an `AtomicBool` aliasing guard. Accept the `smoltcp::Device` directly instead of `*mut ()`.
...
- [ ] 84.F7 — `Config` per-board divergence: define `trait BoardConfig { zenoh_locator, domain_id, from_toml }` in a shared crate, with transport-specific extension traits. Unifies the 4 ~90-line `from_toml` parsers.
- [ ] 84.F8 — Move `_z_listen_udp_unicast` from a hard-coded `-1` stub in `zpico-platform-shim::shim.rs:503` onto `PlatformUdp::udp_listen(...)` with a default `-1` impl so future ports can override
