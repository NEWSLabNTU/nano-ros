...
- [x] 84.B3 — **Partial**: added `impl_param_scalar!` macro mirroring the existing `impl_param_array!`, so `nros_param_{declare,get,set}_{bool,integer,double}` are now one ~60-line macro instance each instead of four hand-rolled 20-line functions. String variants keep their own path (buffer handling). Line count 1175 → 1055 (-10%). **Deferred**: the larger refactor — wrapping `nros-params::ParameterServer` as opaque storage and wiring to real `~/get_parameters` / `~/set_parameters` service endpoints — still needs `param-services`-style service wiring at the C FFI (alloc, Node/Executor integration, callback trampolines on top of `nros-params` which currently has no callback API). Belongs in its own PR per the Group B intro ("every item is multi-day because nros-node must grow the real implementation first").
- [x] 84.B4 — **Partial**: state-machine logic moved to `nros_node::lifecycle::LifecyclePollingNodeCtx` (new C-FFI-compatible variant of the existing `LifecyclePollingNode`). C wrapper `nros_lifecycle_state_machine_t` is now an opaque u64-storage struct that holds the Rust type inline; all register / trigger / finalise paths delegate. ABI-breaking: C callers can no longer reach into `current_state` / `on_configure` / etc. — they must use `nros_lifecycle_get_state()` + `nros_lifecycle_register_on_*()`. Line count 703 → 430 (-39%). State-machine tests moved to `nros_node::lifecycle::tests`. **Deferred**: exposing the REP-2002 `~/change_state` / `~/get_state` ROS 2 services at the C FFI — that requires `param-services`-style service wiring (alloc + `nros_lifecycle_msgs` codegen) and belongs in its own PR.
