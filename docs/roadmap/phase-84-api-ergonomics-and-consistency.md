...
- [ ] 84.D3 — **Deferred** (not a nit; needs state-tracking design). The audit finding's simplest case ("two concurrent `send_request` calls silently overwrite") is actually prevented by the borrow checker — `Promise<'a, _>` borrows `&mut self`, so the compiler forbids a second call while the first is live. The real hazard is *dropping* the first Promise without awaiting, then calling again — which ends the borrow but leaves a pending request in flight on the wire. Properly fixing this needs (a) a per-client "in-flight" flag set on send / cleared on successful `Promise::try_recv`, (b) decision on what `Promise::Drop` does (mark abandoned vs. silently leak), (c) matching logic in the next `call()` to return `Err(TransportError::Full)` if a previous call is abandoned but not resolved. Touches every client / action-client path. Belongs in its own PR.
...
- [ ] 84.D7 — **Deferred** to a dedicated PR. Take `core::time::Duration` on every wait API (`spin_once`, `spin`, `SpinOptions.timeout`, `Promise::wait`, `FeedbackStream::wait_next`, `spin_one_period`). Drop the negative-means-indefinite sentinel — **no in-repo example uses it**, and `spin_once(-1)` has a latent bug at `spin.rs:1391` where `timeout_ms.max(0) = 0` silently freezes timers. Users who want "forever" write `loop { exec.spin_once(Duration::from_millis(10)); }` and get timer delivery for free. Have the executor measure timer delta via `Clock` internally instead of reusing the poll timeout — removes the `spin_once` / timer-delta coupling entirely. Blast radius: every `spin_once(i32)` / `Promise::wait(u64)` call site in the workspace + all examples + book — mass signature migration deserves its own focused PR.
- [x] 84.D8 — `executor.parameter::<T>("name")` should return `Result<ParameterBuilder, NodeError>` instead of `expect`ing
- [ ] 84.D9 — **Deferred** to a dedicated PR. Add `Subscription::recv().await` / `wait_next()` / `futures_core::Stream` impls (currently only `FeedbackStream` has them). Needs a small design pass (how does `wait_next` borrow the executor? matches the pattern from `Promise::wait` but over repeated reads).
