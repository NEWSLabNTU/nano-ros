...
**#0990** (config-header single writer). On PR #235: **#0998** (sertype TU used
hosted headers on freestanding boards), **#1003** (ten entry templates dropped
the session name, so every image registered as `"node"` and XRCE client keys
collided), **#0999** (preflight resolved `config/rust-targets.txt` from the
workspace root, so the build-std fix was inert for every out-of-tree build).
