# Phase 118: Example Matrix Coverage Completion
...
After Phase 118 lands, the matrix has exactly two states per cell:
...
- [ ] **118.A.1 — Matrix snapshot script.** `tools/example-matrix.py` walks `examples/` and prints the table from §B in the same shape, plus per-cell case completeness. Lives next to `tools/`'s existing scripts. Used by 118.A.2 and the eventual lint.
- [ ] **118.A.2 — `examples/README.md` matrix table.** Replace the current ad-hoc list with the autogenerated table, plus a "Deliberately empty" subsection that names every out-of-scope cell with its reason.
- [ ] **118.A.3 — `just check-example-matrix` lint.** Wraps `tools/example-matrix.py --lint`. Wired into `just ci` and PR CI.
...
- [ ] **118.B.1 — `qemu-arm-freertos/{c,cpp,rust}/xrce/`.** Template: `zephyr/c/xrce/`. Transport: UDP via slirp + MicroXRCEAgent on host. Six cases each language → 18 examples.
- [ ] **118.B.2 — `qemu-arm-nuttx/{c,cpp,rust}/xrce/`.** Template: `zephyr/*/xrce/`. NuttX UDP path same as zenoh. 18 examples.
- [ ] **118.B.3 — `qemu-riscv64-threadx/{c,cpp,rust}/xrce/`.** ThreadX NetX-Duo BSD socket layer is the agent transport. 18 examples.
- [ ] **118.B.4 — `threadx-linux/{c,cpp,rust}/xrce/`.** Bridge-net path; mirrors threadx-linux zenoh. 18 examples.
- [ ] **118.B.5 — `qemu-esp32-baremetal/rust/xrce/`.** ESP32 LWIP UDP. Six cases.
- [ ] **118.B.6 — `esp32/rust/xrce/`.** Real-hardware ESP32. Two cases (talker, listener) — service/action variants deferred until ESP32 zenoh ships them too (matrix internal-symmetry rule).
- [ ] **118.B.7 — `stm32f4/rust/xrce/`.** smoltcp UDP via the lan9118 board. Two cases (talker/listener) for symmetry with stm32f4 zenoh.

#### DDS coverage on C/C++

- [ ] **118.C.1 — `qemu-arm-freertos/{c,cpp}/dds/`.** dust-DDS is already on the Rust talker/listener for FreeRTOS; the C ABI from `nros-c`/`nros-cpp` reaches it without backend changes. 12 examples.
- [ ] **118.C.2 — `qemu-arm-nuttx/{c,cpp}/dds/`.** Same shape. 12 examples.
- [ ] **118.C.3 — `qemu-riscv64-threadx/{c,cpp}/dds/`.** Same shape. 12 examples.
- [ ] **118.C.4 — `threadx-linux/{c,cpp}/dds/`.** Same shape. 12 examples.

#### Service / action variants on Rust-on-RTOS cells

- [ ] **118.D.1 — `qemu-arm-baremetal/rust/zenoh/`** add `service-{client,server}` + `action-{client,server}`. Today only `talker`+`listener` and the rtic specials. Four examples.
- [ ] **118.D.2 — `qemu-arm-baremetal/rust/dds/`** add the same four.
- [ ] **118.D.3 — `qemu-esp32-baremetal/rust/{zenoh,dds}/`** add the same four × two RMWs = 8.
- [ ] **118.D.4 — `qemu-arm-freertos/rust/dds/`** add the same four.
- [ ] **118.D.5 — `qemu-arm-nuttx/rust/dds/`** add the same four.
- [ ] **118.D.6 — `qemu-riscv64-threadx/rust/dds/`** add the same four.
- [ ] **118.D.7 — `threadx-linux/rust/dds/`** add the same four.
- [ ] **118.D.8 — `stm32f4/rust/zenoh/`** add `listener` + service/action quartet (today only talker + rtic). Five examples.
- [ ] **118.D.9 — `esp32/rust/zenoh/`** add service/action quartet. Real-hardware constraints — bench on ESP32-WROOM. Four examples.
...
- [ ] **118.F.1 — `nros_tests::matrix` integration test.** A single nextest that drives `tools/example-matrix.py --lint` and asserts no untriaged cells. Catches future drift the same way `just check` catches Cargo workspace drift.
- [ ] **118.F.2 — Per-cell smoke run flag.** `nros_tests` already has per-platform groups; this work item adds a `matrix` group that just *builds* every example (no run) on every PR. `just test-all` retains the actual run-and-assert tier for the platforms that have a test harness.
...
```
tools/example-matrix.py                            (new, 118.A.1)
examples/README.md                                 (rewritten, 118.A.2 + 118.E.x)
examples/qemu-arm-freertos/c/xrce/{6 cases}/       (new, 118.B.1)
examples/qemu-arm-freertos/cpp/xrce/{6 cases}/     (new, 118.B.1)
examples/qemu-arm-freertos/rust/xrce/{6 cases}/    (new, 118.B.1)
examples/qemu-arm-nuttx/{c,cpp,rust}/xrce/...      (new, 118.B.2)
examples/qemu-riscv64-threadx/{c,cpp,rust}/xrce/...(new, 118.B.3)
examples/threadx-linux/{c,cpp,rust}/xrce/...       (new, 118.B.4)
examples/qemu-esp32-baremetal/rust/xrce/...        (new, 118.B.5)
examples/esp32/rust/xrce/{talker,listener}/        (new, 118.B.6)
examples/stm32f4/rust/xrce/{talker,listener}/      (new, 118.B.7)
examples/qemu-arm-freertos/{c,cpp}/dds/{6 cases}/  (new, 118.C.1)
examples/qemu-arm-nuttx/{c,cpp}/dds/...            (new, 118.C.2)
examples/qemu-riscv64-threadx/{c,cpp}/dds/...      (new, 118.C.3)
examples/threadx-linux/{c,cpp}/dds/...             (new, 118.C.4)
examples/{various}/rust/{zenoh,dds}/{service-*,action-*}/  (new, 118.D.x)
packages/testing/nros-tests/tests/example_matrix.rs (new, 118.F.1)
justfile                                           (edited, 118.A.3 + 118.F.2)
```
