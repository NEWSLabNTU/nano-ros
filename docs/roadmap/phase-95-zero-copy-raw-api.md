# Phase 95 — Zero-copy raw pub/sub API (loan + borrow)
...
(to be authored as part of 95.A).
...
┌─────────────────────────────────────────┐
│ 95.A  design doc                        │
│ 95.B  trait surface (SlotLending,       │
│       SlotBorrowing in nros-rmw)        │
│ 95.C  PublishLoan + RecvView types      │
│ 95.D  arena impl (no-lending path)      │
│ 95.E  uORB backend (arena-only impl)    │
│ 95.F  zenoh-pico lending impl (gated)   │
│ 95.G  XRCE-DDS lending impl (gated)     │
│ 95.H  Promise-driven async loan/borrow  │
│ 95.I  migrate PX4 examples to new API   │
│ 95.J  migrate zenoh/xrce examples (opt) │
│ 95.K  benchmark + docs                  │
└─────────────────────────────────────────┘
```

Phases 95.E (uORB) lands the **arena-only path end-to-end** as the proof
that the abstraction is sound. 95.F/G add native lending opt-in. 95.I
...
### v1 (95.A–95.I — required)

- [ ] 95.A — Design doc (`docs/design/zero-copy-raw-api.md`)
- [ ] 95.B — Trait surface (`SlotLending`, `SlotBorrowing` in `nros-rmw`)
- [ ] 95.C — `PublishLoan` + `RecvView` types in `nros-node`
- [ ] 95.D — Arena impl (no-lending path) for `EmbeddedRawPublisher` / `RawSubscription`
- [ ] 95.E — uORB backend wiring (arena-only; serves as parity oracle)
- [ ] 95.F — Zenoh-pico lending impl behind `lending` feature
- [ ] 95.G — XRCE-DDS lending impl behind `lending` feature
- [ ] 95.H — Promise-driven `loan()` / `borrow()` futures w/ `pin-project-lite`
- [ ] 95.I — Migrate PX4 talker/listener examples to loan/borrow

### Post-v1 (95.J + 95.K)

- [ ] 95.J — Migrate zenoh / xrce examples to loan/borrow (showcase
      cross-backend uniformity)
- [ ] 95.K — Benchmark vs `publish_raw`/`try_recv_raw` baseline; docs
...
- Migration plan (Phase 95.I) and expected perf delta
...
### 95.E — uORB backend (arena-only impl, parity oracle)
...
### 95.I — Migrate PX4 examples to loan/borrow
...
### 95.J — Migrate zenoh / xrce examples (post-v1)
...
- **MultiThreadedExecutor (Phase 94.H) interaction:** `RecvView` is
  ...
  arena uses spin-lock or single-task assumption (decision in 95.D).
