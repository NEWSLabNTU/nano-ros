...
- [N/A] 91.E2 — **Dropped by design.** Originally proposed: extract a header-only `nros::examples::pubsub_helpers` in `nros-cpp/include/nros/examples/` to collapse the ~30-line `init → create_node → create_publisher → spin loop` skeleton repeated across 18 C++ example mains. Rejected because each example is a standalone project that users are expected to copy out of the nano-ros source tree as a starting template. Hiding the boilerplate inside a helper header would defeat that pedagogical purpose — the boilerplate **is** the lesson, and a copied-out example must show the full call sequence. Per-main duplication stays as a deliberate design choice.
