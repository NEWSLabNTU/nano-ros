...
**Status.** Complete (2026-06-12). The build-scope + shallow + reclaim levers
landed and are **validated end to end** by dispatch run 27404888355: **all 6
platform-ci e2e cells green** (qemu, nuttx, threadx_riscv64, threadx_linux, esp32,
freertos) — no `No space left on device`, no embedded-compile death. Several CI
regressions + three pre-existing cpp-fixture bugs (#36/#37/#38) the honest e2e
surfaced were fixed en route (not disk — separate owners; see Acceptance).
...
- [x] **Full e2e green on all heavy cells.** Dispatch `run_e2e` run 27404888355:
      **all 6 cells green** — qemu, nuttx, threadx_riscv64, threadx_linux, esp32,
      freertos (freertos, the original disk-exhaustion victim, runs to completion).
      The disk + compile levers carry the heavy `build-all` path end to end. Three
      pre-existing cpp-fixture bugs the honest e2e surfaced en route were fixed (not
      disk — separate owners): **#36** (nuttx cpp newlib-vs-NuttX `stdlib.h` `div_t`
      clash → NuttX `include/cxx` precedence), **#37** (e2e `play_launch_parser`
      missing → `NROS_PLAY_LAUNCH_PARSER` env), **#38** (ThreadX RV64 bare-metal cpp
      heap containers → `NROS_PLATFORM_HAS_MALLOC` opt-in in `baremetal.h`).
