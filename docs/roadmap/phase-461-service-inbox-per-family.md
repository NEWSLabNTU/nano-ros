...
Claim: phase-461-W2b. Depends on: phase-461-W1, phase-461-W2. Owns: packages/rmw/zenoh/nros-rmw-zenoh/src/shim/service.rs (the family selection and BUILTIN_INBOX), packages/rmw/zenoh/nros-rmw-zenoh/build.rs (the third table's sizing), packages/rmw/zenoh/nros-rmw-zenoh/tests/zenoh_integration.rs, scripts/check/config-knob-census.py (the knob's description), book/src/reference/environment-variables.md. Gate: the zenoh_integration builtin-family test and just check knob-single-reader. Status: PR #1224, in the queue; frees 83,040 B at the island's 32-bit entry size. The overflow it is measured against is 56,848 B, not the 53,912 B an earlier draft of this line claimed: `just board-build` on pin f0d191c98 was run on 2026-09-24 and printed `region 'RAM' overflowed by 56848 bytes`, which is the figure `docs/roadmap/phase-5-contract-chain.md` already carried. Headroom is therefore 26,192 B and not 29,128 B. Both numbers are still projections over a measured baseline, and island-W3 replaces them with a link or refutes them.
