...
- [x] **115.K.2 — port nros-rmw-xrce to C (landed 2026-05-12).** Drop `xrce-sys` (auto-
  ...
  - [~] **115.K.2.5** — drop the Rust crate; flip `-DNROS_C_RMW=xrce`
    ...
    - [x] **115.K.2.5.1 — Rust API user migration (landed
      ...
      - [x] **115.K.2.5.1.3-zephyr — landed 2026-05-12.**
        Cross-compile bring-up shipped end-to-end. Six Zephyr
        Rust XRCE examples
        (`examples/zephyr/rust/xrce/{talker,listener,
        service-server,service-client,action-server,action-client}`)
        now produce a `zephyr.elf` on `native_sim/native/64`
        via `just zephyr build-fixtures`.

        Cross-compile pieces that landed in
        `packages/xrce/nros-rmw-xrce-cffi/build.rs`:
        - `_POSIX_C_SOURCE=200809L` define gated on the
          `platform-posix` feature.
        - `transport_posix_{udp,serial}.c`, `udp_transport.c`,
          `udp_transport_posix.c`, `util/time.c` source files
          dropped from the cc::Build set on
          `platform-{zephyr,bare-metal,freertos,nuttx,threadx}`.
        - Generated `<uxr/client/config.h>` drops the
          `UCLIENT_PROFILE_{UDP,TCP,SERIAL}` profiles when no
          POSIX transport is compiled; emits
          `UCLIENT_PLATFORM_ZEPHYR` on the Zephyr feature.
        - New Cargo features:
        ...
        - `internal.h`: `uxrUDPTransport udp` field gated on
          ...
          falls back to the custom-transport MTU when UDP is
          off.
        - `session.c`: POSIX includes + UDP/serial open paths
          all gated on `UCLIENT_PROFILE_UDP` /
          `UCLIENT_PLATFORM_POSIX`. Embedded session_open
          accepts only `custom://` locators; UDP / serial
          return `NROS_RMW_RET_UNSUPPORTED`.

        Zephyr CMake glue (`zephyr/CMakeLists.txt`): XRCE
        selector now sets
        `rmw-cffi,cffi-xrce-c,platform-zephyr,ros-humble`
        (C side) and
        `rmw-cffi,rmw-xrce-cffi,platform-zephyr,ros-humble`
        (C++ side).

        Per-example `.cargo/config.toml` `[patch.crates-io]`
        entries swapped from the deleted `nros-rmw-xrce` path
        to `nros-rmw-xrce-cffi`. Stale `Cargo.lock` files
        regenerated by `west build`.

        Runtime validation (Zephyr serial / UDP transport
        callback registration via Zephyr native socket / UART
        APIs) is the consumer's responsibility — pre-register
        a transport via `nros_rmw_cffi_set_custom_transport`
        before `nros::init`. `xrce-zephyr` retains the
        `xrce_zephyr.c` time + readiness shim.
...
- [x] `book/src/porting/custom-transport.md` documents the
  ...
  framing semantics.
- [x] **115.A.2 — landed 2026-05-12.** Three deliverables ship:

  1. **Rust-side struct + abi_version field.**
     `NrosTransportOps { abi_version: u32, _reserved, user_data,
     open, close, write, read }` in
     `packages/core/nros-rmw/src/custom_transport.rs` (kept
     there because the legacy `nros-rmw-{dds,zenoh}` impl
     crates import it directly and moving would create an
     import cycle). `NROS_TRANSPORT_OPS_ABI_VERSION_V1 = 1`.
  2. **C-side header** at
     `packages/core/nros-rmw-cffi/include/nros/rmw_transport.h`.
     Hand-written (not cbindgen-emitted because cbindgen
     would re-export the struct from `nros-rmw-cffi` which
     doesn't own it — see #1). Declares
     `nros_transport_ops_t`, `NROS_TRANSPORT_OPS_ABI_VERSION_V1`
     macro, threading contract, return-code conventions, and
     the `nros_rmw_cffi_set_custom_transport` C entry.
  3. **C entry point** `nros_rmw_cffi_set_custom_transport` in
  ...
  Spec deviation: struct stayed in `nros-rmw` instead of
  moving to `nros-rmw-cffi` because doing so would create a
  cycle (`nros-rmw-cffi` already depends on `nros-rmw`). The
  C-side header mirrors the layout exactly; downstream
