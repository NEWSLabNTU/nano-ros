...
This guide explains how to create new nros examples for each supported platform. All examples should use **generated message bindings** from `nros generate-rust` — never hand-write message types.
...
2. **`generated/`** directory produced by `nros generate-rust`
...
nros generate-rust --config --nano-ros-path ../../../crates
```
...
```just
cd examples/<platform>/<name> && nros generate-rust --config --nano-ros-path ../../../crates
```
...
├── src/
│   └── main.rs
├── generated/          # nros generate-rust output
│   ├── builtin_interfaces/
│   └── std_msgs/
└── .cargo/
...
Generated by `nros generate-rust --config --nano-ros-path ../../../crates`. The result looks like:
...
Create the `[build]` and `[target.*]` sections **first**, then run `nros generate-rust --config` to add the patch entries. The ConfigPatcher preserves existing sections.
...
C++ examples use the `nros-cpp` header-only library with message types generated by `nano_ros_generate_interfaces(... LANGUAGE CPP)`. The API mirrors rclcpp naming.

### Native C++ Examples (`examples/native/cpp/`)
...
- `nano_ros_generate_interfaces()` creates `<pkg>__nano_ros_cpp` target
- Link against `NanoRos::NanoRosCpp` for core headers + FFI library
- Each example directory needs a `.gitignore` with `/build/`

### Zephyr C++ Examples (`examples/zephyr/cpp/`)
...
- `CONFIG_NROS_CPP_API=y` selects the C++ API (mutually exclusive with `CONFIG_NROS_C_API` and `CONFIG_NROS_RUST_API`)
- `nros_generate_interfaces(... LANGUAGE CPP)` generates C++ bindings and FFI glue
- Entry point is `int main(void)` (standard C++ main, not `rust_main`)
- Zephyr handles `-fno-exceptions -fno-rtti` automatically via `CONFIG_CPP=y`
- No `.cargo/config.toml` or `Cargo.toml` needed — Zephyr module handles Rust compilation

---

## Platform Comparison

|                    | Native (Rust)             | Platform (QEMU/STM32F4)          | Zephyr (Rust)               | Native (C++)                                     | Zephyr (C++)                                 |
|--------------------|---------------------------|----------------------------------|-----------------------------|--------------------------------------------------|----------------------------------------------|
| **Entry point**    | `fn main()`               | `#[entry] fn main() -> !`        | `extern "C" fn rust_main()` | `int main()`                                     | `int main(void)`                             |
| **`std` support**  | Yes                       | No                               | No                          | Yes                                              | No                                           |
...
| **Msg generation** | `nros generate-rust` | `--config --nano-ros-path`       | `nros generate-rust`   | `nano_ros_generate_interfaces(... LANGUAGE CPP)` | `nros_generate_interfaces(... LANGUAGE CPP)` |
