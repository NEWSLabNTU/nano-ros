...
**Native (host)** (requires `libmbedtls-dev` — installed by `just setup`):
...
./scripts/qemu/launch-mps2-an385.sh --tap tap-qemu0 \
    --binary examples/mps2-an385-baremetal/rust/talker/target/thumbv7m-none-eabi/release/qemu-bsp-talker

# Terminal 3: Listener (192.0.2.11)
./scripts/qemu/launch-mps2-an385.sh --tap tap-qemu1 \
    --binary examples/mps2-an385-baremetal/rust/listener/target/thumbv7m-none-eabi/release/qemu-bsp-listener
```
