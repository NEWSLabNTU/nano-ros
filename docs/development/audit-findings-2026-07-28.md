...
- **A1** · `packages/api/nros-c/cmake/NanoRosLink.cmake` · P2 · **FIXED 2026-09-11** (phase-451 W1, issue 1218) · A dead 261-line duplicate of `cmake/NanoRosLink.cmake` defining the same public verbs from a retired design era, sitting beside four live modules so it reads as authoritative. It went on to mislead two of four independent readers in one session, which is what got it filed. Deleted; the finding was also understated — its `_nano_ros_rmw_targets` resolved `zenoh`/`xrce`/`cyclonedds` to `NrosRmwZenoh`/`NrosRmwXrce`/`NrosRmwCyclonedds`, and **no such config package exists in the tree**, so the file could not have run had anything included it.
