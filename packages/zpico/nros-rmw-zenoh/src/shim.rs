...
// Generated by build.rs from NROS_SUBSCRIBER_BUFFER_SIZE env var (default 1024).
...
type ZeroCopyCallbackBox = alloc::boxed::Box<dyn FnMut(&[u8], Option<MessageInfo>) + Send>;
...
/// # Threading
///
/// - **Embedded (polling):** The callback fires synchronously on the executor thread
///   during `spin_once()` → `zp_read()`.
...
/// Trampoline from C callback to the Rust closure stored in the Box.
#[cfg(all(feature = "unstable-zenoh-api", feature = "alloc"))]
extern "C" fn zero_copy_trampoline(
    data: *const u8,
    len: usize,
    attachment: *const u8,
    attachment_len: usize,
    ...
    // Safety: ctx points to a leaked Box<dyn FnMut(...)> that outlives the subscriber.
    let cb: &mut ZeroCopyCallbackBox = unsafe { &mut *(ctx as *mut ZeroCopyCallbackBox) };
    let slice = unsafe { core::slice::from_raw_parts(data, len) };
    let info = if !attachment.is_null() && attachment_len > 0 {
        let att = unsafe { core::slice::from_raw_parts(attachment, attachment_len) };
        MessageInfo::from_attachment(att)
    } else {
        None
    };
    cb(slice, info);
...
impl ShimZeroCopySubscriber {
    ...
    pub fn new(
        ...
        // Box the callback and get a raw pointer for the C ctx parameter
        let mut boxed_cb: ZeroCopyCallbackBox = alloc::boxed::Box::new(callback);
        let ctx_ptr: *mut core::ffi::c_void =
            &mut boxed_cb as *mut ZeroCopyCallbackBox as *mut core::ffi::c_void;

        // Safety: We extend the lifetime because the C shim manages its own state.
        // The boxed callback is leaked (ManuallyDrop) so it outlives the subscriber.
        let subscriber = unsafe {
            let sub_result =
        ...
        Ok(Self {
            _subscriber: subscriber,
            _callback: core::mem::ManuallyDrop::new(boxed_cb),
        })
    }
}
...
mod ghost_checks {
    ...
    /// Uses the lock pattern: check has_data/overflow, lock, call closure, unlock, clear.
    /// Returns Ok(Some(bytes)) on success, Ok(None) if no data, Err on overflow.
