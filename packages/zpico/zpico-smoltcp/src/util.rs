//! Shared parsing utilities for TCP and UDP platform symbols.
...
pub(crate) unsafe fn parse_ip_address(s: *const u8) -> Option<[u8; 4]> {
    if s.is_null() {
        return None;
    }
...
///
/// `s` must be a valid null-terminated C string.
pub(crate) unsafe fn parse_port(s: *const u8) -> Option<u16> {
    if s.is_null() {
        return None;
    }
