...
```bash
cd examples/stm32f4-rs-polling
...
Same as stm32f4-rs-rtic example - see `memory.x` for details.
...
probe-rs run --chip STM32F429ZITx target/thumbv7em-none-eabihf/release/stm32f4-rs-polling-example
```
...
- `examples/stm32f4-rs-rtic/` - RTIC-based example with priority scheduling
