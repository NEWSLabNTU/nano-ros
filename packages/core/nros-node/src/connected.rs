...
impl<const MAX_TOKENS: usize, const MAX_TIMERS: usize> ConnectedNode<MAX_TOKENS, MAX_TIMERS> {
    ...
    ///     .mandatory()?;
    /// ```
    pub fn declare_parameter<'a, T: nros_params::ParameterVariant>(
        &'a mut self,
        name: &'a str,
    ) -> nros_params::ParameterBuilder<'a, T> {
        nros_params::ParameterBuilder::new(&mut self.parameter_server, name)
    }
    ...
    pub fn now(&self) -> nros_core::Time {
        self.clock.now()
    }
...
impl<M: RosMessage, const RX_BUF: usize> ConnectedSubscriber<M, RX_BUF> {
    ...
    pub fn try_recv_with_info(
        &mut self,
    ) -> Result<Option<(M, nros_core::MessageInfo)>, ConnectedNodeError> {
            ...
            .map_err(|_| ConnectedNodeError::DeserializationFailed)?
        {
            Some((len, transport_info)) => {
                ...
                // Convert transport MessageInfo to nros_core MessageInfo
...
mod tests {
    ...
    impl nros_core::RosMessage for MockGoal {
    ...
    impl nros_core::RosMessage for MockResult {
    ...
    impl nros_core::RosMessage for MockFeedback {
