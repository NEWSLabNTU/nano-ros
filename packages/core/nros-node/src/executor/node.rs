...
impl<'a> NodeHandle<'a> {
    ...
    pub fn create_subscription_with_qos<M: MessageForRmw, const RX_BUF: usize>(
        ...
        // phase-454 W10/W13 — see the same line in `executor/spin.rs`.
    ...
    pub fn create_subscription_raw_sized<const RX_BUF: usize>(
        ...
        // phase-454 W10/W13 — the raw path costs the arena exactly what
        // the typed one does, so leaving it out would make the declared depth
        // enforceable everywhere except in the images that decode bytes
        // themselves.
        let qos = crate::declared_qos::honour(topic.type_name, topic.name, qos)?;
        let handle = self
            .session
