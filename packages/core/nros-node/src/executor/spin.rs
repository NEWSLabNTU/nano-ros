...
pub(crate) struct SubscriptionOpen {
    ...
    pub(crate) slot_bytes: usize,
    /// The QoS the backend was actually given — phase-454 W13 through
    /// phase-456 W8.
    ///
    /// NOT the caller's `req.qos`. `declared_qos::honour` REWRITES the depth
    /// when the system declared one, and the twelve prologues this function
    /// replaced each passed that rewritten value on to the entry they built.
    /// Returning it is what keeps that true with one honouring site: a caller
    /// that stored `req.qos` instead would size its entry from a depth the
    /// backend never saw, and nothing would fail — the subscription would
    /// simply hold a different number of samples than the build reserved for
    /// it.
    pub(crate) qos: QoSProfile,
}
...
pub struct Executor<'s> {
    ...
    /// Alive supervision, one record per SchedContext: dispatches are counted
    /// in the dispatch path where the SC index is already resolved, and read
    /// once per monitor tick. CARVED from the backing like `sporadic_states`.
    /// It began as an inline `[_; MAX_SC]` pair on the claim that `MAX_SC` is a
    /// build-time constant; it is a SIZING KNOB (`ExecutorSizing::sc`), so the
...
impl<'s> Executor<'s> {
    ...
    pub(crate) fn open_subscription(
        ...
        // phase-454 W10/W13 — the depth this registration asks for against the
        // depth this system DECLARED for the topic. Before the backend call AND
        // before the arena claim, so the region this registration takes and the
        // one the build reserved are the same number; unchanged when nothing
        // was declared, which is every image with no contract sidecar. See
        // `declared_qos::honour` for why Rust TAKES the declared depth where C
        // and C++ assert it at the call site.
        //
        // phase-456 W8 — this used to be `declared_qos::check` here and
        // `honour` in each of the twelve hand-rolled prologues this function
        // replaced. Collapsing them onto `check` would have silently reverted
        // W13 for every Rust registration: `check` validates and returns, while
        // `honour` REWRITES the depth the backend is then given. One consulting
        // site is the point of this function, and it has to be the honouring
        // one.
        let qos = crate::declared_qos::honour(topic.type_name, topic.name, req.qos)?;
        let handle = {
            let session = self
                .session_at_mut(session_idx)
                .ok_or(NodeError::BackendMismatch)?;
    ...
    ///
    /// `timeout` is the upper bound on the I/O wait. Since phase-436 W2 it is
    /// carried in MICROSECONDS all the way to the park (issue 1193 — an
    /// `as_millis()` here truncated `from_micros(500)` to `0`, and a zero
    /// timeout selects the non-blocking path, so a sub-millisecond `spin()`
    /// was a 100 % CPU loop with no warning); the conversion to whatever the
    /// platform primitive accepts happens once, at the park, and rounds UP.
    /// `Duration` has no negative sentinel, which is what the pre-phase-84
    /// `timeout_ms: i32` signature used to silently mean "freeze the timers
    /// and poll I/O forever".
    ...
    fn spin_once_capturing(
        ...
        let consume_dispatch_runtime_us =
            |desc_idx: usize,
             ...
             alive_slots: &mut [super::monitor::AliveSlot],
             #[cfg(feature = "alloc")] sporadic_atomic_states: &[Option<(
                ...
                // carry a budget.
                if let Some(slot) = alive_slots.get_mut(sc_idx) {
                    slot.dispatches = slot.dispatches.wrapping_add(1);
                }
