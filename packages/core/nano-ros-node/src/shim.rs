...
use heapless::String;
use nano_ros_core::{
    CdrReader, CdrWriter, Deserialize, RosAction, RosMessage, RosService, Serialize,
};
...
pub struct ShimActiveGoal<A: RosAction> {
    /// Goal ID
    pub goal_id: nano_ros_core::GoalId,
    /// Current status
    pub status: nano_ros_core::GoalStatus,
    /// The goal data
    pub goal: A::Goal,
}
...
> ShimNodeActionServer<A, GOAL_BUF, RESULT_BUF, FEEDBACK_BUF, MAX_GOALS>
{
    ...
    pub fn try_accept_goal(
        &mut self,
        goal_handler: impl FnOnce(&A::Goal) -> nano_ros_core::GoalResponse,
    ) -> Result<Option<nano_ros_core::GoalId>, ShimNodeError>
    where
        A::Goal: Clone,
    {
        ...
        let uuid_len = reader
            ...
            .map_err(|_| ShimNodeError::Transport(TransportError::DeserializationError))?
            as usize;
        let mut goal_id = nano_ros_core::GoalId::default();
        if uuid_len == 16 {
        ...
        if accepted {
            let _ = self.active_goals.push(ShimActiveGoal {
                ...
                status: nano_ros_core::GoalStatus::Accepted,
                goal,
            });
            Ok(Some(goal_id))
        } else {
    ...
    pub fn set_goal_status(
        &mut self,
        goal_id: &nano_ros_core::GoalId,
        status: nano_ros_core::GoalStatus,
    ) {
    ...
    pub fn try_handle_cancel(
        ...
        cancel_handler: impl FnOnce(
            &nano_ros_core::GoalId,
            nano_ros_core::GoalStatus,
        ) -> nano_ros_core::CancelResponse,
    ) -> Result<Option<(nano_ros_core::GoalId, nano_ros_core::CancelResponse)>, ShimNodeError> {
        use nano_ros_core::{CancelResponse, CdrReader, CdrWriter};
    ...
    pub fn try_handle_get_result(&mut self) -> Result<Option<nano_ros_core::GoalId>, ShimNodeError>
    where
        A::Result: Clone,
    {
        ...
        if let Some(completed_goal) = completed {
            ...
            // Unknown goal
            writer
                .write_i8(nano_ros_core::GoalStatus::Unknown as i8)
                .map_err(|_| ShimNodeError::Serialization)?;
            A::Result::default()
                .serialize(&mut writer)
                .map_err(|_| ShimNodeError::Serialization)?;
    ...
    pub fn get_goal(&self, goal_id: &nano_ros_core::GoalId) -> Option<&ShimActiveGoal<A>> {
        self.active_goals
            .iter()
            .find(|g| g.goal_id.uuid == goal_id.uuid)
    }
    ...
    ShimNodeActionClient<A, GOAL_BUF, RESULT_BUF, FEEDBACK_BUF>
{
    ...
    pub fn send_goal(&mut self, goal: &A::Goal) -> Result<nano_ros_core::GoalId, ShimNodeError> {
        use nano_ros_core::{CdrReader, CdrWriter};
    ...
    pub fn try_recv_feedback(
        &mut self,
    ) -> Result<Option<(nano_ros_core::GoalId, A::Feedback)>, ShimNodeError> {
        use nano_ros_core::CdrReader;
    ...
    pub fn cancel_goal(
        &mut self,
        goal_id: &nano_ros_core::GoalId,
    ) -> Result<nano_ros_core::CancelResponse, ShimNodeError> {
        use nano_ros_core::{CdrReader, CdrWriter};
        ...
        Ok(nano_ros_core::CancelResponse::from_i8(return_code).unwrap_or_default())
    }
    ...
    pub fn get_result(
        &mut self,
        goal_id: &nano_ros_core::GoalId,
    ) -> Result<(nano_ros_core::GoalStatus, A::Result), ShimNodeError> {
        use nano_ros_core::{CdrReader, CdrWriter};
        ...
        let status = nano_ros_core::GoalStatus::from_i8(status_code).unwrap_or_default();
