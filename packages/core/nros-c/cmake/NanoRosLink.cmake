...
function(nano_ros_link_rmw TARGET)
    ...
    # Phase 104.B.6 — bare-metal explicit-call stub. nros-c
    # declares `nros_app_register_backends()` as a weak no-op;
    # this block writes a strong def per `nano_ros_link_rmw`
    # invocation that calls the backend's `nros_rmw_<x>_register`
    # fn. Multiple calls (e.g. zenoh + xrce on a bridge node)
    # accumulate into one stub.
    #
    # POSIX hosts: the strong def harmlessly duplicates the
    # work the backend's .init_array ctor (phase 104.A) does.
    # `nros_rmw_<x>_register` is idempotent — the runtime
    # vtable slot accepts the most-recent-write.
    # Bare-metal targets without .init_array: the strong def
    # IS the only register call site; without this stub, no
    # backend would ever register and `Executor::open` would
    # fail.
    ...
    set(_stub_content "/* Phase 104.B.6 — auto-generated by nano_ros_link_rmw().\n")
    string(APPEND _stub_content " * Strong def of nros_app_register_backends(); overrides the\n")
    string(APPEND _stub_content " * weak no-op shipped with nros-c's libnros_c_weak_stubs.a.\n")
