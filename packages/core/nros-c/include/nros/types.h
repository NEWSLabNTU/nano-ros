...
/* Probe-derived storage sizes (`NROS_*_SIZE`). Auto-generated by
 * `nros-c/build.rs` from `core::mem::size_of` of the actual Rust types
 * via `nros-sizes-build`. Made transitively available to every nros C
 * header via this `types.h` include. */
...
/* Phase 87: opaque-storage sizes for `nros_support_t`, `nros_publisher_t`,
 * `nros_client_t`, `nros_guard_condition_t`, and `nros_lifecycle_state_machine_t`
 * are now sourced from the Rust SSoT via `nros-sizes-build`. The probed
 * values land in `nros_config_generated.h` as `NROS_SESSION_SIZE`,
 * `NROS_PUBLISHER_SIZE`, `NROS_SERVICE_CLIENT_SIZE`,
 * `NROS_GUARD_CONDITION_SIZE`, and `NROS_LIFECYCLE_CTX_SIZE`. The
 * hand-coded `*_OPAQUE_U64S` macros that used to live here have been
 * deleted. */
