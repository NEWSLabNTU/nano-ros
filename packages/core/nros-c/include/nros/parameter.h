 ...
 *
 * The function declarations below remain hand-written because they are
 * generated by `paste!` macros in nros-c/src/parameter.rs (the typed
 * setters / getters for bool / integer / double / *_array). cbindgen
 * cannot expand these on a stable toolchain (`-Zunpretty=expanded` is
 * nightly-only), so the declarations would be silently missing from
 * the cbindgen output. Keep them here until the macros are manually
 * expanded or cbindgen gains a stable expand mode.
 */
