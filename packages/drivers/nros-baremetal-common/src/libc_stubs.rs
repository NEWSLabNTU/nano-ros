//! Minimal libc stubs for bare-metal targets without a C runtime.
//!
//! Required by C libraries (zenoh-pico, XRCE-DDS) but not available in
//! no_std. Provides minimal implementations sufficient for embedded use.
//!
//! Exported as `#[unsafe(no_mangle)]` symbols directly — these are C
//! standard library functions resolved at link time.
//!
//! **Linker discipline**: enable the `libc-stubs` feature on **exactly
//! one** crate per binary. Multiple definitions cause linker errors.
//! ESP32 / ESP32-QEMU should NOT enable this — `esp-hal` already
...
pub unsafe extern "C" fn __assert_func(
    ...
    loop {
        // ARM Cortex-M breakpoint when available; otherwise just spin so
        // a debugger attached to the chip can still observe the halt.
