...
//! `nros_rmw_cffi_register` on the static vtable. We can't test the
...
fn register_resolves_and_returns() {
    ...
    // `nros_rmw_cffi_register` is unresolved at link time. We
...
/// real `nros_rmw_cffi_register` symbol that ships in the
...
#[unsafe(no_mangle)]
extern "C" fn nros_rmw_cffi_register(_vtable: *const core::ffi::c_void) -> i32 {
    0
}
