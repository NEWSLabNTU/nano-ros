//! phase-281 W3b — runtime E2E for RT-TIERS on embedded C++: the
//! `ws-realtime-cpp` workspace's Zephyr (native_sim) Entry. The FIRST full west
//! link + runtime proof of the W3a `ZephyrBoard::run_tiers` seam (closes the
//! C++ × zephyr cell of the RFC-0015 Model-1 convergence matrix).
//!
...
//! the same closure with its `active_groups` filter installed.
//!
//! Assertion mirrors the Rust sibling `realtime_tiers_zephyr_entry_e2e`: two
...
//!
//! Run with: `cargo nextest run -p nros-tests --test realtime_tiers_cpp_zephyr_e2e`
...
/// `-DCONFIG_NROS_ZENOH_LOCATOR="tcp/127.0.0.1:17857"`).
const REALTIME_CPP_ZEPHYR_ENTRY_PORT: u16 = 17857;
...
#[test]
fn realtime_tiers_cpp_zephyr_entry_schedules_high_and_low() {
    let entry = build_zephyr_workspace_cpp_realtime_entry().unwrap_or_else(|e| {
    ...
    let router =
        ZenohRouter::start_on("127.0.0.1", REALTIME_CPP_ZEPHYR_ENTRY_PORT).unwrap_or_else(|e| {
            nros_tests::skip!("zenohd failed to start on {REALTIME_CPP_ZEPHYR_ENTRY_PORT}: {e}")
        });
        ...
        .unwrap_or_else(|_| {
            ...
            panic!(
                "telem (low tier, 100 ms) never delivered 5 samples from the C++ Zephyr \
                 entry — the low-tier k_thread did not run (281 W3b / ZephyrBoard::run_tiers)"
            )
        });
        ...
        .unwrap_or_else(|_| {
            ...
            panic!(
                "ctrl (high tier, 10 ms) produced nothing — the boot tier was \
                 not scheduled (281 W3b / ZephyrBoard::run_tiers)"
            )
        });
    ...
    assert!(
        ...
        "ctrl (10 ms tier) delivered {ctrl_n} ≤ telem's {telem_n} — the high \
         tier is not outrunning the low tier (281 W3b / ZephyrBoard::run_tiers)"
    );
}
