...
//! **rv-virt** (riscv32): the `ws-realtime-cpp` workspace's riscv Entry
//! (`src/riscv_nuttx_entry`). The riscv sibling of `realtime_tiers_cpp_nuttx_e2e`
//! (arm-virt) and the C++ sibling of `realtime_tiers_riscv_nuttx_e2e` (rust) —
//! this covers the (cpp, nuttx-riscv) cell of the RFC-0015 Model-1 convergence
...
//! slirp gateway (10.0.2.2 → host) at the baked port (17870 — the arm C++
//! sibling uses 17864, the rust riscv sibling 17867); the observers dial
//! 127.0.0.1:17870. No TAP/bridge/root.
//!
...
//!
//! Run with: `cargo nextest run -p nros-tests --test realtime_tiers_cpp_riscv_nuttx_e2e`

use nros_tests::fixtures::{
    ManagedProcess, QemuProcess, ZenohRouter, build_int32_sink,
    build_nuttx_riscv_workspace_cpp_realtime_entry, require_zenohd,
};
use std::{process::Command, time::Duration};

/// The router port baked into the C++ riscv realtime nuttx entry's locator (see the
/// `workspace-cpp-nuttx-riscv-realtime` fixture's `NROS_ENTRY_LOCATOR = "tcp/10.0.2.2:17870"`).
const REALTIME_CPP_RISCV_NUTTX_ENTRY_PORT: u16 = 17870;
...
#[test]
fn realtime_tiers_cpp_riscv_nuttx_entry_schedules_high_and_low() {
    if !require_zenohd() {
    ...
    let entry = build_nuttx_riscv_workspace_cpp_realtime_entry()
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|e| {
    ...
    let router = ZenohRouter::start_on("0.0.0.0", REALTIME_CPP_RISCV_NUTTX_ENTRY_PORT)
        .unwrap_or_else(|e| {
            nros_tests::skip!(
                "zenohd failed to start on {REALTIME_CPP_RISCV_NUTTX_ENTRY_PORT}: {e}"
            )
        });
    let _ = &router;
    let observer_locator = format!("tcp/127.0.0.1:{REALTIME_CPP_RISCV_NUTTX_ENTRY_PORT}");
