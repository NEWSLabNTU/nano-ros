...
use nros_tests::fixtures::{
    ManagedProcess, ZenohRouter, build_native_workspace_mixed_service_client_entry,
    build_native_workspace_mixed_service_server_entry, require_zenohd,
};
use std::{process::Command, time::Duration};

const SERVICE_PORT: u16 = 17873;

#[test]
fn mixed_service_roundtrip_delivers_cross_process() {
    if !require_zenohd() {
        nros_tests::skip!("zenohd not found");
    }

    let server = build_native_workspace_mixed_service_server_entry()
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|e| nros_tests::skip!("mixed service server entry not built: {e}"));
    let client = build_native_workspace_mixed_service_client_entry()
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|e| nros_tests::skip!("mixed service client entry not built: {e}"));
    ...
    let out = cli
        ...
        .unwrap_or_else(|_| {
            ...
            srv.kill();
            panic!(
                "c_add_client never received 3 server-computed sums — the cross-process mixed \
