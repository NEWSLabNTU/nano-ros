...
//! pure-C++ workspace. The C++ sibling of the Rust `action_roundtrip_xprocess_e2e` / the C
//! `c_action_roundtrip_xprocess_e2e`.
//!
//! Issue 0096: an in-process (same-executor) action server+client can't talk, so the server
//! (`cpp_fib_server_pkg`) and client (`cpp_fib_client_pkg`) run as TWO processes — one single-node
...
//!
//! C++ uses the POLL-model component action client (`create_action_client_raw` + the raw
//! `send_goal_async` / `get_result_async` / `poll` FFI + `set_callbacks`), not Rust's declarative
//! dispatch — a component callback must never block the executor.
//!
//! Run with: `cargo nextest run -p nros-tests --test cpp_action_roundtrip_xprocess_e2e`

use nros_tests::fixtures::{
    ManagedProcess, ZenohRouter, build_native_workspace_cpp_action_client_entry,
    build_native_workspace_cpp_action_server_entry, require_zenohd,
};
use std::{process::Command, time::Duration};

const ACTION_PORT: u16 = 17892;

#[test]
fn cpp_action_roundtrip_delivers_cross_process() {
    if !require_zenohd() {
        nros_tests::skip!("zenohd not found");
    }

    let server = build_native_workspace_cpp_action_server_entry()
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|e| nros_tests::skip!("C++ action server entry not built: {e}"));
    let client = build_native_workspace_cpp_action_client_entry()
        .map(|p| p.to_path_buf())
    ...
    let mut srv = {
        ...
        ManagedProcess::spawn_command(cmd, "cpp-fib-server")
            .unwrap_or_else(|e| panic!("spawn server: {e}"))
    };
    srv.wait_for_output_pattern("server ready", Duration::from_secs(10))
        .unwrap_or_else(|_| {
            srv.kill();
            panic!("cpp_fib_server never became ready")
        });

    let mut cli = {
        let mut cmd = Command::new(&client);
        cmd.env("NROS_LOCATOR", &locator);
        ManagedProcess::spawn_command(cmd, "cpp-fib-client")
    ...
    let out = cli
        ...
        .unwrap_or_else(|_| {
            ...
            srv.kill();
            panic!(
                "cpp_fib_client never received the server-computed Fibonacci result — the \
