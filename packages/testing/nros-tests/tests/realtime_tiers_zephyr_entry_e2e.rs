//! phase-276 W2 / issue #128 half 2 — runtime E2E for RT-TIERS on embedded:
//! the `ws-realtime-rust` workspace's Zephyr (native_sim) Entry.
//!
//! `system.toml` declares two `[tiers.*]` (high: 1 ms spin / 10 ms ctrl timer;
//! low: 10 ms spin / 100 ms telem timer) with `[tiers.*.zephyr]` raw
//! priorities, which flips the `nros::main!` Zephyr arm onto
//! `ZephyrBoard::run_tiers` (RFC-0015 Model 1): one `k_thread` per tier over
//! ONE shared zenoh session — the boot thread runs the `high` tier, a
//! static-pool thread runs `low`; each tier registers through the same
//! closure with its `active_groups` filter installed.
//!
//! Assertion mirrors the native `realtime_tiers_e2e`: two `int32-sink`
//! observers on `/ctrl` and `/telem`; anchor on the SLOW tier (5 telem
//! receives ≈ 0.5 s+ elapsed) and require the 10 ms ctrl tier to have
...
//!
//! Run with: `cargo nextest run -p nros-tests --test realtime_tiers_zephyr_entry_e2e`
...
/// The router port baked into the realtime zephyr entry (the west lane's
/// `-DCONFIG_NROS_ZENOH_LOCATOR="tcp/127.0.0.1:17855"`).
...
#[test]
fn realtime_tiers_zephyr_entry_schedules_high_and_low() {
    let entry = build_zephyr_workspace_rust_realtime_entry().unwrap_or_else(|e| {
        nros_tests::skip!("zephyr realtime workspace entry not built (west): {e}")
    });

    // Router on the exact port the fixture's CONFIG_NROS_ZENOH_LOCATOR was baked with.
    let router =
        ZenohRouter::start_on("127.0.0.1", REALTIME_ZEPHYR_ENTRY_PORT).unwrap_or_else(|e| {
            nros_tests::skip!("zenohd failed to start on {REALTIME_ZEPHYR_ENTRY_PORT}: {e}")
        });
    ...
    let telem_out = telem
        ...
        .unwrap_or_else(|_| {
            ...
            telem.kill();
            panic!(
                "telem (low tier, 100 ms) never delivered 5 samples from the Zephyr \
                 entry — the low-tier k_thread did not run (276 W2 / #128 half 2)"
            )
        });
        ...
        .unwrap_or_else(|_| {
            ...
            panic!(
                "ctrl (high tier, 10 ms) produced nothing — the boot tier was \
                 not scheduled (276 W2 / #128 half 2)"
            )
        });
    ...
    assert!(
        ...
        "ctrl (10 ms tier) delivered {ctrl_n} ≤ telem's {telem_n} — the high \
         tier is not outrunning the low tier (276 W2 / #128 half 2)"
    );
}
