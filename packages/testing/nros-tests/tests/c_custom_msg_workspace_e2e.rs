...
//! round-trip in the `ws-custom-msg-c` workspace. The C sibling of the Rust ws-custom-msg demo.
//!
...
//! hand-encode/decode the CDR payload (the RFC-0043 typed-component idiom, identical to how the
//! committed pure-C workspace handles `std_msgs/Int32`) — the differentiator is that the schema
//! is the workspace's own.
//!
//! Run with: `cargo nextest run -p nros-tests --test c_custom_msg_workspace_e2e`

use nros_tests::fixtures::{
    ManagedProcess, ZenohRouter, build_native_workspace_c_custom_msg_listener_entry,
    build_native_workspace_c_custom_msg_talker_entry, require_zenohd,
};
use std::{process::Command, time::Duration};

const CUSTOM_MSG_PORT: u16 = 17911;

#[test]
fn c_custom_msg_delivers_cross_process() {
    if !require_zenohd() {
        nros_tests::skip!("zenohd not found");
    }

    let talker = build_native_workspace_c_custom_msg_talker_entry()
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|e| nros_tests::skip!("C custom-msg talker entry not built: {e}"));
    let listener = build_native_workspace_c_custom_msg_listener_entry()
        .map(|p| p.to_path_buf())
    ...
    let mut tlk = {
        ...
        ManagedProcess::spawn_command(cmd, "c-reading-talker")
    ...
    let mut lis = {
        ...
        cmd.env("NROS_LOCATOR", &locator);
        ManagedProcess::spawn_command(cmd, "c-reading-listener")
    ...
    // The talker ramps `sequence` 0,1,2,…; the listener decodes + prints each one. Early
    // pre-discovery samples may be missed, so assert the field appears ≥3× rather than a strict
    // value prefix (proves the workspace-local custom-msg schema flows end-to-end).
