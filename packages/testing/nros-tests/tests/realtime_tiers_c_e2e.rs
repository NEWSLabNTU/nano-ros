...
//! (`ws-realtime-c`, nros↔nros, no ROS 2 required).
//!
//! `examples/workspaces/ws-realtime-c` declares two C components, each with a
//! cmake `CALLBACK_GROUPS` declaration. `system.toml` maps those groups to two
//! priority tiers (`[tiers.high]` / `[tiers.low]`) via `[[node_overrides]]`.
//! The codegen emits `nros_cpp_create_sched_context` + `nros_cpp_node_create_ex`
//! to bind each node to its posix-priority sched context (RFC-0015 §4.2).
//!
...
//!
//! Run with: `cargo nextest run -p nros-tests --test realtime_tiers_c_e2e`

use nros_tests::fixtures::{
    ManagedProcess, ZenohRouter, build_native_listener, build_native_workspace_c_realtime_entry,
...
#[rstest]
fn realtime_tiers_c_schedule_high_and_low(zenohd_unique: ZenohRouter) {
    if !require_zenohd() {
        nros_tests::skip!("zenohd not found");
    }
    let entry = build_native_workspace_c_realtime_entry()
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|e| nros_tests::skip!("ws-realtime-c entry fixture not built: {e}"));
    ...
    let mut proc =
        ManagedProcess::spawn_command(cmd, "realtime-c").expect("spawn realtime-c entry");
