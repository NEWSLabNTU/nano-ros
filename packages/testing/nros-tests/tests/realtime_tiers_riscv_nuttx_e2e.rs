//! phase-285 W6 (issue #165) — runtime E2E for RT-TIERS on NuttX QEMU
//! **rv-virt** (riscv32): the `ws-realtime-rust` workspace's riscv Entry. The
//! rv-virt sibling of `realtime_tiers_rust_nuttx_e2e` (arm-virt) — this covers
//! the (rust, nuttx-riscv) cell of the RFC-0015 Model-1 convergence matrix
//! (`exec_model_matrix.rs`; the board was promoted to a matrix axis once all
//! three riscv lanes went green, #199 follow-ups).
//!
...
//! the macro's generic OwnedSpin arm onto `<QemuRvVirt>::run_tiers` →
//! `nros_board_nuttx::run_tiers` (RFC-0015 Model 1): one `std::thread` per
//! tier over ONE shared zenoh session.
//!
...
//! slirp gateway (10.0.2.2 → host) at the baked port (17867 — the arm sibling
//! uses 17866); the observers dial 127.0.0.1:17867. No TAP/bridge/root.
//!
//! Assertion mirrors the arm sibling (#158 deterministic per-tier proof):
//! anchor on the SLOW tier (5 telem receives ≈ 0.5 s+ elapsed), then compare
//! the tiers' MONOTONIC payload counters — the 10 ms ctrl tier must outrun the
//! 100 ms telem tier by ≥3×.
//!
//! The fixture is built by `just nuttx build-riscv-rust` (provisions the
//! rv-virt kernel export + the cargo cross lane); this test skips cleanly when
//! the entry ELF / zenohd / qemu-system-riscv32 are absent.
//!
//! Run with: `cargo nextest run -p nros-tests --test realtime_tiers_riscv_nuttx_e2e`

use nros_tests::fixtures::{
    ManagedProcess, QemuProcess, ZenohRouter, build_int32_sink,
    build_nuttx_riscv_workspace_rust_realtime_entry, require_zenohd,
};
use std::{process::Command, time::Duration};

/// The router port baked into the riscv realtime nuttx entry's locator (see the
/// `workspace-rust-nuttx-riscv-realtime` fixture's `NROS_LOCATOR = "tcp/10.0.2.2:17867"`).
const REALTIME_RUST_RISCV_NUTTX_ENTRY_PORT: u16 = 17867;
...
#[test]
fn realtime_tiers_rust_riscv_nuttx_entry_schedules_high_and_low() {
    if !require_zenohd() {
    ...
    let entry = build_nuttx_riscv_workspace_rust_realtime_entry()
        .map(|p| p.to_path_buf())
        .unwrap_or_else(|e| {
            nros_tests::skip!("Rust riscv-nuttx realtime workspace entry not built: {e}")
        });
    ...
    let router = ZenohRouter::start_on("0.0.0.0", REALTIME_RUST_RISCV_NUTTX_ENTRY_PORT)
        .unwrap_or_else(|e| {
            nros_tests::skip!(
                "zenohd failed to start on {REALTIME_RUST_RISCV_NUTTX_ENTRY_PORT}: {e}"
            )
        });
    let _ = &router;
    let observer_locator = format!("tcp/127.0.0.1:{REALTIME_RUST_RISCV_NUTTX_ENTRY_PORT}");
    ...
    // Boot the NuttX rv-virt image (runs until killed). eth0 comes up via the
    // defconfig NETINIT at kernel boot AND `entry_net_init`'s SIOCSIFADDR push
    // (phase-285 W3) before `QemuRvVirt::run_tiers` opens the session.
    ...
    let telem_out = telem
        ...
        .unwrap_or_else(|_| {
            ...
            telem.kill();
            panic!(
                "telem (low tier, 100 ms) never delivered 5 samples from the riscv NuttX \
                 entry — the low-tier thread did not run (285 W4 / QemuRvVirt::run_tiers)"
            )
        });
