...
//! rv-virt sibling of `realtime_tiers_rust_nuttx_e2e` (arm-virt) — riscv-nuttx
//! is an OFF-MATRIX board (see `exec_model_matrix.rs`: the nuttx matrix cells
//! are arm by design), so this test is the board's own proof, not a matrix
//! cell.
//!
