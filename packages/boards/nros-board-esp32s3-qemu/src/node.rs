//! Platform initialization and `run()` entry point for ESP32-S3 QEMU.
//!
...
use nros_platform_esp32s3_qemu::random;
...
fn init_ethernet(config: &Config) {
    ...
    let openeth_config = openeth_smoltcp::Config {
        base_addr: openeth_smoltcp::ESP32S3_BASE,
    ...
    let iface = Interface::new(iface_config, eth, smoltcp::time::Instant::from_millis(nros_platform_esp32s3_qemu::clock::clock_ms() as i64));
    ...
    unsafe {
        ...
        nros_platform_esp32s3_qemu::sleep::set_poll_callback(
...
///
/// ESP32-S3 QEMU uses zenoh-pico's built-in serial support — no additional
...
/// Initialize all ESP32-S3 QEMU hardware and the transport stack.
///
...
pub fn init_hardware(config: &Config) {
    esp_println::println!("");
    esp_println::println!("========================================");
    esp_println::println!("  nros ESP32-S3 QEMU Platform");
    esp_println::println!("========================================");
    esp_println::println!("");

    // Step 1: Initialize ESP32 peripherals. Phase 117.2b — under
    // `dds-heap` we pass an explicit `PsramConfig` so esp-hal's
    // `init_psram` boot path runs, mapping the chip's external
    // SPI RAM (octal PSRAM up to 16 MiB on production parts; 8 MiB
    // on QEMU's modeled ESP32-S3) into the virtual address space.
    // The actual PSRAM heap region registration happens in step
    // 2b below.
    ...
    nros_platform_esp32s3_qemu::sleep::init_clock();
...
///
/// This is the main entry point for ESP32-S3 QEMU applications.
...
///
/// ```ignore
/// use nros_board_esp32s3_qemu::{Config, run};
/// use nros::prelude::*;
///
