//! Hardware initialization and entry point for ESP32 bare-metal
//!
//! Handles transport setup (WiFi + smoltcp or serial) depending on enabled
...
//! serial implementation handles the UART transport natively.
...
// ============================================================================
// Static storage for WiFi network objects
// ============================================================================
...
/// Initialize all ESP32 hardware and the selected transport.
///
/// Sets up ESP32 peripherals, heap allocator, and the selected transport
/// (WiFi + smoltcp or serial). After calling this, you can create an
...
pub fn init_hardware(config: &NodeConfig) {
    ...
    esp_println::println!("  nros ESP32-C3 Serial Platform");
    esp_println::println!("========================================");
    esp_println::println!("");
    ...
    #[cfg(all(feature = "serial", not(feature = "wifi")))]
    {
        esp_println::println!("Serial transport configured.");
        esp_println::println!("  Baudrate: {}", config.baudrate);
        // zenoh-pico's built-in ESP-IDF serial handles UART setup natively.
        // No additional initialization needed from the board crate.
    }
...
/// It handles all hardware and transport initialization, then calls
