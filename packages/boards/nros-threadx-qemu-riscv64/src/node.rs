...
///
/// On ThreadX, hardware and network initialization (NetX Duo + virtio-net)
...
pub fn init_hardware(_config: &Config) {
    // ThreadX network init (NetX Duo + virtio-net) happens in
    // tx_application_define() C code. Nothing to do before the kernel starts.
}
