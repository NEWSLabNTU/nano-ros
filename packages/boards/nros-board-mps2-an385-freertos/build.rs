 ...
 *
 * The `nros_app_config_t` typedef is inlined here so the TU is
 * self-contained (the shipped `<nros/zephyr/app_config.h>` header is
 * not on a `<nros/app_config.h>` include path on non-Zephyr builds
 * outside the legacy cmake-codegen flow). Keep the struct shape in
 * sync with `packages/core/nros-c/include/nros/zephyr/app_config.h`.
 */
