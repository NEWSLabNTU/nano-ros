...
int32_t nros_board_nuttx_run_tiers(const char* locator, uint8_t domain_id, const char* session_name,
    ...
    return nros_board_nuttx_run_tiers_ns(locator, domain_id, session_name, NULL, tiers, n_tiers);
}
 ...
 *
 * There is no environment rung on NuttX, so the bake IS the answer here; the
