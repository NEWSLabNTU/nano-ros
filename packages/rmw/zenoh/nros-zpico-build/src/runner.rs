...
/// A conforming `true`/`false` for NuttX (issue 1039).
///
/// NuttX's own `include/stdbool.h` — upstream's, not our export step — has:
///
/// ```c
/// #define true  (bool)1
/// #define false (bool)0
/// ```
///
/// C99/C11 §7.18 requires `true` to expand to the INTEGER CONSTANT `1` and
/// `false` to `0`. The cast breaks the two things a conforming program may do
/// with them: use them in `#if` (the preprocessor replaces the unknown
/// identifier `bool` with `0` and evaluates `(0)1` — "missing binary operator
/// before token 1"), and paste them (`##` cannot build two distinct identifiers
/// out of two casts, so a template expanded once per value defines one symbol
/// twice).
///
/// zenoh-pico's `src/session/keyexpr.c` does both, legitimately: it includes
/// `keyexpr_match_template.h` once with `true` and once with `false`. Every
/// NuttX build broke the moment the pin took upstream's "Faster key expression
/// matching" (#1175).
///
/// Pulling `<stdbool.h>` in FIRST is the whole trick: it sets NuttX's
/// `__INCLUDE_STDBOOL_H` guard, so the translation unit's own later
/// `#include <stdbool.h>` is a no-op and cannot put the casts back. A `-D`
/// cannot do this — the header is included afterwards and would win.
///
/// This is a WORKAROUND. The defect is NuttX's and the root fix belongs there:
/// it affects every NuttX target (arm, arm-smp and riscv all derive from that
/// one source) and every consumer, not only us. Kept to the zenoh-pico C build
/// so it cannot change what NuttX itself or any other component sees.
const NUTTX_STDBOOL_FIXUP: &str = "\
/* Generated by nros-zpico-build (issue 1039). Do not edit. */\n\
#ifndef NROS_NUTTX_STDBOOL_FIXUP_H\n\
#define NROS_NUTTX_STDBOOL_FIXUP_H\n\
#include <stdbool.h>\n\
#ifdef true\n\
#undef true\n\
#endif\n\
...
fn build_zenoh_pico_unified(
    ...
    // Issue 1039 — force a conforming `true`/`false` ahead of every zenoh-pico
    // TU on NuttX. Keyed on the TARGET TRIPLE, not the platform name: all three
    // exports (arm, arm-smp, riscv) carry the same non-conforming header and
    // every NuttX triple spells it.
    if target.contains("nuttx") {
        let fixup = out_dir.join("nros_nuttx_stdbool_fixup.h");
        std::fs::write(&fixup, NUTTX_STDBOOL_FIXUP)
            .unwrap_or_else(|e| panic!("write {}: {e}", fixup.display()));
        build.flag("-include");
        build.flag(fixup.display().to_string());
    }
    // issue 0383 — implicit-function-declaration / int-conversion as errors.
