...
/// Literal `#define`s lifted verbatim from zenoh-pico's `config.h.in`.
///
/// zenoh-pico 1.10 stopped shipping `include/zenoh-pico/config.h`; it is
/// generated by their CMake, which this build does not run. Of the 95 `Z_*`
/// knobs that template defines, **48 are plain literals** — protocol key bytes
/// (`Z_CONFIG_MODE_KEY 0x40`), sizes (`Z_ZID_LENGTH 16`) and similar. They are
/// not ours to choose: they are the wire protocol.
///
/// They are read from the template at build time rather than copied into this
/// file. Copying is how a generator ends up 54 knobs behind the version it is
/// generating for, which is exactly where this one was — and the copy gives no
/// signal when upstream adds the 96th.
///
/// Only the `@TOKEN@` lines are skipped. Those are the six genuine tunables,
/// and nano-ros emits its own values for them elsewhere in this header.
///
/// `already_defined` is consulted so a knob this build deliberately tunes is
/// never shadowed by the upstream default.
fn upstream_literal_defines(zenoh_pico_dir: &std::path::Path, already_defined: &[&str]) -> String {
    let template = zenoh_pico_dir.join("include/zenoh-pico/config.h.in");
    let Ok(text) = std::fs::read_to_string(&template) else {
        // Not fatal: an older pin ships a real config.h and needs none of this.
        return String::new();
    };

    let mut out = String::new();
    let _ = writeln!(out, "/* Literal constants lifted from {}. */", template.display());
    for line in text.lines() {
        let t = line.trim();
        let Some(rest) = t.strip_prefix("#define ") else { continue };
        // `@TOKEN@` is a CMake substitution — a tunable, not a constant.
        if rest.contains('@') {
            continue;
        }
...
pub fn config_header(
    ...
    // Everything above is a knob nano-ros CHOOSES. Everything below is one it
    // MUST MATCH — protocol constants read out of zenoh-pico's own template so
    // this generator cannot fall behind the version it generates for.
    let chosen: Vec<&str> = header
        .lines()
        .filter_map(|l| l.trim().strip_prefix("#define "))
        .filter_map(|r| r.split_whitespace().next())
        .collect();
    write!(header, "{}", upstream_literal_defines(zenoh_pico_dir, &chosen)).unwrap();
