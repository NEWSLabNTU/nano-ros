...
int main() {
    ...
    if (g_vt->create_publisher(&node, pubr.topic_name, pubr.type_name, "", 98, &req, nullptr,
    ...
    if (g_vt->create_subscription(&node, sub.topic_name, sub.type_name, "", 98, &req, nullptr,
                                  &sub) != NROS_RMW_RET_OK) {
