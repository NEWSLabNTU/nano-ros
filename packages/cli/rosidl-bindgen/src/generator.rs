...
/// read from `rosidl_codegen::codegen_version::NROS_CODEGEN_VERSION` rather than re-spelled here, so
...
/// scope.
///
/// The version is `rosidl_codegen::codegen_version::NROS_CODEGEN_VERSION` -- the runtime constant
/// itself, not a copy of it, so there is nothing here that can drift.
///
/// The assert message does NOT restate the version, and that is deliberate. A
/// `const`-context `assert!` cannot format, so any number in the message would
/// be baked in as text at generation time — a second copy of a fact that is
/// declared three lines above it. The two move together whenever the emitter
/// writes them, so they agree in real generated code; they disagree the moment
/// anything edits one and not the other, and then the message states a version
/// the crate does not carry. A message that can lie is worse than a message
/// that points at the constant, which is right there.
...
"#,
        version = rosidl_codegen::codegen_version::NROS_CODEGEN_VERSION,
    )
}
...
"#;
    ...
    fn lib_rs_carries_the_codegen_version_token() {
        ...
        let expected = format!(
            ...
            rosidl_codegen::codegen_version::NROS_CODEGEN_VERSION
        );
        assert!(
