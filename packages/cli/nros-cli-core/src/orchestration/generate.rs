...
/// phase-267 W1c/C3 — render the `nros-bridge.toml` the runtime
/// [`nros_bridge::run_from_config`] consumes, resolved from a `[[bridge]]` plan.
/// `None` for a non-bridge plan.
///
/// Emits one `[[node]]` per distinct bridged session (rmw + domain + locator)
/// and one `[[bridge]]` per (topic, direction). The user wrote only the topic
/// NAME; the ROS type name is resolved here from the plan's declared entities
