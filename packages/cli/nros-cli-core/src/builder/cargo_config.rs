...
pub struct CargoConfigSpec {
    ...
    /// `[env]` rows whose value is a PATH (a board descriptor, a platform
    /// source tree). Written `{ value = "<rel>", relative = true }` against
    /// [`base_dir`], so cargo hands a build script an absolute path whatever its
    /// working directory, and the file itself stays checkout-independent. A
    /// key present in both maps takes this one.
    pub path_env: BTreeMap<String, PathBuf>,
    ...
    /// The retypable command for the header, when it is not the default
    /// "build the `Cargo.toml` beside this file" (a single-package example's
    /// manifest is the package's own, not one under `build/`).
...
pub fn render(spec: &CargoConfigSpec, config_path: &Path) -> Result<String, String> {
    ...
    for (k, p) in &spec.path_env {
        let mut row = InlineTable::new();
        row.insert("value", rel(p)?.into());
        row.insert("relative", true.into());
        env.insert(k, Item::Value(Value::InlineTable(row)));
    }

    // ---- [profile.<nros preset>] ----------------------------------------------
    //
    // The named presets a build may ask for with `--profile` (`nros-minsizerel`
    // is every NuttX and FreeRTOS image's). cargo refuses an undefined profile,
    // and it finds a `[profile]` only in a manifest or in a config it DISCOVERS
    // from the working directory — which is why a cargo invocation used to have
    // to run from inside this checkout for `--profile nros-*` to work at all.
    // Carrying them here makes the working directory carry no build fact: a
    // copy-out example outside the checkout builds the same way. The table is
    // `nros-cargo-profile`'s, the ONE list the repo's own `.cargo/config.toml`
    // and root `Cargo.toml` are gated against; an identical `[profile]` from a
    // discovered config merges with this one key for key, so the two never
    // disagree.
    for preset in nros_cargo_profile::PRESETS {
        let mut t = Table::new();
        for s in preset.settings {
            let v: Value = nros_cargo_profile::toml_value(s).parse().map_err(|e| {
                format!(
                    "[profile.{}] {} = {}: {e}",
                    preset.name,
                    s.key,
                    nros_cargo_profile::toml_value(s)
                )
            })?;
            t.insert(s.key, Item::Value(v));
        }
        let profile = table(&mut doc, "profile");
        profile.set_implicit(true);
        profile.insert(preset.name, Item::Table(t));
    }
    ...
    let hint = spec.build_hint.clone().unwrap_or_else(|| {
        "cargo build --manifest-path <this dir>/Cargo.toml --config <this file>".to_string()
    });
    let hint: String = hint.lines().map(|l| format!("#   {l}\n")).collect();
    let mut out = format!(
        "# GENERATED by `nros build` / `nros sync` (RFC-0098 D1, phase-445 W4) — DO NOT EDIT.\n\
         #\n\
         # Every cargo setting image `{image}` needs, in ONE file: the board's\n\
         # `cargo_config` (board `{board}`), this image's target dir, its entity\n\
         # facts and pool knobs as `[env]`, the in-repo `[patch]` rows and the\n\
         # nano-ros cargo profiles. Regenerated on every build; edit the board\n\
         # descriptor or the image.\n\
         #\n\
         # Build it yourself with:\n\
         {hint}\
         #\n\
         # Relative paths below resolve against this file's GRANDPARENT\n\
         # (the `build/` side of it), which is how cargo reads a `--config` file.\n\
...
"#;
    ...
    #[test]
    fn a_path_env_row_is_relative_to_the_grandparent_and_marked_relative() {
        let mut s = spec();
        s.path_env.insert(
            "NROS_BOARD_TOML".into(),
            PathBuf::from("/nros/packages/boards/nros-board-esp32-qemu/nros-board.toml"),
        );
        let v = parsed(&render(&s, &cfg_path()).unwrap());
        assert_eq!(
            v["env"]["NROS_BOARD_TOML"]["value"].as_str(),
            Some("../../../../../packages/boards/nros-board-esp32-qemu/nros-board.toml")
        );
        assert_eq!(
            v["env"]["NROS_BOARD_TOML"]["relative"].as_bool(),
            Some(true)
        );
    }

    /// cargo refuses `--profile nros-minsizerel` unless a DISCOVERED config or
    /// the manifest defines it; carrying the presets here is what lets the
    /// working directory stop mattering (phase-445 W4b).
    #[test]
    fn the_nros_profiles_are_carried_from_the_one_table() {
        let v = parsed(&render(&spec(), &cfg_path()).unwrap());
        for preset in nros_cargo_profile::PRESETS {
            let block = &v["profile"][preset.name];
            for setting in preset.settings {
                assert!(
                    block.get(setting.key).is_some(),
                    "[profile.{}] lacks {}",
                    preset.name,
                    setting.key
                );
        ...
        assert_eq!(
            v["profile"]["nros-minsizerel"]["opt-level"].as_str(),
            Some("s")
        );
    }

    #[test]
    fn a_build_hint_replaces_the_default_command_in_the_header() {
        let mut s = spec();
        s.build_hint = Some("cargo build --manifest-path talker/Cargo.toml".into());
        let body = render(&s, &cfg_path()).unwrap();
        assert!(
            body.contains("#   cargo build --manifest-path talker/Cargo.toml\n"),
            "{body}"
        );
        assert!(!body.contains("<this dir>"), "{body}");
    }
