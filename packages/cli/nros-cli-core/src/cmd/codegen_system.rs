...
// Each argument is a distinct bake input; grouping them into a struct would
// only move the coupling, not remove it (issue 0379 lint cleanup, no refactor).
...
// Each argument is a distinct plan input; grouping them into a struct would
// only move the coupling, not remove it (issue 0379 lint cleanup, no refactor).
#[allow(clippy::too_many_arguments)]
fn render_plan_json(
    bringup: &BringupPackageEntry,
...
mod tests {
    ...
    fn scratch_dir(test: &str) -> PathBuf {
        crate::test_support::scratch_dir(&format!("codegen_system_{test}"))
    }
