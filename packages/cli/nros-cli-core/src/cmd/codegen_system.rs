...
fn emit_bake_tree(
    ...
    write_if_changed(
        ...
        &render_system_config_h(&bringup.system, target),
    )?;
    write_if_changed(
...
fn render_system_config_h(sys: &SystemToml, target: Option<&str>) -> String {
    ...
    let rmw = sys.resolved_rmw(target, None);
...
"#,
    ...
    fn system_config_h_emits_capability_defines() {
        ...
        .unwrap();
        let h = render_system_config_h(&sys, None);
        ...
        let hb = render_system_config_h(&bare, None);
        ...
        .unwrap();
        assert!(!render_system_config_h(&off, None).contains("NROS_SYSTEM_SAFETY_E2E"));
    }
    ...
    fn system_config_h_rmw_honours_deploy_override() {
        ...
        let h = render_system_config_h(&sys, Some("native"));
        assert!(
        ...
        let hd = render_system_config_h(&sys, None);
