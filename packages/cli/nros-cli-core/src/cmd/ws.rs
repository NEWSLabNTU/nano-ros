...
pub enum Sub {
    ...
    /// phase-445 W3 (RFC-0098 D3/D5) — print a single-package leaf's
    /// deployment (board, RMW, domain, network identity, and the C/C++ deploy
    /// token) from its `system.toml`, as `KEY=VALUE` lines.
    ///
    /// The seam for a build system that cannot link the reader: the C/C++
    /// `find_package(nano_ros)` asks this before it imports nano-ros. The
    /// answer is `nros_orchestration_ir::leaf_system`'s, the same reader
    /// `nros::main!` and `nros sync` use.
    #[command(name = "leaf-system")]
    LeafSystem(crate::cmd::leaf_system::LeafSystemArgs),
...
/// phase-445 W3 — the launch file of a single-package leaf, synthesised from
/// the `[[component]]` rows of its `system.toml` into
/// `<leaf>/build/nros/launch/<leaf-dir>/system.launch.xml`.
///
/// `None` when `leaf` is not such a leaf (no `system.toml`, not a package, or
/// still on the retiring manifest keys) or declares no component. Written only
/// when the content changes: the model's staleness check compares its mtime
/// against the launch files', so a rewrite on every sync would re-resolve it
/// on every sync.
fn synthesise_leaf_launch(leaf: &Path) -> Result<Option<PathBuf>> {
    use nros_orchestration_ir::leaf_system;
    if !leaf.join(leaf_system::SYSTEM_TOML).is_file() || !leaf_system::is_package_dir(leaf) {
        return Ok(None);
    }
    ...
    let exec = pkg_name
        .as_deref()
        .and_then(|p| crate::orchestration::launch_synth::discover_exec_target(leaf, p).ok());
    let esc = |s: &str| {
        s.replace('&', "&amp;")
            .replace('"', "&quot;")
            .replace('<', "&lt;")
    ...
    let mut xml = String::from(
        "<?xml version=\"1.0\"?>\n\
         <!-- Generated by `nros sync` from this leaf's system.toml [[component]] rows\n     \
         (phase-445 W3, RFC-0098 D3). Do not edit; edit system.toml. -->\n<launch>\n",
    );
    for c in &decl.components {
        let (Some(pkg), Some(name)) = (c.pkg.as_deref().or(pkg_name.as_deref()), c.name.as_deref())
        else {
            bail!(
                "{}: every leaf `[[component]]` needs `pkg` and `name`",
                decl.origin_path().display()
            );
        };
        let exec = exec.as_deref().unwrap_or(name);
        xml.push_str(&format!(
            "  <node pkg=\"{}\" exec=\"{}\" name=\"{}\"/>\n",
            esc(pkg),
            esc(exec),
    ...
    let dir = leaf
        ...
        .join("launch")
        .join(leaf_name);
    let file = dir.join("system.launch.xml");
...
fn resolve_system_models(scan: &[WsPkg], verbose: bool, model_dir: Option<&Path>) -> Result<()> {
    ...
    for pkg in scan {
        let mut launch_dir = pkg.dir.join("launch");
        if !launch_dir.is_dir() {
            // phase-445 W3 (RFC-0098 D3) — a single-package leaf that states
            // its deployment in `system.toml` resolves a model the way a
            // bringup does, from a launch file SYNTHESISED from its
            // `[[component]]` rows, so `nros ws entity-facts --bringup-dir
            // <leaf>` answers for it (RFC-0098 D7). Nothing is authored twice:
            // the rows ARE the launch description of a one-package system.
            match synthesise_leaf_launch(&pkg.dir)? {
                Some(dir) => launch_dir = dir,
                None => continue,
            }
        }
...
fn board_declared_patch_rows(
    ...
    for leaf in leaf_dirs {
        let Some(deploy) = leaf_projection_board(leaf)? else {
