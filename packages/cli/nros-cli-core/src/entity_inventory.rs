...
//!
//! So the declaration is AUTHORED, and there are exactly two places to author
//! it. `nano_ros_node_register(... ENTITIES ...)` was the first and is RETIRED
//! -- phase-412 made that keyword a `FATAL_ERROR`, and phase-454 W9 removed the
//! last vestige of its metadata key. The two live surfaces are:
//!
//!   * the CONTRACT sidecar beside the launch file
//!     (`<bringup>/launch/<stem>.contract.yaml`), folded into the SystemModel
//!     and read by [`EntityInventory::from_model`] -- the road for an image
//!     that runs from a bringup;
//!   * `system.toml`'s `[[component]] entities` (RFC-0098 D8), read by
//!     `nros_orchestration_ir::leaf_system` -- the road for a standalone leaf,
//!     which reaches no bringup and therefore no contract.
//!
//! The component IDENTITY still travels `nros-metadata.json`; what that channel
//! no longer carries is the entity list.
//!
...
impl EntityInventory {
            ...
            return ReceivedTypes::Refused {
                reason: format!(
                    "{} receiving entities state no type, so the size of what they receive is \
                     ...
                     derives; a payload SIZE does. Name the type where the endpoint is \
                     declared: in the contract sidecar beside the launch file that runs this \
                     component (<bringup>/launch/<stem>.contract.yaml), or, for a standalone \
                     leaf, as `<kind>:<pkg>/msg/<Name>[:<name>]` in its `system.toml` \
                     `[[component]] entities`.",
                    untyped.len(),
                    untyped.join("\n")
                ),
    ...
    pub fn to_cmake(&self) -> String {
        ...
        s.push_str(
            "# WHICH ENTITIES THIS IMAGE CREATES, composed from the contract sidecar\n\
             # its bringup resolves (and, for a standalone leaf, from `system.toml`'s\n\
             # `[[component]] entities`). The bound inventory\n\
