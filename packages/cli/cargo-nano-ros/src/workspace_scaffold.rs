...
fn parameterize(lang: &str, rmw: &str, rel: &str, text: &str) -> Result<String> {
    ...
    match (lang, rel) {
        ...
        // No Rust entry anchor any more: the entry is GENERATED by `nros
        // build` (RFC-0065 D4), and its board crate and RMW features come from
        // the selection facade `nros sync` writes from `system.toml`'s `rmw` —
        // the one anchor above. The hand-written `robot_entry` this used to
        // rewrite was the second and third place that choice had to agree.
        _ => {}
    }
    Ok(out)
}
