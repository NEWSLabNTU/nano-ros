...
pub use cpp::{
    ...
    generate_cpp_action_package, generate_cpp_message_package,
...
mod tests {
    ...
    /// `tmp/borrowed_cpp_e2e/` for the host C++ E2E (235.8). `#[ignore]`:
    ///   cargo test -p rosidl-codegen emit_cpp_borrowed_e2e -- --ignored
    #[test]
    #[ignore = "emits files for the host C++ E2E driver"]
    fn emit_cpp_borrowed_e2e() {
        let msg =
            parse_message("uint32 width\nstring label\nuint8[] data\nfloat32[] ranges\n").unwrap();
        let resolver = crate::config::CapacityResolver::from_toml_str(
            r#"
        ...
        .unwrap();
        let pkg =
            generate_cpp_message_package("e2e_msgs", "Borrowed", &msg, "h", &resolver).unwrap();
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .ancestors()
            .nth(3)
            .unwrap()
