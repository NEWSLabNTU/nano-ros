 ...
 *
 * ESP-IDF bundles lwIP and exposes BSD-shaped wrappers via
 * `<lwip/sockets.h>` + `<lwip/netdb.h>`. Symbols are the same
 * `lwip_*` prefixed names as in nros-platform-freertos-c's net.c;
 * we use them directly.
 *
 * Storage layouts mirror nros-platform-freertos-c (same lwIP wire
 * format, so binaries that switch between the two ports without
 * recompilation see the same shape):
 *   endpoint = { struct addrinfo *iptcp; }
 *   socket   = { int fd; }
 */
...
int8_t nros_platform_tcp_create_endpoint(void *ep_raw,
    ...
    return lwip_getaddrinfo((const char *) address, (const char *) port,
...
int8_t nros_platform_tcp_open(void *sock_raw, const void *endpoint, uint32_t timeout_ms) {
    ...
    nros_esp_socket_t *sock = (nros_esp_socket_t *) sock_raw;
    const nros_esp_endpoint_t *ep = (const nros_esp_endpoint_t *) endpoint;
...
size_t nros_platform_tcp_read(const void *sock_raw, uint8_t *buf, size_t len) {
    ...
    const nros_esp_socket_t *sock = (const nros_esp_socket_t *) sock_raw;
...
size_t nros_platform_udp_read(const void *sock_raw, uint8_t *buf, size_t len) {
    ...
    int r = lwip_recvfrom(sock->fd, buf, len, 0, (struct sockaddr *) &raddr, &addrlen);
    return r < 0 ? NROS_PLATFORM_NET_SOCKET_ERROR : (size_t) r;
}
...
void nros_platform_udp_set_recv_timeout(const void *sock_raw, uint32_t timeout_ms) {
    ...
    if (timeout_ms == 0) {
        ...
        if (flags >= 0) (void) lwip_fcntl(sock->fd, F_SETFL, flags | O_NONBLOCK);
        return;
    }
...
/* ---- UDP multicast ----
 *
 * Same lwIP API as FreeRTOS-c. ESP-IDF's lwIP supports
 * IP_ADD_MEMBERSHIP; iface resolution skipped (lwIP doesn't
 * ship getifaddrs in default builds). Application can post-set
 * IP_MULTICAST_IF if needed.
 */
...
int8_t nros_platform_udp_mcast_open(void *sock_raw, const void *endpoint,
    ...
    nros_esp_socket_t *sock = (nros_esp_socket_t *) sock_raw;
    nros_esp_endpoint_t *lep = (nros_esp_endpoint_t *) lep_raw;
    const nros_esp_endpoint_t *ep = (const nros_esp_endpoint_t *) endpoint;
    if (ep->iptcp == NULL) return -1;
    struct addrinfo *ai = ep->iptcp;
...
int8_t nros_platform_socket_accept(const void *in_raw, void *out_raw) {
    ...
    const nros_esp_socket_t *in = (const nros_esp_socket_t *) in_raw;
    nros_esp_socket_t *out = (nros_esp_socket_t *) out_raw;
...
void nros_platform_network_poll(void) {
    /* ESP-IDF runs lwIP in its own thread — no-op. */
}
