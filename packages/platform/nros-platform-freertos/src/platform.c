...
int8_t nros_platform_task_init(void *task, void *attr,
     ...
     * identically-named field meant bytes. One conversion, in the one port that
     * needs it. */
    uint32_t stack_depth = (uint32_t) (stack_bytes / sizeof(StackType_t));
...
size_t nros_platform_task_storage_size(void) {
    return sizeof(nros_freertos_task_t);
}

size_t nros_platform_task_storage_align(void) {
    return _Alignof(nros_freertos_task_t);
}
...
size_t nros_platform_mutex_storage_size(void) { return sizeof(nros_freertos_mutex_t); }
size_t nros_platform_mutex_storage_align(void) { return _Alignof(nros_freertos_mutex_t); }
size_t nros_platform_mutex_rec_storage_size(void) { return sizeof(nros_freertos_mutex_t); }
size_t nros_platform_mutex_rec_storage_align(void) { return _Alignof(nros_freertos_mutex_t); }
size_t nros_platform_condvar_storage_size(void) { return sizeof(nros_freertos_condvar_t); }
size_t nros_platform_condvar_storage_align(void) { return _Alignof(nros_freertos_condvar_t); }

_Static_assert(NROS_PLATFORM_MUTEX_STORAGE_SIZE >= sizeof(nros_freertos_mutex_t),
               "NROS_PLATFORM_MUTEX_STORAGE_SIZE too small for this port");
_Static_assert(NROS_PLATFORM_MUTEX_REC_STORAGE_SIZE >= sizeof(nros_freertos_mutex_t),
               "NROS_PLATFORM_MUTEX_REC_STORAGE_SIZE too small for this port");
_Static_assert(NROS_PLATFORM_CONDVAR_STORAGE_SIZE >= sizeof(nros_freertos_condvar_t),
               "NROS_PLATFORM_CONDVAR_STORAGE_SIZE too small for this port");
_Static_assert(NROS_PLATFORM_TASK_STORAGE_SIZE >= sizeof(nros_freertos_task_t),
