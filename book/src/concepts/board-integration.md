...
| **ESP32-C3 user** | `cargo build --target riscv32imc-unknown-none-elf`, bare-metal over `esp-hal`. | [ESP32 (esp-hal)](../getting-started/esp32.md). The in-tree ESP-IDF component shell was [retired](../getting-started/integration-esp-idf.md) in phase-468 W2; an out-of-tree `idf.py` integration is still possible against the unchanged C API, but nano-ros ships no shell for it. |
