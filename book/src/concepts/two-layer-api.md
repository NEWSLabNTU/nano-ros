...
- **C++** — typed templates wrap each FFI surface: `nros::PollSubscription<M>` + `take`, `nros::PollService<S>` + `take_request` and `nros::PollClient<S>` + `send_request` for L1, `nros::PollingActionServer<A>` for the L1 action path (122.3.d.b), the L2 executor-registered callback model via the existing `nros::ActionServer<A>` API. The L1/L2 split is a TYPE split in C++ since phase-456: one class serving both owners offered the L1 method set on an L2 object, where the storage it reads is never filled.
