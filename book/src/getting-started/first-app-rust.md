...
fn main() {
    ...
    let mut executor =
        Executor::<_, 4, 4096>::open(&config).expect("Failed to open session");
...
- `Executor::<_, 4, 4096>` — up to 4 callback slots and a 4 KB callback
  arena. These are const generics — zero overhead when unused.
