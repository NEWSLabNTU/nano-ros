...
See the [Component + Entry pkg cookbook](../user-guide/component-and-entry-pkg.md) for the Entry-pkg surface.
...
- `nros-board-posix` — native host (Linux / macOS / *BSD); `init_transport`/`wait_link_up` no-ops.
...
- **Workspace shape + how an Entry pkg consumes a board** → [Component + Entry pkg cookbook](../user-guide/component-and-entry-pkg.md).
