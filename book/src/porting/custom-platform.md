...
The vtable header lives at
`packages/core/nros-platform-cffi/include/nros/platform_vtable.h` and
is **auto-generated by cbindgen on every build of `nros-platform-cffi`**
(Phase 93.I) — it is the single source of truth for the C porter.
Browse the rendered reference at
[/api/platform-cffi/](../api/platform-cffi/index.html) for per-field
return-value, threading, and blocking conventions.

Include the generated header in your port:
