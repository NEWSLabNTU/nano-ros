...
# NANO_ROS_BOARD=rv-virt-threadx. The platform module is what
...
if(NOT EXISTS "${_NROS_BOARD_CONFIG_DIR}/tx_user.h")
    message(FATAL_ERROR
        "nano-ros-board-rv-virt-threadx: tx_user.h not found at "
        "${_NROS_BOARD_CONFIG_DIR}/tx_user.h.")
endif()
if(NOT EXISTS "${_NROS_PORT_RISCV64_DIR}/inc/tx_port.h")
    message(FATAL_ERROR
        "nano-ros-board-rv-virt-threadx: forked tx_port.h not found at "
...
if(NOT EXISTS "${_NROS_BOARD_CONFIG_DIR}/link.lds")
    message(FATAL_ERROR
        "nano-ros-board-rv-virt-threadx: linker script not found at "
        "${_NROS_BOARD_CONFIG_DIR}/link.lds.")
endif()
if(NOT EXISTS "${_NROS_BOARD_STARTUP_C}")
    message(FATAL_ERROR
        "nano-ros-board-rv-virt-threadx: startup.c not found at "
        "${_NROS_BOARD_STARTUP_C}.")
endif()
if(NOT EXISTS "${_NROS_BOARD_C_DIR}/board_threadx_qemu_riscv64.c")
    message(FATAL_ERROR
        "nano-ros-board-rv-virt-threadx: board_threadx_qemu_riscv64.c not found at "
...
"/*\n"
" * Auto-generated by cmake/board/nano-ros-board-rv-virt-threadx.cmake\n"
...
" * Phase 214.P block in nano-ros-board-rv-virt-threadx.cmake).\n"
" */\n"
"\n"
"#include <stdint.h>\n"
...
# ---------------------------------------------------------------------------
function(nros_threadx_rv64_rust_app target)
    cmake_parse_arguments(_A "" "CRATE;DOMAIN" "LINK" ${ARGN})
    if(NOT _A_CRATE)
        message(FATAL_ERROR
            "nros_threadx_rv64_rust_app(${target}): CRATE <cargo-package> is required.")
    endif()

    # phase-369 W1 — the RMW is a PARAMETER, not a constant.
    #
    # This seam was `…_rust_cyclone_app` and hardcoded both the crate features
    # and the link call to cyclonedds, which is why the zenoh build of the same
    # six leaves had to go through cargo instead — the two-build-path shape
    # issue 0666 records, and the reason 0688's panic policy was missed on one
    # of them.
    #
    # The feature set differs per RMW and is NOT derivable by pattern: cyclone
    # allocates and must name `alloc` (phase-361 W8.e made `rmw-cyclonedds` stop
    # enabling the heap for you), zenoh does not. These are the same sets the
    # cargo rows in `examples/fixtures.toml` use, so the CMake path produces the
    # image the cargo path produced.
    if(NROS_RMW STREQUAL "cyclonedds")
        set(_rv64_crate_features rmw-cyclonedds alloc)
    elseif(NROS_RMW STREQUAL "zenoh")
        set(_rv64_crate_features rmw-zenoh)
    else()
        # Loudly, rather than importing a crate with no backend and failing at
        # link with an undefined RMW symbol several minutes later.
        message(FATAL_ERROR
            "nros_threadx_rv64_rust_app(${target}): NROS_RMW='${NROS_RMW}' has no "
            "feature set here. Add one beside `zenoh`/`cyclonedds` — a leaf's "
            "features are its own, so this cannot be inferred.")
    endif()
    ...
    corrosion_import_crate(
        ...
        # phase-361 W8.e — `alloc` is spelled HERE, by the consumer, because
...
nros_riscv64_rustflags_env(${_crate_target}-static)
    ...
    if(NOT EXISTS "${_anchor}")
        file(WRITE "${_anchor}"
            "/* Generated by nros_threadx_rv64_rust_app — link anchor only. */\n"
