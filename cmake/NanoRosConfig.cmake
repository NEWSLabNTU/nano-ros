...
function(nano_ros_read_config CONFIG_FILE)
    ...
    foreach(_line IN LISTS _lines)
        ...
        # Section header. Phase 172.K — also handle the array-of-tables
        # `[[transport]]` and dotted `[node.rt]` of the direct-mode nros.toml
        # (section name normalises to `transport` / `node.rt`).
        if("${_line}" MATCHES "^\\[+([a-z._]+)\\]+")
        ...
        if("${_line}" MATCHES "^([a-z_]+)[ \t]*=[ \t]*(.*)")
            ...
            # [node] section (Phase 172.K — direct-mode nros.toml only; legacy
            # [network]/[zenoh]/[platform]/[scheduling] dropped in K.6)
            if("${_section}" STREQUAL "node")
                if("${_key}" STREQUAL "domain_id")
                    set(_domain_id "${_val}")
                endif()
            # [[transport]] section (Phase 172.K)
            elseif("${_section}" STREQUAL "transport")
                if("${_key}" STREQUAL "ip")
                    # CIDR "10.0.2.20/24" → address + prefix(+netmask)
                    if("${_val}" MATCHES "^([^/]+)/([0-9]+)$")
                        _nros_ip_to_c("${CMAKE_MATCH_1}" _ip)
                        set(_prefix "${CMAKE_MATCH_2}")
                        _nros_prefix_to_netmask("${CMAKE_MATCH_2}" _netmask)
                    else()
                ...
                elseif("${_key}" STREQUAL "interface")
                    # Legacy single NIC. Mirror into the list so downstream sees
                    # one consistent shape. Phase 172.K.7.
                    set(_interface "${_val}")
                    set(_interfaces "${_val}")
                elseif("${_key}" STREQUAL "interfaces")
                    # TOML array `interfaces = ["eth0", "eth1"]` (multi-homing).
                    # Strip the brackets, split on commas, unquote each entry into
                    # a CMake list. Phase 172.K.7.
            ...
            # [node.rt] section
            elseif("${_section}" STREQUAL "node.rt")
