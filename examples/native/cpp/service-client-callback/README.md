# `service-client-callback` — native / cpp
...
```bash
cp -r examples/native/cpp/service-client-callback ~/my-service-client-callback && cd ~/my-service-client-callback
...
(`[image.native]` + `[system]`, RFC-0098 D3/D5). `find_package(nano_ros)`
