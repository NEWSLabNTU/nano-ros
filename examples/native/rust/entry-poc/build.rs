//! Phase 212.N.7 step-1 — Entry pkg `build.rs`.
//!
...
//! `<NativeBoard as BoardEntry>::run`'s setup closure.
//!
//! This POC ships a trivial `launch.xml` with no `<node>` entries so
//! the emitted `run_plan` body is empty (`Ok(())`). The shape of the
//! integration is the deliverable; component migration follows in
//! the 212.N.7 wave 4 sweep.

fn main() {
    let launch = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("launch/system.launch.xml");
    ...
    match nros_build::generate_run_plan(&launch) {
        ...
        Err(err) => {
            // POC: don't fail the build on a missing/empty launch
            // file — fall through to an empty placeholder so the
            // Entry pkg's `main.rs` still compiles. Production
