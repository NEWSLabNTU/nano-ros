//! Native Talker — the official ROS 2 `demo_nodes_cpp` talker.
//!
//! Publishes `std_msgs/String` (`Hello World: N`) on `/chatter` once per
//! second.
//!
//! Node pkg shape: `register()` declares node + publisher + timer;
//! `ExecutableNode::on_callback("on_tick")` runs the timer body. `main.rs`'s
//! `nros::main!()` and the board own `nros::init`, executor open, RMW
//! registration and the spin loop — the user authors *only* the declarative
//! + body bits.
//!
//! phase-338 W3 — was an `[package.metadata.nros.application]` example written
//! against the imperative Executor API, the only shape that differed from every
//! other platform's copy. Now Node-class like the rest, so `talker` means one
//! thing everywhere. The imperative API stays documented by the genuinely
//! application-shaped examples (`custom-transport-*`).
