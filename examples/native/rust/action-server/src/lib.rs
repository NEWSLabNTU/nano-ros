//! Native action-server example — shared logic for both build paths
//! (Phase 170.A). `run()` is shared by the pure-cargo `fn main()`
//! (zenoh/xrce) and the Cyclone DDS `rust_main()` C entry. Computes a
//! Fibonacci sequence with feedback.
...
/// Action-server body — serve the `Fibonacci` action on `/fibonacci`,
/// publishing feedback as it computes. Loops forever.
pub fn run() -> ! {
