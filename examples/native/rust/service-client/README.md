...
```bash
cp -r examples/native/rust/service-client ~/my-service-client && cd ~/my-service-client
...
`nros build` leaves the binary at `build/native/target/debug/service-client`.
