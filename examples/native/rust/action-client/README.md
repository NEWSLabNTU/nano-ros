...
```bash
cp -r examples/native/rust/action-client ~/my-action-client && cd ~/my-action-client
...
`nros build` leaves the binary at `build/native/target/debug/action-client`.
