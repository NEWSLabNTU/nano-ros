# `service-server-entry` — qemu-arm-freertos / rust
...
```bash
cp -r examples/qemu-arm-freertos/rust/service-server-entry ~/my-service-server-entry && cd ~/my-service-server-entry
...
QEMU / flashing steps live in the [qemu-arm-freertos README](https://github.com/NEWSLabNTU/nano-ros/blob/main/examples/qemu-arm-freertos/README.md).
