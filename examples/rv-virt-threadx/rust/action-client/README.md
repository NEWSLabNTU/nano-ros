# `action-client` — rv-virt-threadx / rust
...
```bash
cp -r examples/rv-virt-threadx/rust/action-client ~/my-action-client && cd ~/my-action-client
...
`build/rv-virt-threadx/target/<triple>/debug/rv-virt-threadx-action-client`
