# `service-server` — rv-virt-threadx / rust
...
```bash
cp -r examples/rv-virt-threadx/rust/service-server ~/my-service-server && cd ~/my-service-server
...
`build/rv-virt-threadx/target/<triple>/debug/rv-virt-threadx-service-server`
