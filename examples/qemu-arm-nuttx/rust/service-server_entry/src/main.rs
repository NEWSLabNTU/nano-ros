//! Phase 212.N.7 step-2 — Entry pkg for the NuttX QEMU ARM service-server.
//!
//! Sibling to the `service-server` Component pkg. See the talker_entry
...
fn main() {
    ...
    if let Err(err) = outcome {
        eprintln!("nuttx_rs_service_server_entry: run_plan failed: {err:?}");
        std::process::exit(1);
    }
}
...
fn main() {
    eprintln!(
        "nuttx_rs_service_server_entry: this binary only runs on target_os = \"nuttx\"; \
