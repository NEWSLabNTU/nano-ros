//! ESP-IDF talker — Phase 212.M.7 Entry pkg.
//!
...
pub extern "C" fn rust_app_main() -> i32 {
    ...
    // declare a `std_msgs/Int32` publisher on `/chatter`, register a
    // 1 Hz timer and spin. The pre-212 `esp-hal` code is preserved
    // in git history (commit before this migration) for reference.
    0
}
