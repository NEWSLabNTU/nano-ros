/// Talker — typed C component (RFC-0043, phase-240.6 / 247).
///
...
/// `Publisher<Int32>` registers, so the cpp_listener raw sub matches. The raw
/// publish carries the wire bytes verbatim, so no generated message sources are
/// needed. Replaces the legacy `register_talker` + `record_callback_effect`
/// declarative seam.
