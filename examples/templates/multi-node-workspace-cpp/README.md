...
- `src/robot_entry/` — the real `main()` is generated by
  `nano_ros_entry(… TYPED BRINGUP … LAUNCH default)`, which resolves the
  bringup's launch file into a SystemModel under the build tree at configure
  time (SystemModels are build artifacts — never committed).
