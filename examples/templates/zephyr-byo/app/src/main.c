/**
 * @file main.c
 * @brief nano-ros Zephyr starter — zenoh C talker (Phase 205.A).
 *
 * Publishes a std_msgs/Int32 on `/chatter` once per second. Copy this app, edit
 * the topic/message/logic. Booting it needs a zenoh router at
 * CONFIG_NROS_ZENOH_LOCATOR (e.g. `zenohd -l tcp/127.0.0.1:7456`).
 */
...
LOG_MODULE_REGISTER(app_talker, LOG_LEVEL_INF);

/* nano-ros's NROS_CHECK_* macros log via this hook. */
...
int nros_app_main(int argc, char** argv) {
    ...
    LOG_INF("nano-ros BYO talker (zenoh)");
    ...
    NROS_CHECK_RET(nros_node_init(&node, &support, "byo_talker", "/"), 1);
