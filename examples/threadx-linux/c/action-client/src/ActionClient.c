/// @file ActionClient.c
/// @brief ThreadX Linux C action client component — declarative Node pkg (Phase 244 D6).
///
/// Sends `example_interfaces/Fibonacci` goals to `/fibonacci`. The generated
/// runtime (emitted by `nros_threadx_codegen_system`) owns init / executor /
/// spin; this file declares the node + action client and exports the register
...
static nros_ret_t register_action_client(nros_node_context_t *ctx) {
    nros_node_pkg_options_t opts = nros_node_pkg_options("c_action_client");
    ...
    return nros_declared_node_create_action_client_for_name(
        &node, &cli, "/fibonacci", "example_interfaces/action/Fibonacci", "");
}
