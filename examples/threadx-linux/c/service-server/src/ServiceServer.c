...
/// @brief ThreadX Linux C service server component — declarative Node pkg (Phase 244 D6).
///
/// Handles `example_interfaces/AddTwoInts` requests on `/add_two_ints`. The
/// generated runtime (emitted by `nros_threadx_codegen_system`) owns init /
/// executor / spin; this file declares the node + service server and exports
/// the register trampoline via `NROS_NODE_REGISTER`.
...
static nros_ret_t register_service_server(nros_node_context_t *ctx) {
    nros_node_pkg_options_t opts = nros_node_pkg_options("c_service_server");
