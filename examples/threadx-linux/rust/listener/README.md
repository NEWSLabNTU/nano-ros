...
```bash
cp -r examples/threadx-linux/rust/listener ~/my-listener && cd ~/my-listener
...
`nros build` leaves the binary at `build/threadx-linux/target/debug/listener`.
