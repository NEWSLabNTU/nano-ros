# `action-server` — threadx-linux / rust
...
```bash
cp -r examples/threadx-linux/rust/action-server ~/my-action-server && cd ~/my-action-server
...
`nros build` leaves the binary at `build/threadx-linux/target/debug/action-server`.
