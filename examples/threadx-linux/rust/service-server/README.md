# `service-server` — threadx-linux / rust
...
```bash
cp -r examples/threadx-linux/rust/service-server ~/my-service-server && cd ~/my-service-server
...
`nros build` leaves the binary at `build/threadx-linux/target/debug/service-server`.
