...
```bash
cp -r examples/threadx-linux/rust/talker ~/my-talker && cd ~/my-talker
...
`nros build` leaves the binary at `build/threadx-linux/target/debug/talker`.
