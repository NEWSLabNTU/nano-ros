# `service-client` — threadx-linux / rust
...
```bash
cp -r examples/threadx-linux/rust/service-client ~/my-service-client && cd ~/my-service-client
...
`nros build` leaves the binary at `build/threadx-linux/target/debug/service-client`.
