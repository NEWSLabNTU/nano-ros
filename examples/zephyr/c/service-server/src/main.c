...
static bool service_callback(const uint8_t* request_data, size_t request_len,
    ...
    NROS_LOG_INFO(g_logger, "Request [%d]: %lld + %lld = %lld", g_request_count,
...
int nros_app_main(int argc, char** argv) {
    ...
    NROS_CHECK_RET(nros_support_init_named(&support,
                                           ...
                                           CONFIG_NROS_DOMAIN_ID, "xrce_service_server"),
                   1);
...
#error "Phase 168.4 requires CONFIG_NROS_RMW_{ZENOH,XRCE,CYCLONEDDS}=y"
    ...
    NROS_CHECK_RET(
        nros_service_init(&service, &node, &type, "/add_two_ints", service_callback, NULL), 1);
