                             ...
                             size_t* response_len, void* context)
{
    ...
    NROS_LOG_INFO(g_logger, "Request [%d]: %lld + %lld = %lld",
        g_request_count, (long long)request.a, (long long)request.b, (long long)response.sum);
    int32_t len = example_interfaces_srv_add_two_ints_response_serialize(
        &response, response_data, response_capacity);
    if (len < 0) { LOG_ERR("Failed to serialize response"); return false; }
...
int nros_app_main(int argc, char **argv)
{
    ...
    LOG_INF("nros Zephyr Service Server");
    ...
    NROS_CHECK_RET(nros_support_init_named(&support,
        ...
        CONFIG_NROS_DOMAIN_ID, "xrce_service_server"), 1);
#else
#error "Phase 168.4 requires CONFIG_NROS_RMW_{ZENOH,XRCE}=y"
    ...
    NROS_CHECK_RET(nros_executor_register_service(&executor, &service), 1);
