...
int nros_app_main(int argc, char** argv) {
    ...
    NROS_CHECK_RET(nros_support_init_named(&support,
                                           ...
                                           CONFIG_NROS_DOMAIN_ID, "xrce_service_client"),
                   1);
...
#error "Phase 168.4 requires CONFIG_NROS_RMW_{ZENOH,XRCE,CYCLONEDDS}=y"
    ...
    size_t resp_len = 0;
    nros_ret_t ret =
        nros_client_call(&client, req_buf, (size_t)req_len, resp_buf, sizeof(resp_buf), &resp_len);
    if (ret == NROS_RET_OK) {
        ...
        if (example_interfaces_srv_add_two_ints_response_deserialize(&response, resp_buf,
                                                                     resp_len) == 0) {
            NROS_LOG_INFO(g_logger, "Result: %lld + %lld = %lld", (long long)request.a,
