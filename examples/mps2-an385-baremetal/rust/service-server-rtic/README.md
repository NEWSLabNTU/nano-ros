# `service-server-rtic` — mps2-an385-baremetal / rust
...
```bash
cp -r examples/mps2-an385-baremetal/rust/service-server-rtic ~/my-service-server-rtic && cd ~/my-service-server-rtic
...
`build/mps2-an385-baremetal/target/<triple>/debug/qemu-rtic-service-server`
