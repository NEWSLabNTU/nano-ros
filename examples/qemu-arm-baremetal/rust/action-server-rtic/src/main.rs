//! RTIC Fibonacci action-server entry for nros on QEMU MPS2-AN385 (phase-244.D1).
//!
...
//! linked RMW, and runs the `action_server_rtic_pkg` node's `register` + RTIC
//! dispatch loop. Locator/domain come from the deploy overlay — no hardcoded
//! `Config`, no manual `Executor::open`, no hand-written `try_accept_goal` loop.
