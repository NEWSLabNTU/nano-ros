//! STM32F4 Action Client Node pkg.
//!
...
//! `(RTIC, Inline)` is the same matrix cell `nros check` already
//! accepts for `talker-rtic`, and there is no Spawner /
//! RTIC-`task::spawn()` handoff to plumb here.
//!
...
//! `example_interfaces::action::Fibonacci` ships (a follow-up wave
...
//!
//! This example ships the trait-shaped scaffolding only. `register`
...
//! `#[task]` body added by a follow-up wave once the
...
/// (eventually) polls for feedback + result. Skeleton.
pub struct ActionClient;

impl Node for ActionClient {
    const NAME: &'static str = "fibonacci_action_client";

    /// Declares Inline dispatch. The client side has no callbacks on
    /// the spin path (`try_recv*` loops drive goal-acceptance,
    /// feedback, and result), so `(RTIC, Inline)` matches the matrix
    /// cell `nros check` already accepts for `talker-rtic`. See the
    /// module doc for the rationale + Embassy-side parity story.
    const DISPATCH: DispatchStrategy = DispatchStrategy::Inline;

    fn register(ctx: &mut NodeContext<'_>) -> NodeResult<()> {
        let mut node = ctx.create_node(NodeOptions::new("fibonacci_action_client"))?;
        // Action client uses the name-shaped builder (no
        // `create_action_client_static` exists: clients need a USABLE
        // handle, not just a tag, to dispatch goals — see
        ...
        // threaded through yet; a follow-up wave moves those
...
impl ExecutableNode for ActionClient {
    /// Inline pkgs without per-tick state carry `()`. Follow-ups grow
    /// this into an `Option<NodeActionClient<'static, PlaceholderAct>>`
    /// (plus a goal-counter / state-machine field) once the framework
    ...
    fn on_callback(
        ...
        // Inline client has no callbacks: goal-accept, feedback, and
        // result are driven by `try_recv*` loops from an
        // Entry-pkg-owned async task (TODO documented in the
