...
| `esp32/{c,cpp}/*`                                      | Phase 212.M.7 migrated `examples/esp32/rust/` from the bare-metal `esp-hal` shape to the ESP-IDF `idf.py` workflow via `integrations/nano-ros/`. C/C++ harness cells are no longer blocked by a missing hosted runtime (IDF provides FreeRTOS + lwIP), but no C/C++ esp32 example has been written yet.                                                                                                          | A small C/C++ example sweep mirroring `examples/qemu-arm-freertos/{c,cpp}/*` against the IDF integration shell. Tracked as a Phase 212.M.7 follow-up. |
