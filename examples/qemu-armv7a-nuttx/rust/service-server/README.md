# `service-server` — qemu-armv7a-nuttx / rust
...
```bash
cp -r examples/qemu-armv7a-nuttx/rust/service-server ~/my-service-server && cd ~/my-service-server
...
`build/qemu-armv7a-nuttx/target/<triple>/debug/service-server`
