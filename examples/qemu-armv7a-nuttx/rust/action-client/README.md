# `action-client` — qemu-armv7a-nuttx / rust
...
```bash
cp -r examples/qemu-armv7a-nuttx/rust/action-client ~/my-action-client && cd ~/my-action-client
...
`build/qemu-armv7a-nuttx/target/<triple>/debug/action-client`
