# `listener` — qemu-armv7a-nuttx / rust
...
```bash
cp -r examples/qemu-armv7a-nuttx/rust/listener ~/my-listener && cd ~/my-listener
...
`build/qemu-armv7a-nuttx/target/<triple>/debug/listener`
