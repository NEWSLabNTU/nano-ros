# `action-server` — qemu-armv7a-nuttx / rust
...
```bash
cp -r examples/qemu-armv7a-nuttx/rust/action-server ~/my-action-server && cd ~/my-action-server
...
`build/qemu-armv7a-nuttx/target/<triple>/debug/action-server`
