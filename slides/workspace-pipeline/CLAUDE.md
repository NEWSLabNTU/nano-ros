...
4. **Machine** — metadata → launch scan → codegen → link → RT schedule · RT
   config flow (tiers, `ws-realtime-rust`) · safety spec E2E (`features=["safety"]`,
   `ws-safety-rust`, RFC-0028) · board adapters
...
- **All code on slides is REAL — pulled from the tree, never invented.** Sources:
  ...
  `SchedContext` are the load-bearing ones. RT/safety act adds: `callback_group`,
  `callback_groups`/`tiers.<t>` (`ws-realtime-rust`), `run_tiers`,
  `create_subscription_for_callback_name_with_safety`, `ctx.integrity()`/
  `IntegrityStatus`, `features=["safety"]`→`safety-e2e`/`NROS_SYSTEM_SAFETY_E2E`.
