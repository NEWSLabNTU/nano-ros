...
# RT + safety — where you, the user, plug in

<div class="text-sm opacity-70 mb-2">Both are <b>declarative config</b>, not node logic: edit a file → capability registry lowers it → codegen wires it → the board enforces it. Big picture first; the next two slides walk each end to end.</div>
...
```mermaid {scale: 0.52}
flowchart TB
  A["demo_bringup/system.toml<br/>[tiers.*] · features=[…]"] --> R
  B["node Cargo.toml<br/>callback_groups"] --> R
  C["node lib.rs<br/>callback_group() · .with_safety"] --> R
  R{"capability registry<br/>+ codegen"} --> BIN["binary<br/>tiers scheduled · CRC on wire"]
  style R fill:#6b21a8,color:#fff
  style BIN fill:#9f1239,color:#fff
```
...
| Want | Edit | Knob |
|---|---|---|
| **RT** tier rate / prio | `system.toml [tiers.*]` | `priority` · `spin_period_us` |
| ↳ map callbacks → tier | node `Cargo.toml` + `lib.rs` | `callback_groups` · `callback_group()` |
| **Safety** E2E on | `system.toml [system]` | `features = ["safety"]` |
| ↳ read the verdict | node `lib.rs` | `.with_safety` sub · `ctx.integrity()` |

<div class="mt-3 opacity-60 text-xs">Three files at most. Node <i>logic</i> never changes — only config + a label/opt-in.</div>
...
<!--
The map. Audience sees the whole config surface before any detail — where RT
knobs live (tiers + group label) and where safety lives (one feature flag + the
integrity read). Then drill into each.
-->

---

# Real-time — declared in config, scheduled by the board

<div class="text-sm opacity-70 mb-1">Where the <code>SchedContext</code> comes from: <code>ws-realtime-rust</code> — a 10 ms control loop + 100 ms telemetry on two priority tiers. <span class="opacity-60">(RFC-0015)</span></div>

<div class="grid grid-cols-2 gap-4 text-sm">

<div>
<div class="opacity-70 mb-1">Node pkg — names its group → tier</div>
...
```rust
// ctrl_pkg/src/lib.rs — label entities at register
node.callback_group("ctrl")?;
node.create_timer_for_callback_name(
    "on_ctrl", TimerDuration::from_millis(10))?;
```

</div>

<div>
<div class="opacity-70 mb-1">Bringup — per-tier, per-RTOS knobs</div>

```toml
# demo_bringup/system.toml
[tiers.high]
spin_period_us = 1000      # 1 ms spin
[tiers.high.posix]
priority = 80
[tiers.low]
spin_period_us = 10000     # 10 ms spin
[tiers.low.posix]
priority = 10
```
...
<div class="mt-3 p-3 bg-blue-400/10 rounded text-sm">
<code>nros::main!()</code> reads both — group→tier (node) + the per-tier table (bringup) — and emits <code>run_tiers</code> (one priority task per tier) instead of single-tier <code>run</code>. <b>Retune priority / period by editing <code>system.toml</code> — node code untouched.</b> Native = advisory; FreeRTOS / ThreadX = real task priorities.
</div>

<!--
Mixed-criticality without code coupling. Same declare→lower→schedule pattern
as everything else: config is the SSoT, codegen wires it, board enforces it.
-->

---

# Safety spec — end to end, one flag

<div class="text-sm opacity-70 mb-1">Treat the transport as an untrusted black channel (EN 50159). <code>ws-safety-rust</code> turns it on once. <span class="opacity-60">(RFC-0028 · capability registry RFC-0031)</span></div>

<div class="grid grid-cols-2 gap-4 text-sm">

<div>
<div class="opacity-70 mb-1">Declared once in bringup → lowered everywhere</div>

```toml
# demo_bringup/system.toml
[system]
rmw = "zenoh"
features = ["safety"]      # ← the whole opt-in
```

```text
features = ["safety"]   (capability registry)
  → Rust   nros/safety-e2e
           nros-rmw-zenoh/safety-e2e   # CRC attach + validate
  → C/C++  #define NROS_SYSTEM_SAFETY_E2E
           cmake   NANO_ROS_SAFETY_E2E
```

</div>

<div>
<div class="opacity-70 mb-1">Read per-message integrity in the callback</div>

```rust
// safe_listener_pkg/src/lib.rs
node.create_subscription_for_callback_name_with_safety
    ::<Int32>("on_chatter", "/chatter")?;

#[cfg(feature = "safety-e2e")]
if let Some(status) = ctx.integrity() {   // IntegrityStatus
    if status.is_valid() { /* CRC ok · no gap · no dup */ }
    else { state.integrity_faults += 1; }
}
```
...
<div class="grid grid-cols-2 gap-4 text-xs mt-2">
<div class="p-2 bg-green-400/10 rounded">
<b>On the wire</b> — RMW attachment <code>[seq 8][ts 8][gid 16]</code> + <code>[crc32 4]</code> (33→37 B). Publish attaches, receive validates CRC + sequence: corruption · repetition · deletion · resequencing.
</div>
<div class="p-2 bg-amber-400/10 rounded">
<b>Interop</b> — graceful degradation: plain ROS 2 / non-safety nano-ros ignores the extra bytes, <code>integrity()</code> = <code>None</code>. No flag day.
</div>
</div>

<div class="text-xs opacity-50 mt-1">Subscription is ungated (off ⇒ a basic sub); the integrity read is gated on <code>safety-e2e</code>. Zenoh only. Same declare→lower→enforce pattern as tiers.</div>

<!--
The differentiator slide. One flag flows through codegen → backend → ROS API.
Publisher code is unchanged; the backend attaches CRC. Subscriber opts in to the
read. EN 50159 black-channel framing resonates with the safety-island audience.
-->
...
- **RT-aware** — priority tiers in `system.toml` → `SchedContext` (class · priority · budget · deadline); retune without touching node code.
- **Safety E2E** — `features = ["safety"]` → CRC + sequence on the wire; read `ctx.integrity()` in any callback.
...
<div class="text-sm opacity-50 mt-4">RFC-0001 · 0023 · 0032/0043 · 0015 · 0016/0017 · 0028/0031 — docs/design/</div>
